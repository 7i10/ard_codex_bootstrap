...
def train_job(seed: int, history: bool) -> dict[str, object]:
    ...
    command = [
        ...
        str(root),
        "--ordering-policy",
        "history_balanced_v1" if history else "none",
    ]
    return {
        "job_id": f"train-{arm}-s{seed}",
        "run_id": f"ert-rslad-history-ordering-new-{arm}-s{seed}",
        "host": "hamster",
        "command": command,
        "cwd": str(REPO),
        "env": ENV,
        "required_paths": [str(root / "last.pt")],
        "output_dir": str(root),
        ...
        "scientific_identity": {
            "method_id": "I100_sample_keyed_rng",
            "arm": "NEW_HISTORY" if history else "NEW_CONTROL",
            "seed": seed,
            "ordering_policy": "history_balanced_v1" if history else "epoch_shuffle_control",
...
def endpoint_job(seed: int, history: bool, epoch: int) -> dict[str, object]:
    ...
    command = [
        ...
        "--config",
        str(root / "resolved_config.yaml"),
        "--checkpoint",
        str(root / f"epoch-{epoch}.pt"),
        "--output",
        str(endpoint_root),
        "--expected-epoch",
        str(epoch - 1),
    ...
    return {
        "job_id": f"endpoint-{arm}-s{seed}-e{epoch}",
        "run_id": f"ert-rslad-history-ordering-endpoint-{arm}-s{seed}-e{epoch}",
        "host": "hamster",
        "command": command,
        "cwd": str(REPO),
        "env": ENV,
        "required_paths": [],
        "output_dir": str(endpoint_root),
        "completion_marker": "orchestration/completion.json",
        "dependencies": [f"train-{arm}-s{seed}"],
        "estimated_work": 10,
        "retry_policy": {"max_attempts": 2},
        "scientific_identity": {
            "method_id": "CE-PGD20",
...
def main() -> None:
    ...
    jobs.append(
        {
            "job_id": "aggregate-report",
            "run_id": "ert-rslad-history-ordering-aggregate-report",
            "host": "hamster",
            "gpu_count": 0,
            "command": [PYTHON, "scripts/aggregate_history_ordering_campaign.py"],
            "cwd": str(REPO),
            "env": {"PYTHONPATH": str(REPO / "src"), "PYTHONUNBUFFERED": "1"},
            "output_dir": str(RUN_ROOT / "aggregate"),
            "completion_marker": "orchestration/completion.json",
            "dependencies": [job["job_id"] for job in jobs if str(job["job_id"]).startswith("endpoint-")],
            "estimated_work": 1,
            "scientific_identity": {"method_id": "aggregate_report", "campaign": "ert-rslad-history-ordering-dev-v2"},
        }
    )
    manifest = {
        "schema_version": 1,
        "campaign_id": "ert-rslad-history-ordering-dev-v2",
        "source": {
            "git_sha": SOURCE,
            "scientific_source_note": "Frozen source commit; fork lineage records the exact clean source "
        ...
        "hosts": {
            "hamster": {
                ...
                "gpus": [
                    {"index": 0, "uuid": "GPU-7ce112db-1ab9-ff86-9810-5bb92e222c2a", "throughput": 679.0},
                    {"index": 1, "uuid": "GPU-9bb01a06-72d2-cc68-eeca-9c2c9daf37ce", "throughput": 679.0},
                ],
            }
        },
        "jobs": jobs,
