...
def base_config() -> dict:
    return {
        ...
        "method": {"name": "pgd_at", "attack": {"epsilon": "8/255", "step_size": "2/255", "steps": 1}},
...
def test_strict_config_env_override_and_resolved_rationals(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = load_config(path, ["seed=7", "method.attack.random_start=false"])
...
def test_real_dataset_normalization_is_required_in_dev_smoke_and_teacher_must_match(tmp_path: Path) -> None:
    base = {
        "tier": "dev",
        "dataset": {"name": "cifar10", "root": str(tmp_path), "num_classes": 10},
    ...
    write_yaml(path, base)
    with pytest.raises(ValidationError, match="teacher and student normalization profiles to match"):
        load_config(path)

    base["tier"] = "smoke"
    base["teacher"]["normalization"] = {"profile": "cifar10_standard"}
    write_yaml(path, base)
    assert load_config(path).student.normalization.profile == "cifar10_standard"
