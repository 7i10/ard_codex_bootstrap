...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--run", required=True, action="append", type=_labeled_path, help="L1=state-export.json (repeat L1--L4)."
    )
    parser.add_argument(
        "--manifest",
        required=True,
        action="append",
        type=_labeled_path,
        help="L1=run-bundle/manifest.json (repeat L1--L4).",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    for label, path in args.manifest:
        if label in manifest_paths:
            raise LoggingOnlyPredictionError("duplicate H2 manifest label")
        manifest_paths[label] = path.resolve()
    report = analyze_logging_only_exports(exports, manifest_paths=manifest_paths)
