"""Build the frozen Stage A teacher-response modifier report."""
...
from ard.analysis.ert_stage_a_modifiers import build_modifier_report
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--state-l2", type=Path, required=True)
    parser.add_argument("--state-l4", type=Path, required=True)
    parser.add_argument("--mask-l2", type=Path, required=True)
    parser.add_argument("--mask-l4", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args(argv)
    result = build_modifier_report(
        endpoint_root=args.endpoint_root,
        endpoint_report=args.endpoint_report,
        state_paths={"L2": args.state_l2, "L4": args.state_l4},
