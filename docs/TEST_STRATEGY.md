...
The taxonomy gate also rejects runnable `configs/reproduction`, requires exactly two audit configs, two pilot configs,
and eight teacher-explicit production configs. It checks both one-GPU (per-rank 128) and two-GPU (per-rank 64) execution
profiles without executing training.
...
| Changed path | 必須検証 |
|---|---|
| `external.lock.yaml` | external management、teacher registry、tracking lineage、SAAD/TRADES upstream regression |
| `teachers.lock.yaml` / teacher acquisition scripts | teacher registry/acquisition、tracking lineage |
...
| `pyproject.toml` / `requirements/**` | bounded T0–T3 dependency-sensitive config/data/model/tracking/evaluation/smoke coverage |
...
- broad tier/failed gateでは`src/**`、`configs/**`、`requirements/**`、`scripts/**`、`tests/**`、
  `pyproject.toml`、`Makefile`、`.gitignore`、`external.lock.yaml`、`teachers.lock.yaml`
- Python/platform/PyTorch/CUDA/GPU capability
- every locked external repository's locked URL/SHA and observed HEAD/origin/dirty state
- `PYTHONPATH`, `PYTHONHASHSEED`, `CUDA_VISIBLE_DEVICES`, `WANDB_MODE`とupstream-oracle opt-in flags（存在するもの）
...
mixed `--changed` gateでは`docs/**`をtest fingerprintから除外します。docs自体はdocs-only T0選択で
GPU testを生成せず、source/config/test/lockと同時に更新した証跡メモも成功済みtestを無効化しません。
...
freeze済みです。最終host gate `scripts/verify.py --changed --force --non-scientific`は18 commandsを実行し、
...
- TRADES source evidence is opt-in and deterministic: before cloning, the focused core command reported
  `40 passed, 1 skipped`; after bootstrapping the pinned checkout,
  `ARD_TRADES_SOURCE_EVIDENCE=1 ... test_trades_upstream_differential.py` reported `4 passed`.
  `python scripts/verify_external.py --all` passed for both locked repositories.
...
T4、T5、CIFAR本訓練、real full AutoAttack、dependency-complete full SAAD/TRADES runtime parity、monolithic full pytest suiteは
