...
Diagnostics policy is explicit: smoke/dev uses `diagnostics_mode: off`; reproduction and production templates use
fixed-ID `panel` diagnostics with panel size 24 and artifact interval 5. No W&B online run, CIFAR training run, or
real full AutoAttack has been executed in this bootstrap.
