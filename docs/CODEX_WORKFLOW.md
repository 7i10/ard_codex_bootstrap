...
reviewerがそのturn内に結論を返さなかった場合、その試行をapprovalとして記録しない。同じdeltaへreviewerを連続投入せず、
主スレッドで安全確認を続け、milestone終端に簡潔な最終deltaを1回だけ独立reviewへ渡す。
...
## 5. Implementation and operation boundary
...
## 9. Prompting pattern for later milestones
