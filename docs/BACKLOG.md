...
| # | 項目 | 効果 | 費用 | 着手条件 |
|---|---|---|---|---|
| A1 | `docs/NUMERIC_CONSISTENCY_AUDIT.md` の 14 件のうち、記録が正典である 12 件を修正する（残り 2 件は検証済みで対処済み） | 意思決定を駆動する文書の数値が記録と一致する | 机上 半日 | なし |
| A2 | `ERT_CLEAN_WRONG_RESCUE_SUBTYPES.md` と `ERT_CLEAN_WRONG_RELIABILITY_STRATIFIED.md` の C0/C10/C12/C13 表の食い違い（C10 が 0.2043 対 0.2298）を、どちらの replay が正典かまで遡って解消する | 監査の 15 件目。どちらかの文書が間違っている | 机上 数時間 | なし |
