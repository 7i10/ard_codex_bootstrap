...
| `PM /online`（旧 OS-PMP） | plan 0087 experiment B | `+0.41 / +0.06 / +0.46 pp` |
...
`n = 7.85·σ_d²/δ²`。σ_d が暫定値 0.35 pp なら、親 6 本 × 複製 2 本の構成で
検出可能効果は約 0.28 pp。**0092 の確定値で再計算し、0.30 pp を検出できない場合は
親の本数を増やしてから着手する。**
