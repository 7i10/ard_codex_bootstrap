...
- Readiness audit: Ferret has CUDA/PyTorch, torchvision, W&B, RobustBench, AutoAttack, CIFAR-10, and both teacher checkpoints with matching hashes; train/evaluate CLI imports pass. PyArrow 25.0.0 now matches Hamster and `pip check` is clean. Ferret-specific verification passed: PGD GPU `1 passed`, single-/two-GPU synthetic training `2 passed`, networkless W&B unit `47 passed`, and offline bundle/artifact integration `1 passed`. Hamster-initiated rsync collection is verified, so no Ferret-to-Hamster SSH key is required.
