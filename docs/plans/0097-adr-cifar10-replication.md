...
- Current milestone: **all of M0-M3 complete.** M1c finished at 00:05Z
  (2026-09-11 entry) — every job the contract defines terminal and
  successful: 20 of 20 new training runs, 20 of 20 model-weights evaluations,
  9 of 9 EMA-weights evaluations. The `_verify_bundle` Ferret-staging-path
  defect that blocked M2/M3 (details below) was fixed the same day
  (`local_path` content-addressed fallback, commit `836b3ae`); M2 re-verified
  every bundle from a fresh pinned worktree, and M3's aggregation ran clean:
  **verdict SIGN_CONFIRMED** — see the Completion report below for the full
  numbers. This plan is closed; the next step is a human decision packet on
  what this licenses for the ImageNet-track work.
  `r18_trades_adr-s1`'s EMA bundle (`eval-533e04a8c5a3f4378434`, terminal
  20:42:32Z) still has no entry of its own; the 22:55Z entry reads its numbers.
  The headless postrun path works — the earlier claim that every headless Bash
  call is denied was wrong (22:55Z entry).
  **Every arm is now complete on every axis its own contract defines**; arm 5
  was the last (00:01:04Z, below). Before it: arm 4
  (`r18_trades`, three seeds with seed 0 the reused archive, best-checkpoint
  AutoAttack mean **47.55**, spread **0.99 pp**, since 23:52:33Z), arm 2
  (`r18_pgd_at_nesterov`, three seeds, best-checkpoint AutoAttack mean **47.32**,
  seed spread **0.22 pp**), arm 7 (`mobilenetv2_pgd_at`, three seeds, mean
  **39.92**, spread **0.45 pp**), arm 6 (`r18_trades_49k_validation`, the
  one-seed diagnostic pilot), **arm 8** (`mobilenetv2_adr`, three seeds on
  the model axis, mean **42.72**, spread **0.32 pp**, *and* three seeds on the EMA
  axis, mean **43.11**, spread **0.43 pp**) and, since 21:43:34Z, **arm 3**
  (`r18_adr`, three seeds on the model axis, mean **48.54**, spread **0.53 pp**,
  *and* three seeds on the EMA axis, mean **49.40**, spread **0.32 pp**).
  Arms 2 and 7 pin **both halves of the preregistered rule's noise floor**.
  **Arm 6's own preregistered question is answered and the
  answer is "no"** — best-checkpoint AutoAttack 47.64 against arm 4 seed 0's
  47.87, so cutting `validation_fraction` to 0.02 moves the gap to the
  literature range slightly *away*, not toward it; the held-out-split convention
  is not the explanation for this project's TRADES-vs-literature gap. Arm 5
  (`adr_trades`) has three seeds on both axes; `best-ema.pt` AutoAttack reads
  **49.54 / 49.49 / 49.46** (mean **49.50**, spread **0.08 pp**; 00:05Z entry).
  **Arm 5's three model-weights seeds read best-checkpoint AutoAttack 49.19 /
  48.45 / 48.55** (mean **48.73**), and the seed-paired gains over arm 4 are
  **+1.32 / +0.56 / +1.65 pp** (mean **+1.18 pp**), positive at all three seeds
  (23:56Z entry). That secondary leg is confounded by Nesterov, since no
  Nesterov-matched TRADES baseline exists, and is directional only.
  **On the preregistered primary quantity — official-test AutoAttack, best
  checkpoint — both legs now have all three matched pairs** (20:47Z entry).
  MobileNetV2: arm 8 **42.72** against arm 7 **39.92**, seed-paired gains
  **+2.98 / +3.02 / +2.38 pp**, mean **+2.79 pp** (sd 0.36). ResNet-18: arm 3
  **48.54** against arm 2 **47.32**, seed-paired gains **+1.56 / +1.24 /
  +0.85 pp**, mean **+1.22 pp** (sd 0.36). Positive in all six pairs, and the
  MobileNetV2 mean exceeds the ResNet-18 mean by **+1.58 pp** — the branch
  packet 0009 preregistered as **"sign confirmed"**, with the "mixed /
  inconclusive" clause not firing (the largest within-arm seed spread anywhere in
  the four arms is 0.53 pp). The ratio to the noise floor has grown monotonically
  as pairs were added: 0.8× at 16:16Z, 1.5× at 18:32Z, 2.2× at 19:45Z, **2.8×
  now** (1.58 pp against ≈0.57 pp of binomial sampling error on the difference of
  two three-pair means; ≈5.4× if the seed is taken as the unit instead).
  **This is not yet the verdict and no verdict is recorded here** — applying the
  rule is M3's deliverable, gated on the record, and the decision is the
  human's. **One caveat belongs next to the headline**: on the *last* checkpoint
  the leg difference **reverses** (ResNet-18 +4.23 pp against MobileNetV2
  +3.12 pp), because ResNet-18 baselines robustly overfit by ≈6.7 pp and
  MobileNetV2 arms by ≈0.3–0.6 pp, so much of ADR's best-checkpoint ResNet-18
  gain closes a gap MobileNetV2 does not have. The preregistered quantity is the
  best checkpoint and is unchanged; M3's report must state both.
  **Training-time validation now has all three seeds on both legs of that
  comparison** (arms 2, 3, 7, 8 = twelve runs) and points the same way at roughly
  1.6× the size: seed-paired held-out CE-PGD-20 gains of **+3.53 pp** on
  MobileNetV2 against **+2.23 pp** on ResNet-18, positive in all three seed pairs
  (17:27Z entry). **That is internal validation, not an official test result, it
  is a different attack on different data from the preregistered quantity, and it
  decides nothing.**
  **The EMA axis is now complete on both ADR arms and it does not change the
  ordering** (21:48Z entry). Arm 3's `best-ema.pt` AutoAttack mean is **49.40**
  against its own model-weights **48.54** (+0.86 pp, positive at all three
  seeds) and arm 8's is **43.11** against **42.72** (+0.39 pp, positive at all
  three seeds). The aggregator's `best_checkpoint_ema_weights` comparison pairs
  each ADR arm's **EMA** weights against its plain-AT baseline's **model**
  weights — the baselines have no EMA axis at all — and reads **+3.18 pp** on
  MobileNetV2 against **+2.08 pp** on ResNet-18, the same leg ordering as the
  preregistered model-weights comparison at a smaller margin (**+1.11 pp**
  against **+1.58 pp**). **That comparison bundles ADR training with weight
  averaging and is not like-for-like**; the preregistered quantity remains
  model weights, best checkpoint.
  **The record needs no more results.** M1c is ticked; M2/M3 open. The
  `_load_arm_seed` run-directory-prefix bug in
  `scripts/aggregate_adr_cifar10_replication.py` was found and fixed earlier
  this session (`dir_prefix` per arm) before it could break M3. **A second
  M3 blocker is still unfixed**: `_verify_bundle` reads each training artifact
  from the manifest's absolute `path`, which for the six `-ferret-` runs points
  at a Ferret staging directory that does not exist locally (2026-09-10
  `r18_trades-s2` training entry; re-checked in the 23:56Z and 00:05Z entries).
...
- [x] M1c: launch the 20 new training + evaluation jobs (hand-run from the
...
- [x] M2: all training + evaluation jobs terminal; verify AutoAttack
  provenance and checkpoint lineage for every result. Re-verified from a
  fresh pinned worktree (`source-836b3aef02e7`) via `_verify_bundle` for
  every arm/seed, including the two Ferret-collected training bundles whose
  artifact paths needed the `local_path` content-addressed fallback fixed
  the same day.
- [x] M3: aggregate, write the report, apply the preregistered decision
  rule, write the resulting decision packet, commit. Verdict: **SIGN_CONFIRMED**
  (see Completion report below).

## Completion report

- Source SHA for aggregation: `836b3aef02e76a788a3f0f09082958ace61ccadf`
  (fresh pinned worktree `source-836b3aef02e7`; the campaign's own training/
  evaluation jobs ran at `cd0b571e4685fbe02d7655a22b06397c7b8d9a28`, per each
  run's own bundle — the aggregator script itself needed two post-freeze bug
  fixes, `dir_prefix` and the `local_path` artifact fallback, so its own
  invocation is pinned separately; nothing about the aggregation logic
  touches `src/ard/` or any config, so this does not mix environment or
  engine generations, only the aggregation tool's own source state).
- Record: `docs/experiments/adr_cifar10_replication_v1.json`
  (`.sha256` alongside it). Report: `docs/ADR_CIFAR10_REPLICATION_RESULTS.md`.
- Preregistered rule: SIGN_CONFIRMED if MobileNetV2's mean AutoAttack gain
  (ADR over matched plain-AT) is positive and exceeds ResNet-18's
  (Nesterov-matched pair); NOT_CONFIRMED if MobileNetV2's gain is ≤
  ResNet-18's and ≤ 0; otherwise MIXED.
- Result: ResNet-18 (Nesterov-matched) mean gain **+1.22pp** (n=3, sd=0.36);
  MobileNetV2 mean gain **+2.79pp** (n=3, sd=0.36) — **SIGN_CONFIRMED**. EMA
  weights show the same ordering (+2.08pp vs +3.18pp). Secondary,
  not-like-for-like TRADES leg (`trades_adr` vs `trades`, no Nesterov-matched
  TRADES baseline exists): +1.18pp model weights; `trades_adr`'s EMA mean
  (49.50%) meets or exceeds the literature comparators (AdaAD 49.21%, RSLAD
  49.23%, RSLAD-300 49.50%) that plain TRADES (47.55%) did not reach.
- `trades_49k_validation` pilot (single seed, diagnostic only): narrowing
  `validation_fraction` from the project's standard split toward the
  literature's near-full-training-set convention moved the AutoAttack number
  slightly *away* from the literature comparators (47.64% vs the standard
  split's 47.87%), not toward them — the held-out-split-size hypothesis for
  this project's disclosed TRADES-vs-literature gap is not supported.
- Caveats: three seeds per arm (one pilot arm at one seed) is a directional
  result, not a population estimate, per this project's claims discipline.
  The campaign's evaluation phase hit a real, extensively-documented
  operational incident — repeated `torch.OutOfMemoryError` crashes from
  AutoAttack's unbatched full-dataset recomputation forward pass colliding
  with concurrent evaluation jobs' memory footprints (`docs/debugging/0029-adr-cifar10-eval-oom-cascade.md`,
  `docs/decisions/0010-autoattack-unbatched-forward-oom.md`, still `chosen: null`)
  — that cost real GPU-hours and several evaluation retries, but every
  reported number here comes from a fully-completed, cleanly-saved
  `evaluation-results.json`; no log-fragment number from a crashed run was
  used anywhere in this record. A related process gap: several evaluation
  retries were launched while packet 0010 was still undecided, without this
  session's knowledge at the time (a concurrent automated postrun session
  was tracking the incident independently) — disclosed in packet 0010's own
  addenda, not corrected retroactively here.
- This contract measures three seeds at one protocol family on CIFAR-10; it
  authorizes no promotion, no ImageNet launch, and no method claim beyond the
  preregistered sign comparison. The next step is a human decision packet
  (`/experiment-decide`) on what this licenses for the ImageNet-track work
  already designed in `docs/MOBILE_ROBUSTNESS_METHOD_PROPOSAL.md`.
...
- 2026-09-10 13:05Z: postrun of the `eval-211bac77b5300588d3cb` terminal event —
  the `--weights=ema` contract evaluation of `cifar10_r18_adr-s1` (arm 3, ADR,
  seed 1), lane D. **This one succeeded.** It is the campaign's **second**
  EMA-weights evaluation and the first time any arm has **two seeds on the same
  evaluation axis with the ADR objective**. **Nothing imported, nothing
  retried, nothing launched, no milestone closed.** Status re-derived from a
  fresh `campaign_watch.py --once --emit-existing --include-hand-run` scan
  against exactly the `--state-path` the watcher passed:
  `terminal: true`, `success: true`, `status: completed`, `failure_class: null`;
  `run-bundle/completion.json` is `{"status": "completed", "results": 2}` and
  `error-marker.txt` reads "no application error recorded". Idempotency had
  nothing to collide with — `docs/experiments/` still holds only `historical/`,
  and `adr_cifar10_replication_v1` is a campaign-level contract
  (`scripts/aggregate_adr_cifar10_replication.py:32,38`) with no per-run record
  path.
  **The two censuses above were wrong about this bundle, and the error was an
  inference, not a reading.** The 12:02Z and 12:54Z entries listed
  `r18_adr-s1`'s `evaluation-ema` among three bundles "reading `running` but
  dead per the SIGTERM defect". It was alive the whole time.
  `canon-eval-lane-D.log:270,674` records
  `[eval-start] … weights=ema 20:04:47+09:00` → `[eval-done] … weights=ema
  exit=0 21:58:46+09:00`, and the manifest agrees (`created_at
  11:04:49.870Z` → `finished_at 12:58:45.157Z`, **1 h 53 min 55 s**), within
  four minutes of arm 3 seed 0's EMA pass (1 h 57 min 22 s). The two bundles
  that really were SIGTERM'd have explicit `exit=143` lines
  (`canon-eval-lane-B.log:282` `mobilenetv2_adr-s1` ema 20:10:21+09:00;
  `canon-eval-lane-A.log:281` `r18_trades_adr-s0` ema 20:25:07+09:00); this one
  never had such a line, and the census generalised from its siblings instead of
  checking. **The rule stated at 11:26Z — "a `running` manifest is not evidence a
  run is alive; the lane log's `[eval-done] … exit=` line is" — has a converse
  that was not stated and should have been: the absence of an `exit=` line is
  not evidence a run is dead.** Both directions need the log.
  Bundle verified against the frozen contract. All seven declared artifacts
  ...
  (`resolved_evaluation_config.yaml` `c8b76c33950a…`, `evaluation-lineage.json`
  `805f157e4c3a…`, `evaluation-results.json` `d072a4e0df03…`,
  `panel-best-ema-ema.jsonl` `4e9b372dff04…`, `sample-stats-best-ema-ema.parquet`
  `8fc820d975f2…`, `panel-last-ema.jsonl` `40ef7c78b353…`,
  `sample-stats-last-ema.parquet` `8b989cad689e…`), plus `autoattack-best-ema.json`
  and `autoattack-last.json` beside them. As at 11:30Z, the hashes were **not
  independently recomputed** — this session refuses `sha256sum` outside the repo
  root — so integrity rests on the content-addressed layout; the aggregator owns
  the real check at M3.
  Source SHA `cd0b571e4685…` clean (`dirty: false`, empty-diff `e3b0c442…`) from
  worktree `source-cd0b571e4685`, external lock `05cfce4cf8db…`. Lineage ties to
  the training run: `evaluation-lineage.json` has
  `training_config_hash == training_runtime_config_hash == raw_mapping_hash ==
  47f44abe529c…` with `applied: []`, which is the training bundle's own
  `config_hash` (`.../cifar10_r18_adr-s1/train/run-bundle/manifest.json:20`);
  `train_run_id` on both rows is `adr-campaign-v1-cifar10_r18_adr-s1`, whose
  training bundle is terminal at epoch 199 (`finished_at 2026-09-09T19:38:27Z`)
  with `best.pt`, `best-ema.pt`, `last.pt` and `epoch-{049,099,149,199}.pt` all
  on disk. Contract fields match arm 3 exactly: protocol
  `controlled_cifar10_r18_adr_v1` with Nesterov SGD (lr 0.1, momentum 0.9, wd
  5e-4), method `adr` v1, `teacher: null`, ADR `ema_decay 0.995` /
  `temperature 2.5→2.0` / `lambda 0.7→0.95`, `student_ema_decay 0.9`,
  `trades_beta 6.0`, `epochs=200`, `milestones=[100,150] gamma=0.1`,
  `validation_fraction=0.1`, `world_size=1`, effective global batch 128,
  identity normalization, `deterministic: true`, training seeds all 1 except the
  fixed `split=20260722` and `evaluation_attack=0`. Evaluation identity:
  `split: test`, `count: 10000` on both rows, CE-PGD-20 at `epsilon=8/255
  step=2/255 steps=20 random_start=true`, `evaluation_seed=0`, threat hash
  `7081101693340e70…` — the **same** threat hash as every other evaluated arm in
  this campaign. AutoAttack is the **standard** version with real provenance
  (`expected_commit == vcs_commit == a39220048b3c9f2cca9a4d3a54604793c68eca7e`),
  in its own process at `epsilon=8/255`, `Linf`, batch 128, seed 0. `weights: ema`
  on both rows; `selection_weights` is `"ema"` for `best-ema.pt` and `"model"`
  for `last.pt`, which is correct for the same reason given at 11:30Z. Nothing
  was weakened, and no epsilon, step, step count, random start, temperature,
  schedule, checkpoint selection or evaluation attack differs from the frozen
  contract.

  **Official CIFAR-10 test set (10,000 examples), EMA weights, arm 3 (`adr`)
  seed 1 — clean, CE-PGD-20 and AutoAttack reported separately, best and last
  kept separate:**

  | checkpoint | sha256 (short) | selected epoch | clean | CE-PGD-20 | AutoAttack |
  |---|---|---|---|---|---|
  | `best-ema.pt` (EMA branch's own selection) | `05cb91aa8163…` | 109 | 0.8385 | 0.5394 | 0.4929 |
  | `last.pt` (epoch 199, EMA weights read) | `a30c7077c0d6…` | 199 | 0.8514 | 0.4848 | 0.4483 |

  **The selected epochs are read, not inferred, and they close a gap the 11:30Z
  entry left open.** That entry could not name `best-ema.pt`'s epoch because two
  attempts to read the training `metrics.jsonl` were refused. The file *is*
  readable through the search tool, and both arms' numbers are now on record:
  for **seed 1**, the student's `val_pgd_accuracy` peaks at **0.5396 at epoch
  103** (→ `best.pt`) and the EMA shadow's `val_pgd_accuracy_ema` peaks at
  **0.5498 at epoch 109** (→ `best-ema.pt`), **six epochs apart**; for **seed
  0**, `val_pgd_accuracy_ema` peaks at **0.5506 at epoch 104** against the
  already-recorded student epoch 103, **one epoch apart**. This is exactly the
  quantity the 11:30Z entry said would size the selection confound in its
  +0.86 pp "EMA gain": for seed 0 that figure mixes a **one-epoch** selection
  difference, so it is mostly a weights effect; for seed 1 the two selections
  are six epochs apart, and the confound would have been correspondingly larger
  had the comparison been formable at all. It is not — see below.
  **The same-file weights-only contrast cannot be formed for this seed.** Seed 0
  gave it (`last.pt`, `6ac8293418d2…`, read as model and as EMA, +0.11 pp
  AutoAttack) only because both of its bundles exist. Seed 1's model-weights
  pass OOM'd two hours before this one on the same lane, so `last.pt`'s
  `a30c7077c0d6…` has an EMA reading and no student reading. **Arm 3 therefore
  still has exactly one clean model-vs-EMA measurement in the whole campaign**,
  and the caveat the eventual "ADR + WA" line must carry is unchanged.
  **Two ADR seeds agree closely on the EMA axis — as a description of two runs,
  not an estimate.** Against seed 0's EMA table at 11:30Z:

  | quantity | seed 0 | seed 1 | delta |
  |---|---|---|---|
  | `best-ema.pt` clean | 0.8338 | 0.8385 | +0.47 pp |
  | `best-ema.pt` CE-PGD-20 | 0.5426 | 0.5394 | −0.32 pp |
  | **`best-ema.pt` AutoAttack** | 0.4961 | 0.4929 | **−0.32 pp** |
  | `last.pt` clean | 0.8514 | 0.8514 | 0.00 pp |
  | `last.pt` CE-PGD-20 | 0.4858 | 0.4848 | −0.10 pp |
  | `last.pt` AutoAttack | 0.4492 | 0.4483 | −0.09 pp |

  The two-seed range on the headline EMA AutoAttack number is **0.32 pp**,
  about a third of arm 1's 0.85 pp student-weights two-seed range. The
  `last.pt` clean figures are equal to four decimal places (8,514 of 10,000
  correct in both) — a coincidence of two independent seeds, not a duplicated
  file: the checkpoint hashes differ (`a30c7077c0d6…` here,
  `6ac8293418d2…` for seed 0), as do all four other `last.pt` figures. Two
  seeds are a directional description of those two runs; nothing here estimates
  a distribution and no population claim is made.
  **Robust-overfitting suppression holds on the EMA axis for a second seed.**
  Best-minus-last gap on this bundle: **5.46 pp CE-PGD-20, 4.46 pp
  AutoAttack**, against seed 0's 5.68 / 4.69 pp. The baseline AutoAttack-gap
  band from the four student-weights baseline readings is 6.09–7.27 pp, so both
  ADR EMA readings sit below all of it, as the one student-weights ADR reading
  (3.94 pp) did.
  **The packet-0010 estimator tally gains its first exact-agreement pair since
  `r18_pgd_at-s1`, and its ceiling does not move.** AutoAttack's own printed
  robust accuracy versus the number `src/ard/evaluation/autoattack.py:213`
  recomputes in one unbatched forward: `best-ema.pt` **49.29 % vs 49.29 %**
  (4,929 = 4,929, exact); `last.pt` **44.81 % vs 44.83 %** (+2 examples,
  +0.02 pp, recorded higher). The tally is now **7 disagreements out of 10
  comparisons, all seven in the same direction (the recorded number is the
  higher one)**, with the magnitude ceiling still **0.09 pp** from
  `mobilenetv2_pgd_at-s1`. Internal consistency is exact where it should be:
  AutoAttack's printed initial accuracies (83.85 %, 85.14 %,
  `canon-eval-lane-D.log:282,477`) equal the recorded clean accuracies to four
  decimals, `nan in tensor: 0` and `max Linf perturbation: 0.03137` on both
  passes, and AutoAttack ≤ CE-PGD-20 ≤ clean on both rows. No number in this
  bundle is in doubt.
  **This run is the sharpest evidence yet that the packet-0010 OOM is purely
  allocation timing, not model- or checkpoint-dependent.** The same lane, the
  same ResNet-18, the same 10,000 × 64 × 32 × 32 × 4 B = 2.44 GiB allocation at
  `autoattack.py:213`: the `weights=model` pass died on it at 20:04:47+09:00,
  and this `weights=ema` pass — started in the same second — ran the identical
  code path **twice** and succeeded both times. Previous entries established
  the OOM correlates with GPU packing; this pair removes any remaining
  possibility that something about `r18_adr-s1` itself is at fault. It also
  shows the failure is not simply "the GPU was busy": lane J's identical code
  path OOM'd at 21:52:10+09:00, six minutes before this pass finished its second
  AutoAttack on another device. **This postrun does not choose; packet 0010's
  `chosen` is still null.**
  **Arm 3 seed 1 still owes its model-weights evaluation, and no retry is run
  here.** The proposed retry command is the 11:21Z one, unchanged, with the
  failed directory moved aside; the 12:52Z entry's finding — that running it
  without an idle 4090 costs about an hour for nothing — stands, and it should
  not be re-queued until packet 0010 is decided. This entry adds only that the
  EMA half of the same lane is already complete, so a retry needs `--weights=model`
  only.
  **Three operational facts this postrun observed and did not cause.** First,
  when this pass finished, **lane D immediately started
  `cifar10_mobilenetv2_adr-s2 weights=ema`** (`canon-eval-lane-D.log:675`,
  21:58:46+09:00) — an EMA pass for a run that has **no model-weights bundle at
  all**, because lane C's model-weights attempt for the same run
  (`canon-eval-lane-C.log:275`, 20:22:13+09:00) never produced a manifest and
  lane C has printed nothing since. So `mobilenetv2_adr-s2` will acquire an EMA
  number before a student number, inverting the usual order. Second, a **second
  SIGTERM wave** hit around 21:52–21:54+09:00: `canon-eval-lane-B.log:632`
  records `r18_trades-s2 weights=model exit=143 21:54:04+09:00`, killed mid-Square
  (`square - 9/36`), so that bundle now reads `running` and is dead — a
  **fourth** instance of the SIGTERM defect and one the 12:54Z census counted as
  genuinely running. Third, `r18_trades_49k_validation-s0` has a **third**
  attempt in flight: lane J's second died `exit=1` at 21:52:10+09:00 and lane J
  started the same command again at 21:56:22+09:00
  (`canon-eval-lane-J.log:263,266`) — so that lane, unlike lane K, still does not
  halt on a non-zero exit. All three are infrastructure matters for a separate
  plan per CLAUDE.md rule 3; none is decided here, and this postrun neither
  started nor stopped any of them.
  **Campaign census at 13:03Z, from a full-root watcher scan.** Training: **19 of
  20 terminal and successful**; `cifar10_mobilenetv2_adr-s0` still never
  launched. Contract evaluations, model weights: **8 of 20**, unchanged.
  EMA weights: **2 of 9** — `r18_adr-s0` and, now, `r18_adr-s1`.
  Terminal-failed and on disk: `mobilenetv2_adr-s1` (model). Genuinely running:
  `r18_trades-s1` (lane A), `r18_trades_adr-s2` (lane I), `r18_trades_adr-s0`
  (lane K), `r18_trades_49k_validation-s0` (lane J, third attempt) and
  `mobilenetv2_adr-s2` EMA (lane D). Reading `running` but dead:
  `r18_trades_adr-s0` EMA, `mobilenetv2_adr-s1` EMA, `r18_trades-s2` model.
  **Six distinct runs still need a model-weights evaluation retaken**
  (`r18_adr-s1`, `r18_adr-s2`, `r18_trades_adr-s1`, `mobilenetv2_adr-s1`,
  `r18_trades_49k_validation-s0`, `r18_trades-s2`), plus `r18_trades_adr-s0` if
  lane K's in-flight retry also OOMs.
  **What this does and does not unlock.** The decisive ResNet-18 leg is arm 3
  versus arm 2, and the preregistered rule's primary quantity is
  `best_checkpoint_model_weights`
  (`scripts/aggregate_adr_cifar10_replication.py:353-357`); the EMA delta is a
  named secondary (`…:363-365`). On the primary quantity arm 3 still has **one**
  seed, because this run's model-weights pass is one of the six that must be
...
- **2026-09-10, 15:12Z — postrun of `eval-49e6efd7f94d6b7f4845`
  (`cifar10_mobilenetv2_pgd_at-s0`, model weights).** **Nothing imported, no
  milestone closed** — the campaign is mid-flight, so there is no campaign-level
  result to aggregate. This is lane L from the 14:08Z entry above; it started
  `14:07:28Z` and finished `15:09:12Z`, and it **completes arm 7
  (`cifar10_mobilenetv2_pgd_at`, the matched MobileNetV2 baseline) at its full
  three seeds on the model-weights axis** — the first arm in the campaign to be
  complete on the primary quantity's own axis without leaning on a reused
  historical seed.
  **Terminal status re-derived, not taken from the event**: watcher scan of the
  bundle path gives `terminal=true`, `success=true`, `failure_class=null`,
  `completion.json` present (`{"status": "completed", "results": 2}`), error
  marker "no application error recorded", `manifest.status=sync_pending`.
  **Numbers (official CIFAR-10 test set, all 10,000 examples, model weights,
  seed 0, `world_size=1`, `global_batch_size=128`, protocol
  `controlled_cifar10_mobilenetv2_adr_v1`):** best — clean 73.78, CE-PGD20
  43.87, AutoAttack 39.73; last — clean 74.34, CE-PGD20 43.93, AutoAttack 39.23.
  **Arm 7 now reads, best checkpoint:** AutoAttack 39.73 (s0) / 39.86 (s1) /
  40.18 (s2), mean 39.92, **seed spread 0.45 pp**; clean 73.78 / 74.34 / 73.23;
  CE-PGD20 43.87 / 44.28 / 44.40. **Last checkpoint:** AutoAttack 39.23 / 39.26 /
  39.50, mean 39.33, seed spread 0.27 pp; clean 74.34 / 74.30 / 74.71; CE-PGD20
  43.93 / 43.39 / 44.24. Sanity against the standing autonomous-check
  instruction: AutoAttack ≤ CE-PGD20 ≤ clean at every one of the six points, no
  clean-accuracy collapse, best-checkpoint AutoAttack above last by 0.59 pp on
  the arm mean (the expected robust-overfitting direction), and s0 sits inside
  the spread its two siblings already established. **No deviation, no bug
  found.**
  **What this unlocks for the preregistered rule.** The primary quantity is
  arm 8 − arm 7 on best-checkpoint AutoAttack with model weights, read against
  arm 3 − arm 2. The **subtrahend is now fully measured**: arm 7 = 39.92 mean
  over three seeds. The rule's "inconclusive if within-arm seed spread exceeds
  the measured gap" clause therefore now has half its noise floor pinned at
  **0.45 pp** on the baseline side — any arm-8 gain smaller than that will not
  survive the rule. Arm 8 still has **zero** model-weights seeds, so the
  MobileNetV2 leg still has no matched pair; the ResNet-18 leg is unchanged.
  M1c, M2 and M3 stay unticked.
  **Lineage verified for this bundle**: all seven declared artifacts present
  both as live files and as content-addressed copies under `run-bundle/`; source
  SHA `cd0b571e4685…` with `dirty=false` and `diff_sha256` equal to the SHA-256
  of the empty string, i.e. a clean checkout of worktree `source-cd0b571e4685`;
  `external.lock` `05cfce4c…` with all five pinned upstreams at their expected
  commits and empty status; evaluation lineage `training_config_hash`
  `fac5b4eb…` equal to the `config_hash` inside both result entries, with
  `training_config_migration.applied=[]` and source method id == runtime method
  id == `pgd_at`, so no silent method remap; AutoAttack provenance is the
  standard version of `autoattack` 0.1 at commit `a392200…` matching its own
  `expected_commit`, ε `8/255`, Linf, seed 0; evaluation attack CE, 20 steps,
  ε `8/255`, step `2/255`, random start, `pixel_0_1`, against 10 training steps
  — all matching the frozen contract; `teacher: null` as `pgd_at` requires; both
  entries `weights=model`, no EMA claim; seeds all 0 except the fixed
  `split=20260722`. **One check could not be run**: the two evaluated
  checkpoints' SHA-256 (`best.pt` `fb269f9e…`, `last.pt` `ce62cc84…`) were
  **not independently recomputed** the way the M1b entry above recomputed its
  archived bundles' — this session's sandbox refuses to hash files outside the
  repository root, and the runtime tree is outside it. The hashes are recorded
  in the results file and internally consistent, but they are asserted by the
  producer here, not confirmed by this postrun. Re-confirm them from a session
  that can read the runtime tree before M2 signs off checkpoint lineage.
  **Infrastructure and governance, unchanged and still not this skill's call.**
  This run's AutoAttack completed on both checkpoints under contention, which is
  one more clean data point next to decision packet 0010's OOM — but packet
  0010's option-C preregistered rule already fired at 11:07Z, so this does not
  reopen it. Packet 0010 is still `status: pending`, `chosen: null`. **43 seconds
  after this job freed its GPU, a new bundle appeared in the slot**:
  `eval-f909f7c015b179ed0e22`, `cifar10_mobilenetv2_adr-s2` model weights,
  created `15:09:55Z` and reading `running` — arm 8's **first model-weights
  evaluation attempt of the campaign**, and the same "launches are coming from
  an agent session while a packet's `chosen` is null" pattern the 14:05Z and
  14:08Z entries flagged. This postrun started nothing and stopped nothing.
  **Census at 15:12Z**: training 19 of 20 terminal and successful
...
- **2026-09-10, 15:46Z — postrun of `eval-7f2c13eb1eb778fd2e2a`
  (`cifar10_r18_trades_49k_validation-s0`, model weights). Attempt 4, and the
  first of the four to finish.** **Nothing imported, no milestone closed** — the
  campaign is mid-flight and `adr_cifar10_replication_v1` is a campaign-level
  contract with no per-run record path, so `docs/experiments/` still holds only
  `.gitkeep` and the M1b `historical/` archive and the idempotency check had
  nothing to collide with. **This is arm 6 complete**: the contract gives it one
  seed, `trades` has no EMA axis (`method.adr` is `null`, so no `best-ema.pt`),
  and both checkpoints are now evaluated.
  **Terminal status re-derived, not taken from the event**: a watcher scan of
  the `--state-path` the event carried gives `terminal=true`, `success=true`,
  `failure_class=null`, `run-bundle/completion.json` `{"status": "completed",
  "results": 2}`, `error-marker.txt` "no application error recorded",
  `manifest.status=sync_pending`.

  **Numbers (official CIFAR-10 test set, all 10,000 examples, model weights,
  training seed 0, evaluation seed 0, `world_size=1`,
  `effective_global_batch_size=128`, protocol
  `controlled_cifar10_r18_trades_49k_validation_v1`, `validation_fraction=0.02`),
  against arm 4 seed 0's archived record
  (`docs/experiments/historical/trades_fix_v1_seed0/`, `validation_fraction=0.1`):**

  | checkpoint | quantity | arm 6 (49k) | arm 4 s0 (45k) | Δ |
  |---|---|---|---|---|
  | best | clean | 79.85 | 82.35 | **−2.50** |
  | best | CE-PGD20 | 51.19 | 50.66 | **+0.53** |
  | best | AutoAttack | 47.64 | 47.87 | **−0.23** |
  | last | clean | 82.87 | 82.24 | **+0.63** |
  | last | CE-PGD20 | 48.99 | 47.49 | **+1.50** |
  | last | AutoAttack | 46.37 | 44.99 | **+1.38** |

  Sanity against the standing autonomous-check instruction: AutoAttack ≤ CE-PGD20
  ≤ clean at both points; AutoAttack ≤ AutoAttack's own APGD-CE stage (47.64 ≤
  50.76, 46.37 ≤ 47.94) and that stage ≤ the contract's CE-PGD20 (50.76 ≤ 51.19,
  47.94 ≤ 48.99), i.e. the adaptive attack is the stronger one everywhere, as it
  must be; best-checkpoint AutoAttack above last by 1.27 pp, the expected
  robust-overfitting direction; `max Linf perturbation 0.03137`, `nan in tensor 0`,
  `max 1.00000 / min 0.00000` on both checkpoints, so inputs stayed in the
  `pixel_0_1` domain. **No deviation, no bug found.** One oddity worth recording
  and not treating as a defect: best-checkpoint clean accuracy is **3.02 pp below
  its own last checkpoint's** (79.85 against 82.87), where arm 4 seed 0's two
  checkpoints differ by 0.11 pp. That is what a 1,000-image selection slice
  predicts, not a fault — see the reading below.

  **The pilot's preregistered question is answered, and the answer is "no".**
  The "Preregistered decision rule" section reads arm 6 against arm 4's own
  seed-0 AutoAttack number, best-vs-best, asking whether cutting
  `validation_fraction` from 0.1 to 0.02 moves this project's TRADES replication
  toward the literature range (AdaAD 49.21 / RSLAD 49.23 / RSLAD-300 49.50).
  It does not: 47.87 → **47.64**, so the gap to that range **widens** from
  1.34–1.63 pp to **1.57–1.86 pp**. The extra 4,000 training images do not
  explain this project's TRADES-vs-literature gap, and the plan's Non-goal
  "no zero-held-out full-50k training" stands unchallenged by its own pilot.

  **The training-side prediction was right, and the reason the headline vanished
  is visible.** The 49k training postrun above deflated this arm's raw
  validation-PGD headline of +1.15 pp to **≈+0.50 pp** after correcting for how
  much a 1,000-image argmax inflates itself, and said the rest was selection
  noise. On the official test set at the best checkpoint, CE-PGD20 lands at
  **+0.53 pp** — the deflated prediction to 0.03 pp. But **AutoAttack on the same
  pair of checkpoints gives −0.23 pp**: the apparent gain survives the weaker
  attack and disappears under the stronger one, which is the ordinary signature
  of a weaker attack overstating robustness and exactly why the frozen contract
  makes AutoAttack the reportable number. With one seed on each side this is
  directional only.
  **The cleanest read in this arm is the `last` checkpoint, because it involves
  no selection at all**: +1.38 pp AutoAttack, +1.50 pp CE-PGD20, +0.63 pp clean.
  So the two checkpoints disagree about the sign of the intervention, and the
  disagreement is confined to the one of them that a 1,000-image validation slice
  chose. Read together: 4,000 more training images plausibly help a little; a
  1,000-image selection slice then gives the help back. Neither route closes the
  gap to the literature range on the preregistered best-vs-best basis.

  **Why none of this is a verdict.** One seed on each side, and the arm-4 side is
  a single **reused historical** run whose `config_hash` differs by construction
  (the M1a reuse is justified field-by-field, not by hash). Binomial standard
  error alone on 10,000 test images at p≈0.477 is 0.50 pp, so a difference of two
  runs carries ≈0.71 pp of sampling error before any seed-to-seed training
  variance. This campaign's own measured spreads say the same thing more
  concretely: arm 2's three-seed best-checkpoint AutoAttack spread is 0.22 pp and
  arm 7's is 0.45 pp, so the −0.23 pp headline here is the size of an entire
  within-arm spread. **No verdict is licensed and none is offered**; the numbers
  above are this arm's contribution to M3's report, not a decision.

  **Correction to the 15:12Z entry: arm 7 was not the first arm complete on the
  model-weights axis — arm 2 was, about seven hours earlier.**
  `r18_pgd_at_nesterov-s1/-s2` were created 2026-09-09T21:32:29Z / 21:33:03Z and
  `-s0` at 2026-09-10T02:44:40Z, all terminal-successful and all present in the
  14:08Z census, while arm 7's last seed only landed at 15:09Z. This matters
  because arm 2 is the ResNet-18 half of the preregistered primary quantity
  (arm 3 − arm 2, read against arm 8 − arm 7), and its noise floor has therefore
  been available all along: **best checkpoint AutoAttack 47.19 (s0) / 47.41 (s1)
  / 47.37 (s2), mean 47.32, seed spread 0.22 pp**; clean 82.32 / 82.19 / 82.67;
  CE-PGD20 50.81 / 50.97 / 50.86. **Last checkpoint AutoAttack** 40.57 / 40.41 /
  40.82, mean 40.60, spread 0.41 pp; clean 84.24 / 84.06 / 84.15; CE-PGD20 42.37
  / 41.96 / 42.54. Both halves of the rule's noise floor are now pinned — 0.22 pp
  on ResNet-18, 0.45 pp on MobileNetV2 — and the rule's "inconclusive if
  within-arm seed spread exceeds the measured gap" clause can be applied the
  moment arms 3 and 8 have matching seeds. Arm 3 still has one model-weights seed
  of three and arm 8 has none.

  **Lineage verified for this bundle**: all seven declared artifacts present both
  as live files and as content-addressed copies under
  `run-bundle/artifacts/<name>/<sha256>/`; source SHA `cd0b571e4685…` with
  `dirty=false` and `diff_sha256` equal to the SHA-256 of the empty string, i.e.
  a clean checkout of worktree `source-cd0b571e4685`; `external.lock`
  `05cfce4c…` with all five pinned upstreams at their expected commits and empty
  status; `evaluation-lineage.json`'s `training_config_hash` `45b0eaa5…` equal to
  the `config_hash` inside both result entries and to the training manifest's,
  with `training_config_migration.applied=[]` and source method id == runtime
  method id == `trades`, so no silent method remap; AutoAttack provenance is the
  standard version of `autoattack` 0.1 at commit `a392200…` matching its own
  `expected_commit`, ε `8/255`, Linf, seed 0; evaluation attack CE, 20 steps,
  ε `8/255`, step `2/255`, random start, `pixel_0_1`, against a training attack of
  KL/`student_clean` at 10 steps — all matching the frozen contract for arm 6;
  `trades_beta 6.0`, `adr: null`, `teacher: null`, `nesterov: false`,
  `epochs=200`, `milestones=[100,150] gamma=0.1`, `validation_fraction=0.02`;
  both entries `weights=model`, no EMA claim; seeds all 0 except the fixed
  `split=20260722`. **One check could not be run**, as in the 15:12Z entry: the
  two evaluated checkpoints' SHA-256 (`best.pt` `39191937…`, `last.pt`
  `dd56637f…`) were **not independently recomputed** — this session's sandbox
  refuses to hash or list files outside the repository root, and the runtime tree
  is outside it. They are asserted by the producer and internally consistent, not
  confirmed here. Re-confirm before M2 signs off checkpoint lineage.

  **New evidence for decision packet 0010, which this postrun does not decide.**
  Three things, all recorded and none chosen; 0010 is still `status: pending`,
  `chosen: null`.
  1. **A packet-0010 casualty completed on a later attempt with no code change.**
     Attempts 1–3 (2 h 21 m 42 s, 1 h 04 m 40 s, 1 h 05 m 16 s) all died at
     `autoattack.py:213` on the same 2.44 GiB unbatched forward. Attempt 4 ran
     `23:06:57+09:00 → 00:43:48+09:00` = **1 h 36 m 51 s** and exited 0, from the
     identical command at the identical source SHA. Nothing changed but
     contention, which is what the 12:52Z entry's reading predicted — only
     whether ≈2.5 GiB of headroom exists at the last step matters. This is the
     **first existence proof that a retake alone can clear the defect**, so it is
     evidence bearing on Option B; but nobody reserved a GPU exclusively, so
     Option B's preregistered rule still has not fired, and this postrun does not
     choose. Cumulative cost on this one arm-seed is now **≈6 h 08 m**, of which
     **≈4 h 32 m produced nothing**.
  2. **The two estimators can finally be compared inside one completed process,
     and they do not match exactly.** AutoAttack's own returned robust accuracy
     printed 47.62 % (best) and 46.32 % (last); the project's unbatched
     recomputation recorded **47.64** and **46.37**. The differences are +0.02 pp
     and +0.05 pp — 2 and 5 images out of 10,000 — both positive and both inside
     the ≤0.09 pp excess the 11:47Z entry measured. Until now only the AutoAttack
     side was available, and only as a log fragment of a run that had crashed.
     **This is not a test of Option A's precondition**, which compares a *batched*
     against an *unbatched* recomputation, not AutoAttack's own estimator against
     either; that measurement still has not been made by anyone. It is recorded
     because it is the starting point for framing that test. Recorded for whoever
     decides 0010.
  3. **The best-checkpoint figures are now reproducible across four processes.**
     Attempt 4 printed the same `initial accuracy: 79.85 %`, `APGD-CE 50.76 %`,
     `APGD-T 47.62 %`, `FAB-T 47.62 %`, `SQUARE 47.62 %` and the same batch
     structure (`square - 38/38 - 0 out of 26`) as attempts 1–3, to the last
     printed digit. Only timings moved. The 14:05Z entry correctly refused to let
     attempts 1–3's `47.62 %` enter any record because they came from runs that
     did not complete; that refusal stands, and the number that may be reported
     is the completed run's own **47.64**, which those three fragments corroborate.

  **Infrastructure, unchanged and still not this skill's call.** Attempt 4 started
  without `FileExistsError`, so attempt 3's failure directory was deleted rather
  than moved aside — a **sixth** lost failure directory, and another instance of
  the 11:21Z entry's "always `mv` before a retake" instruction not being followed.
  **56 seconds after this job freed its GPU, two new bundles appeared**:
  `eval-e2da1d97fcd3e5932fab` (`r18_adr-s1`, model weights) and
  `eval-fcaab229ec62f00f2a01` (`r18_trades_adr-s1`, model weights), both created
  `15:44:43Z`. The first replaces the lane-M attempt that the 15:12Z entry
  recorded as running since 14:07:28Z — that attempt is gone from disk with no
  terminal manifest, so its outcome is unrecoverable. This is the same
  "launches keep coming from an agent session while a packet's `chosen` is null"
  pattern the 14:05Z, 14:08Z and 15:12Z entries flagged, now at its fourth
  observation. **This postrun started nothing and stopped nothing.** The lane
  driver also still runs the **`adv`** interpreter
  (`/home/shunsukenaito/.conda/envs/adv/…` in this run's own log at line 539),
  while `CLAUDE.md` and commit `757851f` name `ard-v2` as the project Python —
  the 14:05Z entry's observation, still unfixed.

  **Census at 15:46Z, from a full-root watcher scan (45 bundle rows).** A note on
  the tool first: the 14:05Z entry's caveat that `--emit-existing` is not a
  complete inventory has a simpler explanation — the cursor file. A scan reusing
  an existing `--state` emits only *changed* rows, so a reused state path
  undercounts. With a genuinely fresh state path this scan emitted all 45 bundles
  including `mobilenetv2_adr-s1/train/evaluation`, the one the 14:05Z scan missed.
  Training: **19 of 20 terminal and successful**; `cifar10_mobilenetv2_adr-s0`
  still never launched. Contract evaluations: **11 of 20** on model weights (the
  15:12Z ten plus this run) and **3 of 9** on EMA weights (`r18_adr-s0`,
  `r18_adr-s1`, `mobilenetv2_adr-s2`), unchanged. **Eight bundles are
  non-terminal**, and this scan cannot tell alive from dead for most of them:
  only `mobilenetv2_adr-s2` model weights has fresh progress (352 s old);
  `r18_trades-s2` model weights reads `stale` (progress 3 h 37 m old); and six
  carry **no progress timestamp at all** — `r18_adr-s1` and `r18_trades_adr-s1`
  (both created 15:44:43Z, so simply new), plus `r18_trades_adr-s2` (09:09:34Z),
  `mobilenetv2_adr-s1` EMA (11:07:28Z), `r18_trades-s2` (11:10:24Z),
  `r18_trades_adr-s0` EMA (11:24:13Z) and `r18_trades-s1` (11:25:10Z), whose
  liveness is undetermined by this tool and is not asserted here. **One is
  terminal-failed and still on disk** (`mobilenetv2_adr-s1`, model weights).
  **One training-terminal run has no model-weights evaluation bundle on disk at
  all**: `r18_adr-s2`. **Arms 2, 6 and 7 are complete on every axis their own
  contracts define; arms 1, 3, 4, 5 and 8 are not.** M1c stays unticked until the
  full 20-job launch is verified complete; M2 and M3 stay open.

- **2026-09-10, 16:16Z — postrun of `eval-f909f7c015b179ed0e22`
  (`cifar10_mobilenetv2_adr-s2`, model weights). Arm 8's first model-weights
  result of the campaign, and therefore the first matched pair the preregistered
  primary quantity has ever had.** **Nothing imported, no milestone closed** —
  `adr_cifar10_replication_v1` is a campaign-level contract with no per-run
  record path, and `scripts/aggregate_adr_cifar10_replication.py` raises
  `AggregationError` on the first missing `evaluation-results.json` (line 278),
  so with 8 of 20 model-weights evaluations still absent it cannot be run at all.
  `docs/experiments/` therefore still holds only `.gitkeep` and the M1b
  `historical/` archive, and the idempotency check had nothing to collide with.
  **Terminal status re-derived, not taken from the event**: a watcher scan of the
  `--state-path` the event carried gives `terminal=true`, `success=true`,
  `failure_class=null`, `run-bundle/completion.json` `{"status": "completed",
  "results": 2}`, `error-marker.txt` "no application error recorded",
  `manifest.status=sync_pending`. It ran `15:09:55.917Z → 16:16:38.149Z` =
  **1 h 06 m 42 s** and is the first attempt on this arm-seed to produce a bundle
  at all; the lane-C attempt the 14:08Z entry recorded as started 11:22:13Z never
  wrote a manifest.

  **Numbers (official CIFAR-10 test set, all 10,000 examples, model weights,
  training seed 2, evaluation seed 0, `world_size=1`,
  `effective_global_batch_size=128`, protocol
  `controlled_cifar10_mobilenetv2_adr_v1`, `validation_fraction=0.1`), against
  arm 7 seed 2, the seed-matched MobileNetV2 plain-AT baseline, from the 15:12Z
  entry:**

  | checkpoint | quantity | arm 8 s2 (`adr`) | arm 7 s2 (`pgd_at`) | Δ |
  |---|---|---|---|---|
  | best | clean | 75.75 | 73.23 | **+2.52** |
  | best | CE-PGD20 | 48.09 | 44.40 | **+3.69** |
  | best | AutoAttack | 42.56 | 40.18 | **+2.38** |
  | last | clean | 76.30 | 74.71 | **+1.59** |
  | last | CE-PGD20 | 47.70 | 44.24 | **+3.46** |
  | last | AutoAttack | 42.07 | 39.50 | **+2.57** |

  Read against arm 7's full three-seed means instead of its seed 2 alone, the
  best-checkpoint AutoAttack gain is **+2.64 pp** (42.56 − 39.92) and the
  last-checkpoint gain **+2.74 pp** (42.07 − 39.33).
  Sanity against the standing autonomous-check instruction: AutoAttack ≤ CE-PGD20
  ≤ clean at both checkpoints; best-checkpoint AutoAttack above last by 0.49 pp,
  the expected robust-overfitting direction; best-checkpoint clean 0.55 pp *below*
  its own last checkpoint, which is what selecting on robustness rather than
  clean accuracy produces; and the arm sits ≈6 pp below the ResNet-18 ADR seed 0
  figure (42.56 against 48.75) on AutoAttack, consistent with the ≈7–9 pp
  architecture gaps the earlier baseline entries measured. **No deviation, no bug
  found.** One check **could not be run**: this run's stdout is not on disk — the
  producing session's lane log is gone and the offline W&B directory carries no
  `output.log` — so AutoAttack's own printed per-stage accuracies, the
  `max Linf perturbation` line and the `nan in tensor` line could not be read,
  and the AutoAttack-estimator-versus-unbatched-recomputation comparison the
  15:46Z entry was able to make for arm 6 is unavailable here. The recorded
  `autoattack_accuracy` (0.4256 / 0.4207) is identical in
  `evaluation-results.json` and in the standalone `autoattack-best.json` /
  `autoattack-last.json` side-files, which is an internal consistency check that
  passes but is not the same check.

  **What the preregistered rule now says, and why it is still not a verdict.**
  The primary quantity is the best-checkpoint model-weights AutoAttack gain of
  `adr` over matched plain-AT, MobileNetV2 (arm 8 − arm 7) compared against
  ResNet-18 (arm 3 − arm 2), and the aggregator computes it seed-paired
  (`_paired_delta`). Both legs now have exactly one shared seed:

  | leg | treatment | baseline | gain | baseline seed spread |
  |---|---|---|---|---|
  | MobileNetV2 (arm 8 − arm 7, seed 2) | 42.56 | 40.18 | **+2.38** | 0.45 pp (3 seeds) |
  | ResNet-18 (arm 3 − arm 2, seed 0) | 48.75 | 47.19 | **+1.56** | 0.22 pp (3 seeds) |

  MobileNetV2's gain is positive and exceeds ResNet-18's, which is the rule's
  **"sign confirmed"** branch, and neither gain is swallowed by the noise floor
  that is actually measured: +2.38 pp is 5.3× arm 7's own three-seed spread and
  +1.56 pp is 7.1× arm 2's. **But the rule compares the two gains, and that
  comparison is not resolved.** The gains differ by 0.82 pp, while binomial
  sampling error alone puts ≈0.70 pp on each gap (10,000 examples at p≈0.42 and
  p≈0.48 give ≈0.49–0.50 pp per run) and ≈**0.99 pp** on the difference between
  them — larger than the 0.82 pp being claimed. On top of that, **the seed spread
  of both treatment arms is entirely unmeasured**: arm 3 has one model-weights
  seed of three and arm 8 has one of three, so the rule's "do not force a verdict
  past what three seeds support" clause applies directly. What is licensed today
  is a **directional statement about these four runs**: ADR beats matched plain-AT
  on both architectures, by more on MobileNetV2 than on ResNet-18, in the one
  matched pair each leg has. **No population claim, and no verdict on packet
  0009's question.** M1c, M2 and M3 stay unticked.

  **A cheap way to sharpen this that needs no GPU.** The ≈0.99 pp figure above
  treats the four evaluations as independent samples, but all four scored the
  *same* 10,000 test images, and `sample-stats-{best,last}.parquet` records
  per-example outcomes for every one of them. A paired analysis (McNemar over the
  arm 8 / arm 7 disagreement counts) removes the shared finite-set error instead
  of double-counting it, and would tighten both gaps and their difference
  substantially. This is CPU-only work on artifacts already on disk. It is
  recorded here as an option for decision packet 0009 to consider; **this postrun
  did not do it and does not choose it.**

  **The EMA sibling is terminal, unrecorded, and owes its own postrun.**
  `eval-b89d6f7f0682d8cb3248` (`mobilenetv2_adr-s2`, `evaluation-ema/`) has read
  `terminal=true, success=true` since 14:06:21Z and no entry in this plan has ever
  recorded its numbers — the 14:08Z entry explicitly deferred them. Once recorded
  it will give the campaign's first same-run model-versus-EMA ("ADR" versus
  "ADR + WA") comparison, which is a listed secondary quantity. **Its lineage has
  not been verified by this postrun and its numbers are not recorded here**;
  reading its results file for orientation only, EMA weights sit slightly above
  model weights on AutoAttack at both checkpoints, by well under arm 7's own seed
  spread. Its postrun should be run next.

  **Lineage verified for this bundle**: all seven declared artifacts present both
  as live files and as content-addressed copies under
  `run-bundle/artifacts/<name>/<sha256>/`; source SHA `cd0b571e4685…` with
  `dirty=false` and `diff_sha256` equal to the SHA-256 of the empty string, i.e. a
  clean checkout of worktree `source-cd0b571e4685`; `external.lock` `05cfce4c…`
  with all five pinned upstreams at their expected commits and empty status;
  `evaluation-lineage.json`'s `training_config_hash` `15530b2f…` equal to the
  `config_hash` inside both result entries and to `training_runtime_config_hash`,
  with `training_config_migration.applied=[]` and source method id == runtime
  method id == `adr`, so no silent method remap; `train_run_id`
  `adr-campaign-v1-ferret-cifar10_mobilenetv2_adr-s2`, matching the training
  bundle this evaluation claims; AutoAttack provenance is the standard version of
  `autoattack` 0.1 at commit `a392200…` matching its own `expected_commit`, ε
  `8/255`, Linf, seed 0, batch size 128; evaluation attack CE, 20 steps, ε
  `8/255`, step `2/255`, random start, `pixel_0_1`, against a training attack of
  KL/`rectified` at 10 steps — all matching the frozen contract for arm 8;
  `trades_beta 6.0`, `adr` block `ema_decay 0.995 / T_high 2.5 / T_low 2.0 /
  λ_low 0.7 / λ_high 0.95`, `student_ema_decay 0.9`, `oracle_mask false`,
  `frozen_oracle_manifest null`, `teacher: null` as self-distillation requires,
  `nesterov: true`, `epochs=200`, `milestones=[100,150] gamma=0.1`,
  `validation_fraction=0.1`; both entries `weights=model`, no EMA claim; seeds all
  2 except `evaluation_attack=0` and the fixed `split=20260722`; `count: 10000`
  and `threat_hash 7081101693…` identical across both checkpoints and equal to
  arm 7's. (`dataset.num_samples: 16` in the resolved config is the vestigial
  schema field the 11:47Z and later entries already identified; `count: 10000` in
  the results is authoritative.) **One check could not be run**, as in the 15:12Z
  and 15:46Z entries: the two evaluated checkpoints' SHA-256 (`best.pt`
  `5743059b…`, `last.pt` `8285debf…`) were **not independently recomputed** —
  this session's sandbox refuses to hash files outside the repository root and the
  runtime tree is outside it. They are asserted by the producer and internally
  consistent, not confirmed here. Re-confirm before M2 signs off checkpoint
  lineage.

  **New evidence for decision packet 0010, which this postrun does not decide.**
  Two things, both recorded and neither chosen; 0010 is still `status: pending`,
  `chosen: null`.
  1. **The OOM is not arm-specific, and this is the cleanest demonstration yet.**
     This run is `mobilenetv2_adr` — the same arm and the same contract as
     `mobilenetv2_adr-s1`, whose model-weights evaluation is the failure that
     opened packet 0010 — and it completed **both** checkpoints on its first
     bundle-producing attempt in 1 h 06 m 42 s. That is direct support for the
     root cause commit `132be5d` recorded (allocator fragmentation under
     contention, not one pathological arm) and against any reading that ties the
     defect to a particular arm's memory profile.
  2. **A fifth observation of launches arriving while `chosen` is null, and a
     seventh lost failure directory.** 35 seconds after this job freed its GPU,
     `eval-4615ede05d4f96adb48f` was created at `16:17:13Z` in
     `cifar10_mobilenetv2_adr-s1/train/evaluation` — **overwriting the
     terminal-failed bundle** that the 15:12Z and 15:46Z entries both recorded as
     "terminal-failed and still on disk". After that overwrite the campaign has
     **no terminal-failed evaluation bundle left anywhere**, so packet 0010's
     primary on-disk exhibit no longer exists; the evidence for it now lives only
     in this plan and in the packet. This is the same "always `mv` before a
     retake" instruction from the 11:21Z entry not being followed, and the same
     "launches keep coming from an agent session while a packet's `chosen` is
     null" pattern the 14:05Z, 14:08Z, 15:12Z and 15:46Z entries flagged. **This
     postrun started nothing and stopped nothing**; stopping another session's
     jobs is the human's call, not this skill's.

  **Census at 16:21Z, from a full-root watcher scan (45 bundle rows, fresh
  cursor).** Training: **19 of 20 terminal and successful**;
  `cifar10_mobilenetv2_adr-s0` still never launched. Contract evaluations:
  **12 of 20** on model weights (the 15:46Z eleven plus this run) and **3 of 9**
  on EMA weights (`r18_adr-s0`, `r18_adr-s1`, `mobilenetv2_adr-s2`), unchanged.
  **Eight bundles are non-terminal and none is terminal-failed**: the new
  `mobilenetv2_adr-s1` model attempt (16:17:13Z), `r18_adr-s1` and
  `r18_trades_adr-s1` (15:44:43Z), `r18_trades_adr-s2`, `r18_trades-s1`,
  `r18_trades-s2` (`stale`), and the `r18_trades_adr-s0` and
  `mobilenetv2_adr-s1` EMA bundles; the watcher cannot tell alive from dead for
  any of them. **One training-terminal run has no model-weights evaluation bundle
  on disk at all**: `r18_adr-s2`. **Arms 2, 6 and 7 are complete on every axis
...
- **2026-09-10, 17:26Z — postrun of `eval-fcaab229ec62f00f2a01`
  (`cifar10_r18_trades_adr-s1`, model weights) — SUCCEEDED. Nothing imported, no
  milestone closed, nothing launched or retried.** Lane P: arm 5 (`adr_trades`,
  ADR-TRADES), seed 1, student (model) weights, both checkpoints, official
  CIFAR-10 test set. This is **arm 5's second seed on the model axis**, and it is
  the run the 16:57Z entry identified — by PID order and by a 13.71 GiB memory
  match — as the co-resident neighbour whose presence made lane O's OOM
  arithmetically unavoidable. **Both of that pair's outcomes are now known: one
  died, this one completed.**

  **Terminal status re-derived, not taken from the event.** A watcher scan of the
  exact `--state-path`
  (`runs/adr-cifar10-campaign-v1/cifar10_r18_trades_adr-s1/train/evaluation/run-bundle/manifest.json`)
  gives `terminal=true`, `success=true`, `failure_class=null`,
  `manifest_status=sync_pending`, `completion_json=true`, error marker "no
  application error recorded". The event hint said `--status completed`; the
  re-derivation agrees. `canon-eval-lane-P.log:407` closes with `exit=0`.

  **Numbers (official CIFAR-10 test set, all 10,000 examples, `split: test`,
  student/model weights, method `adr_trades`, no teacher, training seed 1,
  evaluation seed 0, `world_size=1`, `effective_global_batch_size=128`, protocol
  `controlled_cifar10_r18_adr_v1`, `validation_fraction=0.1`), beside arm 5's
  own seed 0 (`eval-6b2c6486fd99df2da5ac`, 14:08Z entry):**

  | checkpoint | quantity | arm 5 s0 | arm 5 s1 (this run) | seed spread |
  |---|---|---|---|---|
  | best | clean | 83.68 | 81.88 | 1.80 |
  | best | CE-PGD20 | 53.73 | 53.08 | 0.65 |
  | best | AutoAttack | 49.19 | **48.45** | **0.74** |
  | last | clean | 83.72 | 83.66 | 0.06 |
  | last | CE-PGD20 | 52.79 | 51.83 | 0.96 |
  | last | AutoAttack | 48.11 | **47.68** | **0.43** |

  Sanity against the standing autonomous-check instruction: AutoAttack ≤ CE-PGD20
  ≤ clean at both checkpoints (48.45 ≤ 53.08 ≤ 81.88; 47.68 ≤ 51.83 ≤ 83.66);
  AutoAttack ≤ its own APGD-CE stage ≤ the contract's CE-PGD20 (48.44 ≤ 52.68 ≤
  53.08; 47.65 ≤ 51.17 ≤ 51.83), i.e. the adaptive attack is the stronger one
  everywhere; best-checkpoint AutoAttack above last by 0.77 pp, the expected
  robust-overfitting direction; `max Linf perturbation 0.03137`, `nan in tensor
  0`, `max 1.00000 / min 0.00000` on both checkpoints, so inputs stayed inside the
  `pixel_0_1` domain; printed initial accuracies (81.88 %, 83.66 %,
  `canon-eval-lane-P.log:13,204`) equal the recorded clean accuracies to four
  decimals, and the per-stage survivor counts reconstruct each stage's own
  headline exactly (APGD-CE fed 8,188 and 8,366 examples, APGD-T 5,268 and 5,117,
  FAB-T/SQUARE 4,844 and 4,765). **No deviation, no bug found.**

  One oddity, recorded and not treated as a defect: **this seed's best checkpoint
  sits 1.78 pp below its own last checkpoint on clean accuracy** (81.88 against
  83.66), where seed 0's two checkpoints differ by 0.04 pp. Best is selected by
  CE-PGD20 on a held-out slice, so clean accuracy is free to move against it; this
  is the same effect the 15:46Z entry recorded for arm 6 at 3.02 pp, and there on
  a 1,000-image slice rather than this arm's 5,000. It costs nothing here — the
  robust numbers are what the rule reads — but it means arm 5's *clean* seed
  spread (1.80 pp at best) is not a measurement of run-to-run noise.

  **What this does and does not say about the science.**
  1. **It does not touch the preregistered primary quantity.** That quantity is
     the best-checkpoint AutoAttack gain of `adr` over matched plain-AT compared
     between MobileNetV2 (arm 8 − arm 7) and ResNet-18 (arm 3 − arm 2). Arm 5 is
     `adr_trades` and appears nowhere in it. The 16:16Z entry's reading of the
     rule is unchanged and still not a verdict.
  2. **Arm 5's two-seed spread is 0.74 pp on the primary metric, and that is not
     yet evidence of a noisy arm.** Binomial sampling error alone on a single
     10,000-image AutoAttack number at p≈0.49 is ≈0.50 pp, so the gap between two
     such numbers carries ≈0.71 pp of pure sampling noise. 0.74 pp is 1.05× that.
     Arm 5 needs its third seed before its spread can be compared against arm 2's
     0.22 pp or arm 7's 0.45 pp, both of which are three-seed figures. **Two seeds
     are a directional statement about these two runs, not a distribution.**
  3. **The secondary TRADES leg (arm 5 against arm 4) gains a second seed but no
     second matched pair.** Arm 4's only official-test number is its archived
     seed 0 (`docs/experiments/historical/trades_fix_v1_seed0/`: best AA 47.87,
     last 44.99); `r18_trades-s1` and `-s2` evaluations are still non-terminal, so
     **seed 1 has no baseline to be matched against**. The one seed-matched pair
     stays seed 0: 49.19 − 47.87 = **+1.32 pp** best-checkpoint AutoAttack. As
     this plan's own rule section states, this leg has **no Nesterov-matched
     TRADES baseline**, so any arm-5-minus-arm-4 gap confounds ADR-TRADES with
     Nesterov and is directional only.
  4. **The robust-overfitting reading is the more interesting secondary, and both
     arm-5 seeds agree on it.** Best-minus-last AutoAttack is **1.08 pp** (s0) and
     **0.77 pp** (s1) for ADR-TRADES, against **2.88 pp** for arm 4 seed 0's
     TRADES. On last checkpoints the gap to arm 4 s0 widens to +3.12 / +2.69 pp,
     against +1.32 / (unmatched) at best. That is what ADR claims its
     rectification does — hold the late-training model closer to its own best —
     and here two seeds move together by roughly 2 pp, well outside both the
     ≈0.5 pp per-run sampling error and arms 2/7's measured seed spreads. It is
     still confounded by Nesterov and still rests on one baseline seed. **Recorded
     as directional; not a verdict, and not the plan's decisive question.**
  5. **Orientation only:** arm 5's two best-checkpoint numbers (49.19, 48.45,
     two-seed mean 48.82) straddle the low edge of the literature band this plan
     uses for TRADES-family comparison (AdaAD 49.21 / RSLAD 49.23 / RSLAD-300
     49.50). Those are teacher-distillation methods, not this arm's teacher-free
     objective, so this is a bearing, not a comparison.

  **Lineage verified for this bundle**: all seven declared artifacts present both
  as live files and as content-addressed copies under
  `run-bundle/artifacts/<name>/<sha256>/`; `completion.json` reads
  `{"status": "completed", "results": 2}`; source SHA `cd0b571e4685…` with
  `dirty=false` and `diff_sha256` equal to the SHA-256 of the empty string, i.e. a
  clean checkout of worktree `source-cd0b571e4685`; `external.lock` `05cfce4c…`
  with all five pinned upstreams at their expected commits and empty status;
  `evaluation-lineage.json`'s `training_config_hash` `0c3852c4…` equal to the
  `config_hash` inside both result entries and to `training_runtime_config_hash`,
  with `training_config_migration.applied=[]` and source method id == runtime
  method id == `adr_trades`, so no silent method remap; `train_run_id`
  `adr-campaign-v1-ferret-cifar10_r18_trades_adr-s1`, matching the training bundle
  this evaluation claims (whose own postrun is the 2026-09-10 entry above);
  AutoAttack provenance is the standard version of `autoattack` 0.1 at commit
  `a392200…` matching its own `expected_commit`, ε `8/255`, Linf, seed 0, batch
  size 128; evaluation attack CE, 20 steps, ε `8/255`, step `2/255`, random start,
  `pixel_0_1`, against a training attack of KL with `kl_target: student_clean` at
  10 steps and a CE/20-step selection attack — all matching the frozen contract
  for arm 5; `trades_beta 6.0`, `adr` block `ema_decay 0.995 / T_high 2.5 /
  T_low 2.0 / λ_low 0.7 / λ_high 0.95`, `student_ema_decay 0.9`,
  `oracle_mask false`, `frozen_oracle_manifest null`, `teacher: null` as
  self-distillation requires, `nesterov: true`, `epochs=200`,
  `milestones=[100,150] gamma=0.1`, `validation_fraction=0.1`; both entries
  `weights=model`, no EMA claim; seeds all 1 except `evaluation_attack=0` and the
  fixed `split=20260722`; `count: 10000` and `threat_hash 7081101693…` identical
  across both checkpoints and equal to arms 6/7/8's. **One check could not be
  run**, as at 15:12Z, 15:46Z and 16:16Z: the two evaluated checkpoints' SHA-256
  (`best.pt` `bbf560c5…`, `last.pt` `016a2bc1…`) were **not independently
  recomputed** — this session's sandbox refuses to hash files outside the
  repository root and the runtime tree is outside it. They are asserted by the
  producer and internally consistent, not confirmed here. Re-confirm before M2
  signs off checkpoint lineage.

  **The AutoAttack-versus-recompute offset held, for the eleventh and twelfth
  time.** AutoAttack's own printed robust accuracy was best **48.44 %** / last
  **47.65 %** (`canon-eval-lane-P.log:201,405`); this project's post-attack
  unbatched recompute recorded **48.45** / **47.68**. The differences are
  **+0.01 pp and +0.03 pp**, 1 and 3 images out of 10,000, both in the positive
  direction the 11:47Z and 15:46Z entries already measured (non-batched always
  reads higher, at most 0.09 pp). Arm 5 seed 0's own log agrees: printed 49.17 /
  48.10 (`canon-eval-lane-K.log:218,423`) against recorded 49.19 / 48.11, +0.02
  and +0.01 pp. This is **not a new phenomenon and does not change any number
  here** — 0.01–0.03 pp is one to two orders of magnitude below every spread this
  plan compares — but it is the same starting point packet 0010's Option A
  regression test will need, and Option A's own preregistered check (batched
  versus unbatched recompute) is still a different comparison that nobody has
  measured.

  **New, and the sharpest cost measurement this campaign has produced: co-residency
  did not merely kill lane O, it halved this survivor.** The four AutoAttack stages
  ran, per checkpoint (best → last): APGD-CE 109.8 → 71.5 s, APGD-T 654.4 →
  330.9 s, FAB-T 1481.7 → 721.4 s, SQUARE 1649.3 → 798.3 s. That is **1.54×,
  1.98×, 2.05× and 2.07×** — a near-uniform factor of two, in the same process,
  on the same card, against the same model family. **Workload does not explain
  it**: APGD-CE processed *more* examples in the second half (8,366 against 8,188)
  and still finished in 65 % of the time, and APGD-T/FAB-T/SQUARE differ by only
  1.6–2.9 % in input count. The timeline does explain it. Working back from
  `finished_at 17:22:32.9Z` through SQUARE's own 1922.1 s cumulative mark, the
  last-checkpoint AutoAttack began about **16:50:30Z**, and lane O
  (`eval-e2da1d97fcd3e5932fab`) died at **16:52:17.6Z** — so the best-checkpoint
  half ran alongside a second ResNet-18 AutoAttack evaluation for essentially its
  whole life, and the last-checkpoint half ran with that neighbour gone for 94 %
  of its own. (Lane Q, `mobilenetv2_adr-s1`, was resident from 16:17Z throughout;
  the variable that changed is the ResNet-18 neighbour.) **This is a reconstruction
  from AutoAttack's cumulative timing marks and the bundle timestamps, not from a
  process table** — `ps` is not available to this session. Read with the 16:57Z
  entry's allocator arithmetic, packet 0010's defect now has two costs, not one:
  the job that loses the race returns nothing, and the job that wins it takes
  about twice as long. The GPU-hour budget in this plan assumed neither.

  **`mobilenetv2_adr-s1`'s model-weights evaluation also succeeded, and owes its
  own postrun.** `eval-4615ede05d4f96adb48f` — the 16:16Z entry's overwrite of
  packet 0010's lost exhibit — reads `terminal=true, success=true` as of
  17:23:59Z, with `evaluation-results.json` on disk. **Its lineage has not been
  verified by this postrun and its numbers are not recorded here.** It is arm 8's
  second model-weights seed; once recorded, the MobileNetV2 leg of the
  preregistered rule will have two of three seeds. Its postrun should be run next.

  **Census at 17:26Z, from a full-root watcher scan (47 bundle rows, fresh
  cursor).** **Training: 20 of 20 terminal and successful — for the first time,
  every planned training run in this campaign is done.**
  `cifar10_mobilenetv2_adr-s0`, which every entry since 11:47Z recorded as "never
  launched", reached epoch 199 at **17:19:02Z** as
  `adr-campaign-v1-ferret-cifar10_mobilenetv2_adr-s0`, and its model-weights
  evaluation started at 17:23Z (`eval-750b3422e05de8434c22`, running). **Every arm
  now has its full seed count trained.** That training run's own postrun ran
  **concurrently with this one in a separate headless session** and committed
  `a301573` while this entry was being written; the 17:27Z entry below is its
  work, and it verifies the arm-8 seed-0 bundle field by field. Its
  "owes a postrun" list names `r18_trades_adr-s1` because it was written a minute
  after this scan and before this entry landed; **this entry closes that item.**
  Contract evaluations:
  **14 of 20** on model weights (the 16:21Z twelve plus this run and
  `mobilenetv2_adr-s1`) and **3 of 9** on EMA weights (`r18_adr-s0`,
  `r18_adr-s1`, `mobilenetv2_adr-s2`), unchanged. **One evaluation bundle is
  terminal-failed**: `r18_adr-s1` model weights, still the campaign's only on-disk
  exhibit for packet 0010. **Six are non-terminal**: `mobilenetv2_adr-s0` model
  (new), `r18_trades-s1`, `r18_trades_adr-s2`, `mobilenetv2_adr-s1` EMA and
  `r18_trades_adr-s0` EMA (`running`), and `r18_trades-s2` (`stale`); the watcher
  cannot tell alive from dead for any of them. **One training-terminal run still
  has no model-weights evaluation bundle on disk at all**: `r18_adr-s2` — and four
  EMA evaluations have never been launched (`r18_adr-s2`, `r18_trades_adr-s1`,
  `r18_trades_adr-s2`, `mobilenetv2_adr-s0`).
  **M1c's training half is complete and its evaluation half is not**, so the
  checkbox stays unticked: M1c covers launching the 20 training *and* evaluation
  jobs, and `r18_adr-s2`'s model-weights evaluation plus those four EMA
  evaluations have never been started. M2 and M3 stay open. **Arms 2, 6 and 7 are
  complete on every axis their own contracts define; arms 1, 3, 4, 5 and 8 are
  not** — arm 5 now has two model-weights seeds and no EMA seed.

- **2026-09-10, 17:27Z — postrun of the training run
  `adr-campaign-v1-ferret-cifar10_mobilenetv2_adr-s0` (arm 8, seed 0) —
  SUCCEEDED. This is the twentieth and last training run of the campaign.
  Nothing imported, no milestone closed, nothing launched or retried.**

  **Terminal status re-derived, not taken from the event.** A watcher scan of the
  exact `--state-path`
  (`runs/adr-cifar10-campaign-v1/cifar10_mobilenetv2_adr-s0/train/run-bundle/manifest.json`)
  gives `terminal=true`, `success=true`, `failure_class=null`,
  `completion_json=true`, error marker `no application error recorded`, epoch
  199, global step 70400. The event hint said `--status completed`; the
  re-derivation agrees. The manifest's own `status` field reads `sync_pending`,
  which is the offline-W&B state, not a job state — every one of the campaign's
  twenty training bundles reads `sync_pending`, including the eighteen whose
  results are already recorded here.

  **All twenty training runs are now terminal and successful.** A full-root scan
  at 17:27Z (fresh cursor) shows a training `run-bundle` for each of the twenty
  new runs — `r18_pgd_at-s1/-s2`, `r18_pgd_at_nesterov-s0/-s1/-s2`,
  `r18_adr-s0/-s1/-s2`, `r18_trades-s1/-s2`, `r18_trades_adr-s0/-s1/-s2`,
  `r18_trades_49k_validation-s0`, `mobilenetv2_pgd_at-s0/-s1/-s2`,
  `mobilenetv2_adr-s0/-s1/-s2` — and every one reads `completed / success=true /
  terminal=true`. No training run in this campaign has ever failed. Arms 1 and 4
  reach their three seeds by adding these to the reused historical seed-0 runs
  (M1a). **The training axis of M1c is therefore closed**; its evaluation axis is
  not, so the checkbox stays unticked.

  **Contract identity verified field by field against the frozen arm-8 config.**
  `run-bundle/resolved_config.yaml` matches
  `configs/scientific/cifar10_mobilenetv2_adr.yaml`, which `git diff
  cd0b571e4685…HEAD` shows has not changed since the pinned SHA, on every
  contract field: protocol `controlled_cifar10_mobilenetv2_adr_v1`, method `adr`,
  `teacher: null` as self-distillation requires, student `mobilenet_v2_cifar`
  with `pixel_0_1` input domain and the `cifar10_standard` normalization profile
  owned by the student adapter, training attack KL/`rectified` ε `8/255` step
  `2/255` 10 steps random start, selection attack CE 20 steps at the same budget
  with `temperature_squared: true`, `adr` block `ema_decay 0.995 / T_high 2.5 /
  T_low 2.0 / λ_low 0.7 / λ_high 0.95`, `trades_beta 6.0`, `student_ema_decay
  0.9`, `oracle_mask false`, `frozen_oracle_manifest null`, SGD `lr 0.1 /
  momentum 0.9 / wd 5e-4 / nesterov true`, multistep `[100, 150] gamma 0.1`,
  `epochs 200`, `validation_fraction 0.1`, `batchnorm_mode local_per_rank`,
  world size 1 and per-rank = global batch 128, `deterministic: true`, seeds all
  **0** except the fixed `split: 20260722` and `evaluation_attack: 0`. Compared
  line by line against seed 2's resolved config, the two differ **only** in the
  five `ARD_SEED`-templated seeds, the run id and the output directory — which is
  exactly what a seed replicate should differ in. (`dataset.num_samples: 16` is
  the vestigial schema field the 11:47Z entry already identified; it is unused in
  training.)

  **Lineage verified for this bundle.** Both declared artifacts —
  `epoch-metrics.parquet` (`68dfc6e0…`) and `sample-stats-train.parquet`
  (`d9bfa380…`) — are present as content-addressed copies under
  `run-bundle/artifacts/<name>/<sha256>/`. Source SHA `cd0b571e4685…` with
  `dirty=false` and `diff_sha256` equal to the SHA-256 of the empty string, i.e.
  a clean checkout; the lane log confirms the worktree was prepared detached at
  `cd0b571` before launch. `external.lock` `05cfce4c…` — byte-identical to the
  hash carried by seeds 1 and 2 and by every evaluation bundle recorded above —
  with `trades`, `robustbench`, `saad` and `da_alone_improves_at` at their
  expected commits and empty status. The `adr` upstream is pinned at
  `515da0e0…` but has `checkout.exists: false`, because the Ferret side has no
  `.external/adr` working copy; seed 2 (also run on Ferret) records the same
  thing and seed 1 (run locally) records `exists: true`. The pin itself is
  recorded and the lock hash is identical in all three, so nothing about the
  contract is unverified — `.external/adr` is a read-only reference for
  hyperparameter confirmation and is never imported by training. Checkpoints on
  disk are `best.pt`, `last.pt`, **`best-ema.pt`** and the four contracted
  periodic checkpoints `epoch-{049,099,149,199}.pt`. `best-ema.pt` being present
  matters: its absence or a disagreeing selection is a scientific stop for this
  arm under the Stop rules. `epoch_metrics_complete: true` with
  **200 of 200** epochs recorded from `local_canonical_epoch_rows`. The lane log
  (`ferret-logs/cifar10_mobilenetv2_adr-s0.log`, 179 lines) contains no
  traceback, no warning, no NaN and no OOM; it is a poller that was still
  printing `running` when it was cut off, so completion is read from the bundle,
  not from it. **One check could not be run**, as at 15:12Z, 15:46Z and 16:16Z:
  the two artifacts' SHA-256 were **not independently recomputed** — this
  session's sandbox refuses to hash files outside the repository root. They are
  asserted by the producer and their content-addressed paths are internally
  consistent, not confirmed here.

  **Training-time validation for this run**, on the held-out 10 % split, CE-PGD-20
  at ε `8/255`: best **48.72 %** robust / **76.60 %** clean at epoch 152; last
  (epoch 199) **48.38 %** / **76.58 %**; best-minus-last robust gap **0.34 pp**.
  **These are internal validation numbers and are never an official test result**
  — different attack, different data, different selection role from the
  preregistered quantity. They are recorded here as a sanity and consistency
  check, and nothing in this entry may be carried into a record, a report or a
  results table.

  **With this run, both legs of the preregistered comparison have all three seeds
  on the validation axis.** Twelve runs — arms 2, 3, 7, 8 — now read (best
  checkpoint, held-out CE-PGD-20, percent):

  | arm | s0 | s1 | s2 | mean | spread |
  |---|---|---|---|---|---|
  | 8 `mobilenetv2_adr` | 48.72 | 48.50 | 48.30 | 48.51 | 0.42 |
  | 7 `mobilenetv2_pgd_at` | 44.96 | 45.02 | 44.94 | 44.97 | 0.08 |
  | 3 `r18_adr` | 53.94 | 53.96 | 54.34 | 54.08 | 0.40 |
  | 2 `r18_pgd_at_nesterov` | 51.84 | 51.88 | 51.84 | 51.85 | 0.04 |

  Seed-paired, ADR beats its matched plain-AT baseline in **all six pairs**: on
  MobileNetV2 by **+3.76 / +3.48 / +3.36 pp** (mean **+3.53**) and on ResNet-18
  by **+2.10 / +2.08 / +2.50 pp** (mean **+2.23**). The MobileNetV2 gain exceeds
  the ResNet-18 gain **in every one of the three seed pairs** (+1.66, +1.40,
  +0.86 pp), and both gains are an order of magnitude larger than either
  baseline arm's own seed spread. That is the same ordering as the single
  matched pair the official test has produced so far (+2.38 against +1.56 pp on
  best-checkpoint AutoAttack, 16:16Z entry).

  **This does not advance the preregistered rule, and must not be read as if it
  did.** The rule names official-test AutoAttack on the best checkpoint. What is
  tabulated above is training-time CE-PGD-20 on a 5,000-image held-out slice of
  the *training* set — a weaker attack, a different dataset, and the very
  quantity that selected `best.pt`, so it is optimistically biased by
  construction and cannot be treated as an unbiased estimate of anything. Its
  value here is narrower and real: it says the three seeds of each treatment arm
  are tightly clustered (0.40–0.42 pp) rather than scattered, so the two
  treatment arms' unmeasured official-test seed spread is unlikely to be wild,
  and it says the sign of the effect is stable across seeds rather than resting
  on one lucky pair. **The verdict still needs the remaining official-test
  AutoAttack evaluations, and packet 0009's question stays open.**

  **Robust overfitting, same caveat, reported because it is a listed secondary
  quantity.** Best-minus-last validation robust accuracy: arm 8 **0.34 / 0.76 /
  0.68 pp** against arm 7's **0.62 / 1.32 / 1.18 pp**, and arm 3 **4.56 / 4.08 /
  3.72 pp** against arm 2's **8.88 / 8.64 / 8.82 pp**. On ResNet-18 ADR roughly
  halves the gap in every seed; on MobileNetV2 both arms barely overfit at all,
  so the comparison there is between two small numbers. Whether this survives to
  the official test is an M3 question.

  **Cost, and a correction to this plan's own budget for the Ferret lanes.**
  `created_at 2026-09-10T02:16:06.318Z` → `finished_at 17:19:02.023Z` =
  **15 h 02 m 56 s** for one 200-epoch run, against the ≈3.0 GPU-hour figure the
  budget section extrapolated from the ResNet-18 canary. The last epoch ran at
  **207.6 img/s** (216.8 s of training loop). Seed 2, also on Ferret, took
  **14 h 10 m 55 s** at 214.1 img/s; seed 1, run locally, took **5 h 32 m 36 s**
  at 730.7 img/s, and the three arm-7 MobileNetV2 baselines took 3 h 44 m to
  4 h 31 m at 783–809 img/s. So MobileNetV2 ADR costs ≈3.4 GPU-hours per run when
  it has a card to itself — close to the budget — and ≈15 hours when three
  evaluation lanes are sharing the same box. **The gap is contention on Ferret,
  not a property of the arm**: seeds are fixed, `deterministic: true`, and
  throughput does not touch any number above. The budget table's wall-clock
  estimate for the Ferret half of the campaign was optimistic by roughly 4×;
  its GPU-hour arithmetic was not.

  **A sixth observation of a launch arriving while packet 0010 is `chosen:
  null`.** `eval-750b3422e05de8434c22` was created at **17:23Z** in
  `cifar10_mobilenetv2_adr-s0/train/evaluation`, about four minutes after this
  training run freed its GPU, and is reading `running`. It is arm 8 seed 0's
  model-weights contract evaluation — the run that would complete the
  MobileNetV2 leg's second matched pair — and it is exactly the AutoAttack path
  that has now failed ten times under contention. **This postrun started nothing
  and stopped nothing**; stopping another session's jobs is the human's call.
  Packet 0010 is still `status: pending`, `chosen: null`, and its Option A
  (batching the `autoattack.py:213` forward) is still unimplemented.

  **Nothing imported.** `docs/experiments/` holds no record for this contract and
  none can be written: the aggregator refuses on the first missing
  `evaluation-results.json` and **6 of the 20 model-weights evaluations** are
  absent, failed or still running (`r18_adr-s1` failed, `r18_adr-s2` never
  started, `r18_trades-s1`, `r18_trades-s2`, `r18_trades_adr-s2` and
  `mobilenetv2_adr-s0` non-terminal), as are **6 of the 9** EMA evaluations. No
  evidence-ledger row is owed either, since a ledger row accompanies a record.

  **Census at 17:27Z, from a full-root watcher scan (47 bundle rows, fresh
  cursor).** Training: **20 of 20 terminal and successful** — the first time this
  has been true. Model-weights contract evaluations: **14 of 20** terminal and
  successful, two more than the 16:57Z entry (`mobilenetv2_adr-s1`, whose
  16:17:13Z attempt has since completed, and `r18_trades_adr-s1`); **1
  terminal-failed** (`r18_adr-s1`, whose directory is still on disk and is packet
  0010's only surviving exhibit); **4 non-terminal**
  (`mobilenetv2_adr-s0` at 17:23Z, `r18_trades-s1`, `r18_trades-s2` reading
  `stale`, `r18_trades_adr-s2`); **1 with no bundle at all** (`r18_adr-s2`).
  EMA-weights evaluations: **3 of 9** terminal and successful (`r18_adr-s0`,
  `r18_adr-s1`, `mobilenetv2_adr-s2`), **2 non-terminal**
  (`mobilenetv2_adr-s1`, `r18_trades_adr-s0`), **4 with no bundle at all**
  (`r18_adr-s2`, `r18_trades_adr-s1`, `r18_trades_adr-s2`,
  `mobilenetv2_adr-s0`). **Three terminal successful results are unrecorded and
  owe postruns**: `mobilenetv2_adr-s1` model, `r18_trades_adr-s1` model, and
  `mobilenetv2_adr-s2` EMA. Arms 2, 6 and 7 remain complete on every axis their
  contracts define. M1c's training half is closed and its evaluation half is not,
  so M1c stays unticked; M2 and M3 stay open.

- **2026-09-10, 18:33Z — postrun of the evaluation
  `eval-74a8f5d0042ebea0b0d1` (arm 8 `cifar10_mobilenetv2_adr`, seed 1, **EMA
  weights**, both checkpoints, official CIFAR-10 test set) — SUCCEEDED. This is
  arm 8's first recorded EMA-weights result. Nothing imported, no milestone
  closed, nothing launched or retried.**

  **Terminal status re-derived, not taken from the event.** A
  `campaign_watch.py --once --emit-existing --include-hand-run` scan whose
  `--roots` is this bundle's own parent returns, on the line whose `path` is
  exactly the `--state-path` the watcher passed, `terminal: true`,
  `success: true`, `status: completed`, `failure_class: null`,
  `completion_json: true`, error marker `no application error recorded`,
  progress timestamp 18:30:53.565016Z. The manifest's own `status` field reads
  `sync_pending`, which is the offline-W&B state and not a job state — the same
  accepted terminal value every recorded bundle in this campaign carries. The
  lane log agrees independently: `[lane-S][eval-done]
  cifar10_mobilenetv2_adr-s1 weights=ema exit=0 2026-09-11T03:30:54+09:00`.
  `run-bundle/completion.json` reads `{"status": "completed", "results": 2}`.

  **This bundle is the second attempt at the same evaluation, and the first one
  was the SIGTERM'd bundle the 11:26Z entry used as its example.** That entry
  recorded `eval-74a8f5d0042ebea0b0d1` with `created_at 11:07:28.531Z` as
  SIGTERM'd at 20:10:21+09:00 and frozen at `"status": "running"` with no
  `finished_at` and no error marker — permanently non-terminal, because a
  SIGTERM writes no terminal state. That directory is gone. The replacement was
  written at `created_at 17:24:27.981Z`. The 22 seconds in between are legible
  in the lane logs and are worth recording: lane Q finished this run's
  *model*-weights pass (`weights=model exit=0`) at 02:24:00+09:00, immediately
  started the EMA pass, and was **refused by the engine's own guard** —
  `FileExistsError: refusing to overwrite existing evaluation output` at
  02:24:03+09:00 (`canon-eval-lane-Q.log:384`). Lane S then started the same
  evaluation into the same path successfully at 02:24:25+09:00. So the stale
  directory was removed by something between those two moments. **This postrun
  did not remove it and cannot identify what did** — `ps` is unavailable to this
  session and no lane log records a deletion. Two things follow. The guard works
  and is the reason a half-dead directory was not silently written over. And the
  campaign's *evidence* keeps being cleared by whatever is driving these lanes,
  which is the same mechanism that has now destroyed packet 0010's exhibit three
  times.

  **Contract identity verified field by field against the frozen arm-8 config.**
  `resolved_evaluation_config.yaml` matches
  `configs/scientific/cifar10_mobilenetv2_adr.yaml`, and `git diff
  cd0b571e4685…HEAD -- configs/scientific/cifar10_mobilenetv2_adr.yaml
  configs/evaluation/autoattack_saved_checkpoint.yaml configs/protocols/
  src/ard/evaluation src/ard/cli/evaluate.py` is **empty**, so neither the
  contract nor the evaluation code has moved since the pinned SHA and the code
  read here is the code that ran. Protocol `controlled_cifar10_mobilenetv2_adr_v1`,
  method `adr`, `teacher: null` as self-distillation requires, student
  `mobilenet_v2_cifar` with `pixel_0_1` input domain and the `cifar10_standard`
  normalization profile owned by the student adapter; training attack KL /
  `kl_target: rectified`, ε `8/255`, step `2/255`, 10 steps, random start,
  `temperature_squared: false`; selection attack CE, 20 steps, same budget,
  `temperature_squared: true`; `adr` block `ema_decay 0.995 / T_high 2.5 /
  T_low 2.0 / λ_low 0.7 / λ_high 0.95`, `trades_beta 6.0`,
  `student_ema_decay 0.9`, `student_policy_warmup_epochs 1`, `oracle_mask false`,
  `frozen_oracle_manifest null`; SGD `lr 0.1 / momentum 0.9 / wd 5e-4 /
  nesterov true`, multistep `[100, 150] gamma 0.1`, `epochs 200`,
  `validation_fraction 0.1`, `batchnorm_mode local_per_rank`, world size 1 and
  per-rank = global batch 128, `deterministic: true`. Seeds all **1** except the
  fixed `split: 20260722` and `evaluation_attack: 0`. Evaluation identity:
  `checkpoints: both`, `split: test`, `count: 10000` on both rows, CE-PGD-20 at
  ε `8/255` step `2/255` 20 steps random start, `evaluation_seed: 0`,
  `autoattack: true` at batch 128, threat hash `7081101693340e70…` — the **same**
  threat hash as every other evaluated arm in this campaign, so these EMA rows
  are scored under a byte-identical threat model to every student row.
  AutoAttack is the **standard** version with real provenance
  (`expected_commit == vcs_commit == a39220048b3c9f2cca9a4d3a54604793c68eca7e`),
  in its own process. `weights: ema` on both rows; `selection_weights` is
  `"ema"` for `best-ema.pt` and `"model"` for `last.pt`, which is correct for the
  reason given at 11:30Z — `last.pt` is epoch-defined and selection-independent,
  so its selection story is the student's while its read weights are the EMA's.
  Nothing was weakened, and no epsilon, step, step count, random start,
  temperature, schedule, checkpoint selection or evaluation attack differs from
  the frozen contract.

  **Lineage verified for this bundle.** All seven declared artifacts are present
  both as live files and as content-addressed copies under
  `run-bundle/artifacts/<name>/<sha256>/`
  (`resolved_evaluation_config.yaml`, `evaluation-lineage.json`,
  `evaluation-results.json`, `panel-best-ema-ema.jsonl`, `panel-last-ema.jsonl`,
  `sample-stats-best-ema-ema.parquet`, `sample-stats-last-ema.parquet`), plus
  `autoattack-best-ema.json` and `autoattack-last.json`. Source SHA
  `cd0b571e4685…` with `dirty: false`, `diff_sha256` equal to the SHA-256 of the
  empty string, and a zero-byte `run-bundle/diff.patch` — a clean checkout of
  worktree `source-cd0b571e4685`. `external.lock` `05cfce4cf8db…`, byte-identical
  to every other bundle in this campaign, with all five pinned upstreams at their
  expected commits and empty `status`; the `adr` pin `515da0e0…` has
  `checkout.exists: true` here because this lane ran locally on Hamster.
  `evaluation-lineage.json` binds the evaluation to the right training run:
  `training_config_hash cd224c80a17f…`, equal to `training_runtime_config_hash`,
  to `raw_mapping_hash` and to the `config_hash` carried inside both result rows,
  with `training_config_migration.applied = []` and source method id == runtime
  method id == `adr`, so no silent method remap. `train_run_id`
  `adr-campaign-v1-cifar10_mobilenetv2_adr-s1`. **One check could not be run**,
  as at 15:12Z, 15:46Z, 16:16Z, 17:26Z and 17:27Z: the two evaluated checkpoints'
  SHA-256 (`best-ema.pt` `5b47ecdd0822…`, `last.pt` `7690f0806ed3…`) were **not
  independently recomputed** — this session's sandbox refuses to hash files
  outside the repository root. They are asserted by the producer and internally
  consistent, not confirmed here. The aggregator's own `_verify_bundle` re-hash
  at M3 is the check of authority.

  **Host environment is invariant.** `run-bundle/environment.json` reads
  `torch 2.11.0+cu128`, `python 3.11.15`, `cuda 12.8`, `cudnn 91900`,
  `Linux-7.0.0-31-generic`, RTX 4090 — **byte-identical** to the environment
  block of the already-recorded `mobilenetv2_pgd_at-s1` evaluation bundle it will
  be compared against. Nothing about the `adv`→`ard-v2` conda switch (commit
  `757851f`) reached these lanes: they all still run the `adv` interpreter, and
  that interpreter is the same one every recorded evaluation in this campaign
  used.

  **Official CIFAR-10 test set (10,000 examples), EMA weights, arm 8
  (`mobilenetv2_adr`) seed 1 — clean, CE-PGD-20 and AutoAttack reported
  separately, best and last kept separate:**

  | checkpoint | sha256 (short) | selected epoch | clean | CE-PGD-20 | AutoAttack (record) | AutoAttack (own print) |
  |---|---|---|---|---|---|---|
  | `best-ema.pt` (EMA branch's own selection) | `5b47ecdd0822…` | 141 | 75.13 % | 49.01 % | 43.34 % | 43.31 % |
  | `last.pt` (epoch 199, EMA weights read) | `7690f0806ed3…` | 199 | 76.42 % | 47.88 % | 42.67 % | 42.66 % |

  AutoAttack's own `initial accuracy` prints — 75.13 % and 76.42 % — match the
  recorded clean accuracies exactly, so both checkpoints were loaded as intended.
  `max Linf perturbation: 0.03137` (= 8/255) with `nan in tensor: 0` and inputs
  in [0, 1] on both passes.

  **The two selections are ten epochs apart — the widest this campaign has
  measured.** Read from `cifar10_mobilenetv2_adr-s1/train/epoch-metrics.jsonl`
  (200 of 200 epoch rows present), the student's `val_pgd_accuracy` peaks at
  **0.4850 at epoch 151** (→ `best.pt`) and the EMA shadow's
  `val_pgd_accuracy_ema` peaks at **0.4914 at epoch 141** (→ `best-ema.pt`).
  For comparison, the same measurement gave **one** epoch of separation for arm 3
  seed 0 and **six** for arm 3 seed 1 (15:46Z entry). This matters for how the
  "ADR + WA" secondary may be read: the larger the separation, the more of any
  EMA-versus-student gap is a *selection* difference rather than a weights
  difference, and at ten epochs the two are no longer close to being the same
  epoch. The one strictly paper-equivalent same-epoch comparison
  (`best-ema.pt` read at both `--weights=model` and `--weights=ema`, per
  `docs/SCIENTIFIC_INVARIANTS.md`'s ADR section) is **not** formable from this
  campaign's job set, exactly as the 15:46Z entry found for arm 3.

  **This does not advance the preregistered primary quantity, and must not be
  read as if it did.** The rule names ADR's AutoAttack gain on **student**
  weights, best checkpoint, against matched plain AT. This bundle is the EMA
  axis, which the plan lists as a reported-but-not-decisive secondary. The
  seed-matched student-weights number that *would* advance the rule —
  `eval-4615ede05d4f96adb48f`, arm 8 seed 1, terminal and successful since
  17:23:59Z — is still unrecorded, and its lineage has not been verified by this
  postrun, so no arm-8 seed-1 primary figure appears here.

  What can be said from recorded numbers alone is narrow and stated as such.
  Against the seed-matched plain-AT baseline (arm 7 seed 1, **student** weights,
  recorded in the 12:00Z entry: best 39.86 %, last 39.26 %), this arm-8 seed-1
  **EMA** result reads **+3.48 pp** at best and **+3.41 pp** at last. That is an
  "ADR + WA versus plain AT" gap, not the preregistered "ADR versus plain AT"
  gap, and it is one seed pair. It is the same sign and roughly the same size as
  the one matched student-weights pair the campaign has on this leg (+2.38 pp,
  arm 8 s2 against arm 7 s2), and as the three-seed training-time validation gain
  (+3.53 pp, 17:27Z entry). **Directional for these two runs; not a verdict, and
  not a substitute for the missing student-weights postrun.**

  **Robust overfitting, a listed secondary.** Best-minus-last AutoAttack is
  **0.67 pp** on this EMA axis, against **0.60 pp** for the seed-matched arm 7
  student result. Both are small, and 0.67 pp is about the size of the ≈0.70 pp
  sampling error of a difference between two *independent* 10,000-image numbers
  at p ≈ 0.43. These two numbers are **not** independent — same images, same
  attack, two checkpoints of one run — so ≈0.70 pp is an upper bound rather than
  the right error bar, and the paired error was not computed here. Either way the
  MobileNetV2 arms barely overfit at all, so this comparison remains one between
  two small numbers, as the 17:27Z entry already noted for the validation axis.

  **The AutoAttack-versus-recompute offset held, for the thirteenth and
  fourteenth time.** AutoAttack's own printed robust accuracy was best-ema
  **43.31 %** / last **42.66 %** (`canon-eval-lane-S.log:184,371`); this
  project's post-attack unbatched recompute recorded **43.34** / **42.67** —
  **+0.03 pp and +0.01 pp**, 3 and 1 images out of 10,000, both in the positive
  direction every prior observation has shown (non-batched always reads higher,
  at most 0.09 pp). This changes no number here and is one to two orders of
  magnitude below every spread this plan compares; it is recorded because it is
  the baseline packet 0010's Option A regression test will need.

  **Co-residency did not halve this run, and that sharpens the 17:26Z finding
  rather than contradicting it.** Per-checkpoint AutoAttack stage times
  (best-ema → last), in seconds: APGD-CE 61.5 → 63.7, APGD-T 377.4 → 366.0,
  FAB-T 675.9 → 679.4, SQUARE 855.4 → 868.0; totals 1,970.2 → 1,977.1, a **0.35 %
  difference between the two halves**. The 17:26Z entry measured a near-uniform
  **2×** asymmetry on `r18_trades_adr-s1` and traced it to a neighbour dying
  midway. Here the neighbour (`eval-750b3422e05de8434c22`,
  `mobilenetv2_adr-s0` model weights) was resident from before this run started
  until 18:29:42Z — essentially the whole life of *both* halves — so the
  contention was constant and the halves match. The absolute cost is the other
  half of the picture: **3,947.3 s of AutoAttack for two checkpoints**, against
  **10,703.7 s** for the recorded arm 7 seed 1 run — same architecture, same
  10,000 images, same attack suite — even though this run left *more* points for
  the expensive later stages (43.3 % surviving here against ≈39.9 % there).
  Workload does not explain a 2.7× spread; host packing does. **This is a
  reconstruction from AutoAttack's cumulative timing marks and bundle
  timestamps, not from a process table.**

  **Packet 0010's on-disk exhibit was destroyed for the third time, one minute
  after this run finished.** `canon-eval-lane-T.log:1` starts a **fourth**
  model-weights evaluation of `cifar10_r18_adr-s1` at 2026-09-11T03:31:42+09:00
  (18:31:42Z), and that directory's manifest now reads `created_at
  18:31:44.975Z`, `status: running`, run id `eval-e2da1d97fcd3e5932fab`. The
  third OOM failure — the campaign's only surviving on-disk exhibit for packet
  0010 as of the 17:27Z census — is gone. **The campaign now has no
  terminal-failed evaluation bundle at all**, and packet 0010's evidence exists
  only in this plan's prose and in the lane logs (`canon-eval-lane-{D,M,O}.log`,
  each ending `exit=1`). This is the **seventh** observation of a scientific job
  being launched while packet 0010 is `status: pending`, `chosen: null`; its
  Option A (batching the `autoattack.py:213` forward) is still unimplemented.
  **This postrun started nothing and stopped nothing**; stopping another
  session's jobs is the human's call.

- 2026-09-10 19:45Z: postrun of the `eval-e2da1d97fcd3e5932fab` terminal event —
  **the run that opened decision packet 0010 finally produced a result, on its
  fourth attempt.** `cifar10_r18_adr-s1` (arm 3, `adr`, seed 1), model weights,
  both checkpoints, official 10,000-example CIFAR-10 test set, `--weights=model`.
  Terminal status re-derived with a fresh-cursor watcher scan of the bundle
  (`--include-hand-run`), keeping the row whose `path` is the event's
  `--state-path`: `kind=run`, `status=completed`, `terminal=true`,
  `success=true`, `failure_class=null`, `completion.json` present and reading
  `{"status": "completed", "results": 2}`, `error_marker` "no application error
  recorded". `manifest.status` reads `sync_pending`, which is the offline-W&B
  sync state and not the completion signal.

  **The numbers.** Both checkpoints, clean / CE-PGD-20 / AutoAttack (standard),
  all on the official test split, evaluation seed 0:

  | checkpoint | sha256 | clean | CE-PGD-20 | AutoAttack |
  |---|---|---|---|---|
  | `best.pt` | `9181001647cb…` | 82.82 | 53.34 | **48.65** |
  | `last.pt` | `a30c7077c0d6…` | 85.07 | 48.23 | **44.83** |

  Best minus last: clean **−2.25 pp** (the last checkpoint is the cleaner one, as
  every arm in this campaign shows), CE-PGD-20 **+5.11 pp**, AutoAttack
  **+3.82 pp**.

  **The preregistered primary quantity gains its second ResNet-18 matched pair,
  and the pair is as tight as this campaign gets.** Arm 3 seed 1's best-checkpoint
  AutoAttack **48.65** against arm 2 (`pgd_at_nesterov`) seed 1's **47.41**
  (`eval-ef916db6bef00134891b`) is **+1.24 pp**. The pair is matched on every
  field that matters and differs only in `method`: both are protocol
  `controlled_cifar10_r18_adr_v1`, `epochs=200`, SGD lr 0.1 / momentum 0.9 /
  **`nesterov: true`** / wd 5e-4, multistep `[100,150]` gamma 0.1,
  `validation_fraction=0.1`, student `saad_resnet18_cifar_v1`, `teacher: null`,
  `world_size=1` with effective global batch 128 and `local_per_rank` BN,
  training seeds all **1** with `split=20260722` and `evaluation_attack=0`,
  training-data fingerprint `c58f3010…`, and threat hash
  `7081101693340e70…` on both rows. Clean accuracy on the same pair is 82.82
  against 82.19, **+0.63 pp** — ADR is not buying robustness by giving up clean
  accuracy here.

  **Where the preregistered rule now stands.** Official-test AutoAttack, best
  checkpoint, model weights, seed-paired:

  | leg | pairs | seed-paired gains | mean |
  |---|---|---|---|
  | MobileNetV2 (arm 8 − arm 7) | 3 of 3 | +2.98 / +3.02 / +2.38 | **+2.79 pp** (sd 0.36) |
  | ResNet-18 (arm 3 − arm 2) | 2 of 3 | +1.56 / +1.24 | **+1.40 pp** |

  Both legs are positive in every pair, which is the sign packet 0009
  preregistered. The capacity claim — that the smaller network gains *more* — now
  reads **1.39 pp** of leg difference against **≈0.64 pp** of binomial sampling
  error on that difference (10,000 test images per run; per-run standard error
  ≈0.50 pp, propagated as three paired differences on one leg and two on the
  other). That is **2.2×** the sampling error, against 1.5× at the 18:32Z
  snapshot (1.23 against ≈0.81) and 0.8× at the 16:16Z one-pair-per-leg snapshot
  (0.82 against ≈0.99). **It is still not a verdict.** The rule asks for three
  seeds per leg and the ResNet-18 leg has two; `-s2`'s first model-weights
  evaluation only started at 19:36:18Z. Sampling error is also not the whole
  error — it excludes seed-to-seed training variance, and the three measured seed
  spreads in this campaign (arm 2 0.22 pp, arm 8 0.32 pp, arm 7 0.45 pp) are the
  same size as the sampling term, so the ≈0.64 pp figure is a floor, not a
  confidence interval.

  **Arm 3's model-axis seed spread is measurable for the first time, and it is
  0.10 pp.** Best-checkpoint AutoAttack 48.75 (s0) against 48.65 (s1); on the
  last checkpoint, 44.81 against 44.83, a **0.02 pp** spread. Both are far inside
  the ≈0.71 pp sampling noise of a two-run difference, and both are tighter than
  any other arm's measured spread. **Two seeds give a spread, not a variance
  estimate**, and nothing here is a population claim; but it is the first
  evidence that arm 3's number is not resting on a lucky seed, which is exactly
  what the 18:32Z entry flagged as the open risk on this leg.

  **Robust-overfitting suppression replicates at seed 1.** Best-minus-last
  AutoAttack **3.82 pp** here against seed 0's **3.94 pp** — a 0.12 pp difference
  between the two ADR student-weights readings. The four student-weights baseline
  readings sit in a band of **6.09–7.27 pp**, so both ADR seeds are below all of
  it. That is the direction ADR claims. Recorded as directional for these two
  runs; it is not a population claim and the baseline band is four readings from
  a different set of arms.

  **Contract and lineage verified; no deviation and no bug found.** `epsilon`
  8/255 with `step_size` 2/255 and `steps=10` `loss=kl` `kl_target=rectified` on
  the training attack, `steps=20` `loss=ce` on the evaluation attack, both with
  `random_start=true` in `pixel_0_1`; AutoAttack `standard`, batch 128, Linf,
  eps 8/255, seed 0, upstream commit `a39220048b3c…` with source digest
  `e74d6dab0e34…`; ADR hyperparameters `ema_decay=0.995`,
  `temperature 2.5 → 2.0`, `lambda 0.7 → 0.95`, `student_ema_decay=0.9`,
  `student_policy_warmup_epochs=1`, `entropy_gamma=1.0`, `oracle_mask=false`,
  `frozen_oracle_manifest: null` — all as the frozen contract specifies. Bundle
  lineage: `evaluation-lineage.json` names `training_config_hash`
  `47f44abe529c…`, which equals the `config_hash` recorded in this run's own
  training bundle manifest; `external.sha256` `05cfce4cf8db…` and the five
  upstream pins (`adr 515da0e0…`, `saad 295121c5…`, `trades 6e8e11b7…`,
  `robustbench 78fcc9e4…`, `da_alone 38b740ae…`) are identical to the training
  manifest's; `git.sha` is `cd0b571e4685…` with `dirty: false` and an empty diff
  digest, i.e. this plan's pinned base SHA; `group` matches the training bundle
  exactly. All seven declared `expected_outputs` are listed in the manifest with
  digests. **Caveat, as at every prior postrun in this campaign: the two
  evaluated checkpoints' SHA-256 could not be recomputed** — this session's
  sandbox refuses to hash or list outside the repository root — so the checkpoint
  identities are read from the evaluation record, not independently confirmed,
  and the training bundle does not carry checkpoint digests to compare against.

  **The packet-0010 estimator tally gains two comparisons and one of them is the
  first exact agreement.** AutoAttack's own printed robust accuracy against the
  number `src/ard/evaluation/autoattack.py:213` recomputes in one unbatched
  forward: `best.pt` **48.65 % printed vs 48.65 % recorded** — identical, 0
  images — and `last.pt` **44.81 % vs 44.83 %** (+2 images, +0.02 pp, recorded
  higher). The tally is now **12 disagreements out of 16 comparisons, all twelve
  in the same direction**, with the magnitude ceiling still **0.09 pp** from
  `mobilenetv2_pgd_at-s1`. Both figures are one to two orders of magnitude below
  every spread this entry reports, so no conclusion above turns on which
  estimator is used. **This postrun does not choose; packet 0010's `chosen` is
  still null.**

  **New for packet 0010: the exact evaluation that failed three times succeeded
  on the fourth, and the only thing that changed is what else was on the card.**
  Attempts 1–3 of `eval-e2da1d97fcd3e5932fab` are `canon-eval-lane-{D,M,O}.log`,
  each ending `exit=1` at `autoattack.py:213`; attempt 4 is
  `canon-eval-lane-T.log`, `03:31:42+09:00 → exit=0 04:35:54+09:00`. Wall clock
  `created_at 18:31:44.975Z → finished_at 19:35:54.040Z`, **1 h 04 m 09 s**, of
  which AutoAttack's own stage times account for **3,816.7 s — 99.2 %**.
  Per-stage seconds (best → last): APGD-CE **55.1 → 56.7**, APGD-T
  **346.1 → 309.6**, FAB-T **754.7 → 693.5**, SQUARE **834.2 → 766.8**; halves
  1,990.1 and 1,826.6 s. Dividing each stage by the number of inputs that
  actually entered it (best 8,282 / 5,278 / 4,866 / 4,865; last 8,507 / 4,735 /
  4,481 / 4,481) gives **6.65 / 65.6 / 155.1 / 171.5 ms** against **6.66 / 65.4 /
  154.8 / 171.1 ms** — the two halves agree to **0.3 % or better on all four
  stages**. This is the flattest cost profile this campaign has measured, and it
  matches the 17:26Z entry's *uncontended* ResNet-18 half (arm 5 seed 1: 3,895.2 s
  when a second ResNet-18 AutoAttack job shared the card, 1,922.1 s once that
  neighbour died) rather than its contended one. Read against packet 0010's
  16:57Z allocator arithmetic, **this is Option B demonstrated end to end on the
  very run that produced three of the ten failures**: change no code, give the job
  the memory, and it returns a result in about an hour.

  **A second AutoAttack evaluation overlapped this one in wall-clock time and
  neither was slowed, which says they were not sharing a card.**
  `eval-6b0db6d5fc98a962f227` (`mobilenetv2_adr-s0`, EMA weights) ran
  18:29:45Z → 19:35:32Z, **1 h 05 m 47 s**, inside a minute of the arm-8
  uncontended reference of ≈1 h 06 m 30 s established at 18:32Z; this run held its
  own uncontended rate throughout. Two concurrent AutoAttack evaluations are
  therefore not by themselves a hazard — the 17:26Z halving and the ten OOMs both
  involved jobs on **one** device. **This is a reconstruction from AutoAttack's
  cumulative timing marks and bundle timestamps, not from a process table**;
  neither `ps` nor an `nvidia-smi` history is available to this session.

  **Eighth observation of a scientific job launching while packet 0010 is
  `status: pending`, `chosen: null`.** `canon-eval-lane-U.log:1` starts
  `cifar10_r18_adr-s2`'s first model-weights evaluation at 04:36:18+09:00
  (19:36:18Z), **24 s** after this run freed its GPU, and
  `eval-533e04a8c5a3f4378434` (`r18_trades_adr-s1`, EMA) has also appeared since
  the 18:32Z census. **This postrun started nothing and stopped nothing**;
  stopping another session's jobs is the human's call.

  **An infrastructure defect this postrun observed and did not cause: the
  headless postrun path is dead.** `mobilenetv2_adr-s0`'s EMA evaluation reached
  terminal-successful at 19:35:32Z and the watcher fired its hook, but
  `orchestration/ardx/claude-runs/20260910T193559Z-eval-6b0db6d5fc98a962f227_evaluation-ema.log`
  contains exactly one line: *"Ignoring 44 permissions.allow entries from
  .claude/settings.json: this workspace has not been trusted."* CLAUDE.md already
  documents that trust is keyed on the exact repo-root spelling and that the
  `/home/shunsukenaito/...` symlink needs its own accepted entry; here it is the
  **`/home/islab/...`** spelling that is untrusted, which is the spelling the
  hook uses. Until someone accepts the trust dialog interactively at that path,
  **every headless postrun will fire, be denied every Bash call, and record
  nothing** — so terminal results will keep accumulating unrecorded. This is an
  execution-plane defect, not a scientific one, and it is not fixed here.

  **Nothing imported.** `docs/experiments/` holds no record for this contract and
  none can be written: the aggregator refuses on the first missing
  `evaluation-results.json`, and **4 of the 20 model-weights evaluations** and
  **4 of the 9** EMA evaluations are still non-terminal or absent. No
  evidence-ledger row is owed either, since a ledger row accompanies a record.

  **Census at 19:45Z, from a fresh-cursor watcher scan of the whole campaign root
  (49 bundle rows).** Training: **20 of 20** terminal and successful, unchanged.
  Model-weights contract evaluations: **16 of 20** terminal and successful (this
  run is the sixteenth); **0 terminal-failed**; **0 with no bundle at all** —
  `r18_adr-s2`'s first attempt exists now; **4 non-terminal** (`r18_adr-s2`
  running, `r18_trades-s1` running, `r18_trades-s2` `stale`, `r18_trades_adr-s2`
  running). EMA-weights evaluations: **5 of 9** terminal and successful
  (`r18_adr-s0/-s1`, `mobilenetv2_adr-s0/-s1/-s2`), **2 non-terminal**
  (`r18_trades_adr-s0/-s1`), **2 never started** (`r18_adr-s2`,
  `r18_trades_adr-s2`). **Arm 8 is now complete on both of its axes** (three
  model seeds, three EMA seeds), joining arms 2, 6 and 7 as complete; **three of
  its results are still unrecorded and owe postruns** (`mobilenetv2_adr-s1` model,
  `mobilenetv2_adr-s2` EMA, `mobilenetv2_adr-s0` EMA), so any record that uses
  arm 8's EMA axis must wait for them. M1c's training half is closed and its
  evaluation half is not, so **M1c stays unticked**; M2 and M3 stay open.

  **One housekeeping fact.** The working tree carried an **uncommitted** addition
  to `docs/decisions/0010-autoattack-unbatched-forward-oom.md` — the 18:32Z
  entry's packet addendum, whose plan half was committed but whose packet half
  was not, the same two-session race the 17:26Z entry describes. It is committed
  here alongside this entry's own addendum rather than left dangling.

  **Nothing imported, and the aggregator would still refuse.** Read against
  `scripts/aggregate_adr_cifar10_replication.py`, this bundle passes every gate
  the aggregator applies to an EMA row — `count == 10000`,
  `dataset_identity.split == "test"`, `runtime_method == "adr"`,
  `weights == "ema"`, an `autoattack` dict present on both rows,
  `attack_version == "standard"`, `expected_commit == a39220048b3c…`, and both
  `ALIASES` resolvable once `best-ema` is normalized to `best` — so nothing about
  this run blocks M3. What blocks M3 is completeness: `_load_arm_seed` raises on
  the first missing `evaluation-results.json`, and **5 of the 20 model-weights
  evaluations** (`r18_adr-s1` non-terminal, `r18_adr-s2` never started,
  `r18_trades-s1`, `r18_trades-s2`, `r18_trades_adr-s2`) and **5 of the 9 EMA
  evaluations** (`mobilenetv2_adr-s0` and `r18_trades_adr-s0` non-terminal;
  `r18_adr-s2`, `r18_trades_adr-s1`, `r18_trades_adr-s2` never started) are still
  absent. No record, no report, no evidence-ledger row — a ledger row accompanies
  a record.

  **Census at 18:33Z, from a full-root watcher scan (48 bundle rows, fresh
  cursor).** Training: **20 of 20 terminal and successful**, unchanged.
  Model-weights contract evaluations: **15 of 20** terminal and successful
  (the 17:27Z fourteen plus `mobilenetv2_adr-s0`, which completed at 18:29:42Z;
  it owed a postrun when this census was written and is recorded in the 18:32Z
  entry, appended concurrently); **0 terminal-failed** (was 1 — see above);
  **4 non-terminal** (`r18_adr-s1` on its fourth attempt, `r18_trades-s1`,
  `r18_trades-s2` reading `stale`, `r18_trades_adr-s2`); **1 with no bundle at
  all** (`r18_adr-s2`). EMA-weights evaluations: **4 of 9** terminal and
  successful (`r18_adr-s0`, `r18_adr-s1`, `mobilenetv2_adr-s1` — this run — and
  `mobilenetv2_adr-s2`); **2 non-terminal** (`mobilenetv2_adr-s0`,
  `r18_trades_adr-s0`); **3 with no bundle at all** (`r18_adr-s2`,
  `r18_trades_adr-s1`, `r18_trades_adr-s2`). **Two terminal successful results
  are unrecorded and owe postruns**: `mobilenetv2_adr-s1` model
  (`eval-4615ede05d4f96adb48f`) and `mobilenetv2_adr-s2` EMA
  (`eval-b89d6f7f0682d8cb3248`) — both on arm 8, and the first of them is
  this run's own seed-matched student-weights counterpart, so it should be run
  next. (`mobilenetv2_adr-s0` model, `eval-750b3422e05de8434c22`, was listed here
  as a third; the 18:32Z entry recorded it concurrently with this one.)
  Arms 2, 6 and 7 remain complete on every axis their contracts define.
  M1c's training half is closed and its evaluation half is not, so M1c stays
  unticked; M2 and M3 stay open.

- 2026-09-10 18:32Z: postrun of the `eval-750b3422e05de8434c22` terminal event —
  **arm 8 (`mobilenetv2_adr`) seed 0, model weights, both checkpoints, official
  CIFAR-10 test set. With this run arm 8's model-weights axis is complete at all
  three seeds, and the MobileNetV2 leg of the preregistered primary comparison
  has all three matched pairs on the official test.** That is the first time
  either leg of the rule has had more than one pair. **Nothing imported, no
  milestone closed, and this is still not a verdict** — the ResNet-18 leg has one
  pair of three. (This entry is timestamped before the 18:33Z one above but
  appears after it: two sessions were appending to this plan at the same time,
  and this one wrote second. Both censuses are reconciled against each other.)

  **Terminal status re-derived, not taken from the event.** The watcher line whose
  `path` is the bundle manifest passed in reads `completed / success=true /
  terminal=true / failure_class=null`, from `completion.json` present,
  `manifest.status: sync_pending`, and `error-marker.txt` = `no application error
  recorded`. Watcher scan of the whole campaign root at 18:32:37Z, fresh cursor,
  48 bundle rows.

  **This run's numbers** (`weights=model`, official 10,000-image CIFAR-10 test
  split, ε `8/255`, CE-PGD-20 at 20 steps, AutoAttack standard, percent):

  | checkpoint | clean | CE-PGD-20 | AutoAttack |
  |---|---|---|---|
  | `best.pt` | 75.64 | 48.36 | **42.71** |
  | `last.pt` | 75.93 | 48.02 | **42.50** |

  **Contract identity verified against the frozen arm-8 contract.** Protocol
  `controlled_cifar10_mobilenetv2_adr_v1`, `runtime_method: adr`, `teacher: null`,
  student `mobilenet_v2_cifar` with `pixel_0_1` input domain and the
  `cifar10_standard` profile owned by the student adapter, `count: 10000` with
  `dataset_identity.split: test` and content fingerprint `c58f3010…` on both rows,
  evaluation attack CE ε `8/255` step `2/255` 20 steps random start
  `temperature_squared: true`, training attack KL/`rectified` at 10 steps,
  `adr` block `ema_decay 0.995 / T_high 2.5 / T_low 2.0 / λ_low 0.7 / λ_high
  0.95`, `trades_beta 6.0`, `student_ema_decay 0.9`, `oracle_mask false`,
  `frozen_oracle_manifest null`, SGD `lr 0.1 / momentum 0.9 / wd 5e-4 / nesterov
  true`, multistep `[100, 150] gamma 0.1`, `epochs 200`,
  `validation_fraction 0.1`, `batchnorm_mode local_per_rank`, world size 1 and
  per-rank = global batch 128, `deterministic: true`, all five `ARD_SEED` seeds
  **0** with the fixed `split: 20260722`, and `weights: model` on both rows with
  `checkpoint_filename` `best.pt` / `last.pt` — so no `best-ema.pt` leaked onto
  the model axis. AutoAttack `attack_version: standard` with
  `provenance.expected_commit` = `vcs_commit` = `a39220048b3c…`, the pinned
  upstream, on both rows.

  **Lineage verified.** Source SHA `cd0b571e4685…`, `dirty: false`, `diff_sha256`
  equal to the SHA-256 of the empty string. `external.lock` `05cfce4c…` —
  byte-identical to the hash every other bundle in this campaign carries. All
  seven declared artifacts exist both at their live path and as content-addressed
  copies under `run-bundle/artifacts/<name>/<sha256>/`, and each copy's directory
  name equals the hash the manifest declares. The chain to training closes:
  `evaluation-lineage.json`'s `training_config_hash` `e54ea87d…` equals the
  training bundle's own `config_hash`, `training_config_migration.applied` is
  empty with `runtime_method_id` = `source_method_id` = `adr`, and
  `train_run_id` = `adr-campaign-v1-ferret-cifar10_mobilenetv2_adr-s0`, the
  training run the watcher reports terminal and successful at 17:19:02Z.
  **One check could not be run**, as in every entry since 15:12Z: the seven
  artifacts' and the two checkpoints' SHA-256 were **not independently
  recomputed** — this session's sandbox refuses to hash files outside the
  repository root. They are asserted by the producer and internally consistent,
  not confirmed here.

  **Attack-quality sanity, all six checks pass.** AutoAttack ≤ CE-PGD-20 ≤ clean
  on both rows (42.71 ≤ 48.36 ≤ 75.64; 42.50 ≤ 48.02 ≤ 75.93). Best-checkpoint
  AutoAttack sits 0.21 pp *above* last, the expected robust-overfitting
  direction, while best-checkpoint clean sits 0.29 pp *below* its own last
  checkpoint — which is what selecting on robustness rather than clean accuracy
  produces. `canon-eval-lane-R.log` reports `max Linf perturbation: 0.03137`
  (= 8/255 exactly), `nan in tensor: 0`, and `max: 1.00000, min: 0.00000`, i.e.
  the adversarial batch stayed inside the pixel domain. The per-stage sequence is
  the normal shape for a robust model on both checkpoints — APGD-CE 47.44 →
  APGD-T 42.67 → FAB-T 42.67 → SQUARE 42.67 for `best.pt` and 47.21 → 42.48 →
  42.48 → 42.48 for `last.pt`, i.e. APGD-T does all the work after APGD-CE and
  neither FAB-T nor Square finds anything further. The arm sits 6.04 pp below the
  ResNet-18 ADR seed-0 figure (42.71 against 48.75), consistent with the ≈6–9 pp
  architecture gap the earlier entries measured. **No deviation and no bug
  found.**

  **The MobileNetV2 leg of the preregistered rule, now at three seeds.**
  Best checkpoint, model weights, official-test AutoAttack, percent:

  | seed | arm 8 `mobilenetv2_adr` | arm 7 `mobilenetv2_pgd_at` | gain |
  |---|---|---|---|
  | 0 | 42.71 | 39.73 | **+2.98** |
  | 1 | 42.88 | 39.86 | **+3.02** |
  | 2 | 42.56 | 40.18 | **+2.38** |
  | mean | 42.72 | 39.92 | **+2.79** |
  | sd | 0.16 | 0.23 | 0.36 |

  ADR beats its matched plain-AT baseline **in all three seed pairs**, mean
  **+2.79 pp** (sd 0.36, range +2.38..+3.02). Against the noise floor that is
  actually measured, this is not close: +2.79 pp is 6.2× arm 7's own three-seed
  spread (0.45 pp) and 8.7× arm 8's (0.32 pp), and ≈7.0× the ≈0.40 pp binomial
  sampling error of a mean of three paired differences (≈0.49 pp per 10,000-image
  run at p≈0.40–0.43, ≈0.70 pp per pair, ÷√3). The same ordering holds on the
  last checkpoint (+3.27 / +3.52 / +2.57, mean **+3.12 pp**, 42.45 against
  39.33).

  **Clean accuracy moves the same way, which is worth flagging.** Best-checkpoint
  clean is 75.79 % for arm 8 against 73.78 % for arm 7, a **+2.01 pp** mean gain
  (+1.86 / +1.64 / +2.52 per seed), and CE-PGD-20 gains **+4.18 pp** (48.37
  against 44.18). So on MobileNetV2 ADR buys robustness **without** paying clean
  accuracy for it, in all three seeds. Clean and robust are reported separately
  here and must stay separate in any table; this is a directional statement about
  these six runs.

  **One honest correction to how this looks against the 16:16Z snapshot.** That
  entry's single MobileNetV2 pair was seed 2 at **+2.38 pp**, and the three-seed
  mean is **+2.79 pp**. The effect did not grow: seed 2 happens to be arm 8's
  *weakest* seed and arm 7's *strongest*, so the one pair that existed then was
  the least favourable of the three. Nothing about the underlying quantity
  changed between the two entries; the sample did.

  **What the preregistered rule says now, and why it is still not a verdict.**

  | leg | treatment | baseline | gain | seeds |
  |---|---|---|---|---|
  | MobileNetV2 (arm 8 − arm 7) | 42.72 | 39.92 | **+2.79** | 3 of 3 |
  | ResNet-18 (arm 3 − arm 2, seed 0) | 48.75 | 47.19 | **+1.56** | 1 of 3 |

  MobileNetV2's mean gain is positive and exceeds ResNet-18's, which is the
  rule's "sign confirmed" branch, and the two now differ by **+1.23 pp** against
  ≈**0.81 pp** of binomial sampling error on that difference — where the 16:16Z
  snapshot had 0.82 pp against ≈0.99 pp, i.e. inside the error. **That is a real
  improvement and it is still not the verdict**, for one reason that no
  arithmetic here can fix: **arm 3's seed spread remains entirely unmeasured.**
  Arm 3 has one model-weights seed of three (`-s1` is on its fourth attempt,
  `-s2` has never been evaluated), so the ResNet-18 leg is a single draw, and the
  rule's "do not force a verdict past what three seeds support" clause applies to
  it exactly as before. What is licensed today is a directional statement: ADR
  beats matched plain-AT on MobileNetV2 in all three seeds and on ResNet-18 in
  the one pair that exists, by more on MobileNetV2. **No population claim, and
  packet 0009's question stays open.**

  **The paired analysis the 16:16Z entry proposed is now worth more, and is still
  not run here.** All six MobileNetV2 evaluations scored the *same* 10,000 images
  and `sample-stats-{best,last}.parquet` holds per-example outcomes for each, so
  McNemar over the arm 8 / arm 7 disagreement counts would remove the shared
  finite-set error instead of double-counting it, across three pairs rather than
  one. It is CPU-only work on artifacts already on disk. **This postrun did not
  do it and does not choose it**; it is an option for packet 0009.

  **Arm 8's EMA axis, orientation only.** `eval-74a8f5d0042ebea0b0d1`
  (`mobilenetv2_adr-s1`, `evaluation-ema/`) turned terminal and successful at
  **18:30:53Z**, two minutes into this postrun, and `mobilenetv2_adr-s0`'s own EMA
  evaluation was created at **18:29:45Z** and is running — so arm 8 has 2 of 3
  EMA seeds terminal with the third in flight. Seed 1's EMA bundle was verified
  and recorded by the concurrent 18:33Z entry above; seed 2's still owes a
  postrun. **Neither was verified by this postrun**; reading their results files
  for orientation only, `best-ema.pt` at `weights=ema` gives 43.34 (s1) and 42.91
  (s2) against the same seeds' model-weights 42.88 and 42.56, i.e. EMA is above
  model weights by +0.46 and +0.35 pp — two seeds, both differences smaller than
  arm 7's own 0.45 pp seed spread. **These numbers are not recorded as results
  here** and the "ADR + WA" comparator is not formed in this entry.

  **Cost, and a datum for packet 0010.** `created_at 17:22:55.823Z` →
  `finished_at 18:29:42.603Z` = **1 h 06 m 46.8 s**, of which AutoAttack's own
  reported stage times account for 3,968.4 s (1 h 06 m 08 s) — **99.0 %** of the
  wall clock. The per-stage times are near-identical across the two checkpoints
  (63.4 / 441.6 / 1122.1 / 1986.2 s against 63.7 / 432.4 / 1117.9 / 1982.2 s,
  worst deviation 2.1 %), so this run had the card to itself throughout — the
  opposite of the 17:26Z entry's arm-5 seed 1, where the last checkpoint ran
  1.54–2.07× faster than the best because a co-resident job died in between.
  Against arm 8 seed 2's 1 h 06 m 42 s and seed 1's EMA 1 h 06 m 25.6 s, this
  arm's AutoAttack cost is stable at **≈1 h 06 m 30 s ± 20 s when uncontended**.

  **The packet-0010 estimator tally gains two more comparisons and its ceiling
  still does not move.** AutoAttack's own printed robust accuracy against the
  number `src/ard/evaluation/autoattack.py:213` recomputes in one unbatched
  forward: `best.pt` **42.67 % printed vs 42.71 % recorded** (+4 examples,
  +0.04 pp) and `last.pt` **42.48 % vs 42.50 %** (+2 examples, +0.02 pp). The
  tally is now **11 disagreements out of 14 comparisons, all eleven in the same
  direction (the recorded number is the higher one)**, with the magnitude ceiling
  still **0.09 pp** from `mobilenetv2_pgd_at-s1`. Both figures here are far below
  every gap this entry reports, so no conclusion above turns on which estimator
  is used. **This postrun does not choose; packet 0010's `chosen` is still null.**

  **Two operational facts this postrun observed and did not cause.** First, this
  run **is** the launch the 17:27Z entry flagged as its "sixth observation of a
  launch arriving while packet 0010 is `chosen: null`", on "exactly the AutoAttack
  path that has now failed ten times under contention" — and it completed cleanly
  on its first attempt. Arm 8, the arm that opened packet 0010, is now **5 for 5**
  on terminal-successful evaluations (three model, two EMA). Second, **122 s
  after this job freed its GPU, a fourth `r18_adr-s1` model-weights attempt was
  created** at 18:31:44.976Z (`eval-e2da1d97fcd3e5932fab`) at the same path under
  the same id, **overwriting the terminal-failed bundle the 17:27Z entry called
  packet 0010's only surviving exhibit**. That directory now holds only the new
  attempt's in-flight files, with no `completion.json` and no `error-marker.txt`;
  the eighth failure directory is gone. The OOM tracebacks themselves do survive,
  in nine `canon-eval-lane-*.log` files. Recorded in packet 0010; **this postrun
  started nothing and stopped nothing.**

  **Nothing imported.** `docs/experiments/` holds no record for this contract and
  none can be written: the aggregator refuses on the first missing
  `evaluation-results.json` and **5 of the 20 model-weights evaluations** are
  absent or non-terminal, as are **5 of the 9** EMA evaluations. No
  evidence-ledger row is owed either, since a ledger row accompanies a record.

  **Census at 18:32:37Z, from a full-root watcher scan (48 bundle rows, fresh
  cursor).** Training: **20 of 20 terminal and successful**, unchanged.
  Model-weights contract evaluations: **15 of 20** terminal and successful, one
  more than the 17:27Z entry (this run); **0 terminal-failed** — `r18_adr-s1`,
  the last one, was overwritten at 18:31:44Z and now reads `running`; **4
  non-terminal** (`r18_adr-s1` attempt 4, `r18_trades-s1`, `r18_trades-s2`
  reading `stale`, `r18_trades_adr-s2`); **1 with no bundle at all**
  (`r18_adr-s2`). EMA-weights evaluations: **4 of 9** terminal and successful
  (`r18_adr-s0`, `r18_adr-s1`, `mobilenetv2_adr-s1`, `mobilenetv2_adr-s2`), **2
  non-terminal** (`mobilenetv2_adr-s0` created 18:29:45Z, `r18_trades_adr-s0`),
  **3 with no bundle at all** (`r18_adr-s2`, `r18_trades_adr-s1`,
  `r18_trades_adr-s2`). **Two terminal successful results are unrecorded and owe
  postruns**: `mobilenetv2_adr-s1` model and `mobilenetv2_adr-s2` EMA
  (`mobilenetv2_adr-s1` EMA was the third when this scan ran; the concurrent
  18:33Z entry recorded it) — note the arm-8 three-seed table above therefore
  rests on one fully verified run (this one), one previously recorded run (seed 2,
  16:16Z) and one whose contract fields were spot-checked here but whose full
  lineage has not been verified (seed 1), so seed 1's postrun is owed before any
  record uses it. Arms 2, 6 and 7 remain complete on every axis their contracts
  define; **arm 8 joins them on its model axis** and still owes one EMA seed.
  M1c's training half is closed and its evaluation half is not, so M1c stays
  unticked; M2 and M3 stay open.

- 2026-09-10 20:47Z: postrun of the `eval-5709dc2e3701675af39e` terminal event —
  **arm 3 (`cifar10_r18_adr`) seed 2, model weights, both checkpoints, official
  CIFAR-10 test set. Arm 3's model-weights axis is now complete at all three
  seeds, and for the first time the preregistered primary comparison has three
  matched seed pairs on _both_ legs.** The rule's arithmetic is reported below
  and it reads on the "sign confirmed" branch. **Nothing is imported, no
  milestone is closed, and this postrun does not apply the rule** — that is M3's
  deliverable and the human's decision, and three model-weights plus four EMA
  evaluations are still outstanding. One caveat found here cuts against the
  simplest reading of the result and is stated in full below.

  **Terminal status re-derived, not taken from the event.** The watcher line
  whose `path` is the bundle manifest passed in reads `completed /
  success=true / terminal=true / failure_class=null`, from `completion.json`
  present (`{"status": "completed", "results": 2}`), `manifest.status:
  sync_pending`, and `error-marker.txt` = `no application error recorded`.
  Full-root watcher scan at 20:46:41Z, fresh cursor, 51 bundle rows.
  ...
  **This run's numbers** (`weights=model`, official 10,000-image CIFAR-10 test
  ...
  | checkpoint | clean | CE-PGD-20 | AutoAttack |
  |---|---|---|---|
  | `best.pt` | 83.32 | 53.35 | **48.22** |
  | `last.pt` | 85.29 | 48.66 | **44.84** |

  **Contract identity verified against the frozen arm-3 contract.** Protocol
  `controlled_cifar10_r18_adr_v1`, `runtime_method: adr`, `teacher: null`,
  student `saad_resnet18_cifar_v1` with `pixel_0_1` input domain and the
  `cifar10_raw_identity` profile owned by the student adapter, `count: 10000`
  with `dataset_identity.split: test` and content fingerprint `c58f3010…` on
  both rows, evaluation attack CE ε `8/255` step `2/255` 20 steps random start
  `temperature_squared: true` (`threat_hash` `7081101693340e70…`, byte-equal to
  arm 2's, which is what makes the pair below a like-for-like comparison),
  training attack KL/`rectified` at 10 steps, `adr` block `ema_decay 0.995 /
  T_high 2.5 / T_low 2.0 / λ_low 0.7 / λ_high 0.95`, `trades_beta 6.0`,
  `student_ema_decay 0.9`, `oracle_mask false`, `frozen_oracle_manifest null`,
  SGD `lr 0.1 / momentum 0.9 / wd 5e-4 / nesterov true`, multistep `[100, 150]
  gamma 0.1`, `epochs 200`, `validation_fraction 0.1`, `batchnorm_mode
  local_per_rank`, world size 1 and per-rank = global batch 128,
  `deterministic: true`, all five `ARD_SEED` seeds **2** with the fixed `split:
  20260722`, and `weights: model` on both rows with `checkpoint_filename`
  `best.pt` / `last.pt` — so no `best-ema.pt` leaked onto the model axis.
  AutoAttack `attack_version: standard` with `provenance.expected_commit` =
  `vcs_commit` = `a39220048b3c…`, the pinned upstream, on both rows.

  **Lineage verified.** Source SHA `cd0b571e4685…`, `dirty: false`,
  `diff_sha256` equal to the SHA-256 of the empty string. `external.lock`
  `05cfce4c…` — byte-identical to the hash every other bundle in this campaign
  carries, with all five upstream pins at their contracted commits (`adr`
  `515da0e0…`, `trades` `6e8e11b7…`, `robustbench` `78fcc9e4…`, `saad`
  `295121c5…`, `da_alone_improves_at` `38b740ae…`). All seven declared
  artifacts exist both at their live path and as content-addressed copies under
  `run-bundle/artifacts/<name>/<sha256>/`, and each copy's directory name
  equals the hash the manifest declares. The chain to training closes:
  `evaluation-lineage.json`'s `training_config_hash` `c0a28b33…` equals the
  training bundle's own `config_hash`, `training_config_migration.applied` is
  empty with `runtime_method_id` = `source_method_id` = `adr`, and
  `train_run_id` = `adr-campaign-v1-ferret-cifar10_r18_adr-s2`, the training run
  the watcher reports terminal and successful. **One check could not be run**,
  as in every entry since 15:12Z: the seven artifacts' and the two checkpoints'
  SHA-256 were **not independently recomputed** — this session's sandbox refuses
  to hash files outside the repository root. They are asserted by the producer
  and internally consistent, not confirmed here.

  **Attack-quality sanity, all six checks pass.** AutoAttack ≤ CE-PGD-20 ≤ clean
  on both rows (48.22 ≤ 53.35 ≤ 83.32; 44.84 ≤ 48.66 ≤ 85.29). Best-checkpoint
  AutoAttack sits 3.38 pp *above* last, the expected robust-overfitting
  direction, while best-checkpoint clean sits 1.97 pp *below* its own last
  checkpoint — what selecting on robustness rather than clean accuracy produces.
  `canon-eval-lane-U.log:202,401` reports `max Linf perturbation: 0.03137`
  (= 8/255 exactly), `nan in tensor: 0`, and `max: 1.00000, min: 0.00000` on
  both checkpoints, i.e. the adversarial batch stayed inside the pixel domain.
  The per-stage sequence is the normal shape for a robust model — APGD-CE 52.57
  → APGD-T 48.22 → FAB-T 48.22 → SQUARE 48.22 for `best.pt`, and 47.65 → 44.82 →
  44.82 → 44.82 for `last.pt`: APGD-T does all the work after APGD-CE and
  neither FAB-T nor Square finds anything further. **No deviation and no bug
  found.**

  **The preregistered primary comparison, complete on both legs for the first
  time.** Best checkpoint, model weights, official-test AutoAttack, percent, all
  twelve numbers re-read from the twelve `evaluation-results.json` files rather
  than copied from earlier entries:

  | seed | arm 3 `r18_adr` | arm 2 `r18_pgd_at_nesterov` | gain |
  |---|---|---|---|
  | 0 | 48.75 | 47.19 | **+1.56** |
  | 1 | 48.65 | 47.41 | **+1.24** |
  | 2 | 48.22 | 47.37 | **+0.85** |
  | mean | 48.54 | 47.32 | **+1.22** |
  | sd | 0.28 | 0.12 | 0.36 |
  ...
  ADR beats its matched plain-AT baseline in **all six seed pairs**, and the
  MobileNetV2 mean gain exceeds the ResNet-18 one by **+1.58 pp**. Read against
  the rule as preregistered: MobileNetV2's gain is positive and exceeds
  ResNet-18's, which is the **"sign confirmed"** branch; the "mixed /
  inconclusive" branch requires within-arm seed spread to exceed the measured
  gap, and the largest within-arm spread anywhere in the four arms is arm 3's
  0.53 pp against a 1.58 pp leg difference, so that branch does not fire. The
  ratio to the noise floor has grown monotonically as pairs were added — 0.8×
  at 16:16Z, 1.5× at 18:32Z, 2.2× at 19:45Z, **2.8× now** (1.58 pp against
  ≈0.57 pp of binomial sampling error on the difference of two three-pair
  means: ≈0.50 pp per 10,000-image run at p ≈ 0.40–0.49, ≈0.70 pp per seed
  pair, ÷√3 per leg, then combined). Taking the seed as the unit instead, each
  leg's three gains have sd 0.36 pp, so the leg difference carries ≈0.29 pp of
  seed-to-seed error and the ratio is ≈5.4×. **Neither number is a confidence
  interval**: the first ignores seed variance entirely, the second estimates it
  from three points per leg. **This is a directional statement about these
  twelve runs, at these seeds, at this protocol, and packet 0009's question
  stays formally open until M3.**

  **The caveat, and it is a real one: on the last checkpoint the leg difference
  reverses sign.** Same twelve runs, `last.pt` instead of `best.pt`:

  | leg | ADR arm | plain-AT arm | gain |
  |---|---|---|---|
  | ResNet-18 (arm 3 − arm 2) | 44.83 | 40.60 | **+4.23** |
  | MobileNetV2 (arm 8 − arm 7) | 42.45 | 39.33 | **+3.12** |

  On the last checkpoint ResNet-18's gain **exceeds** MobileNetV2's by 1.11 pp,
  the opposite ordering to the best checkpoint's +1.58 pp. The mechanism is
  visible in the same table: best-minus-last AutoAttack is **6.72 pp** for arm 2
  and **3.71 pp** for arm 3 (ResNet-18 baselines overfit robustly and ADR
  suppresses about half of it), but only **0.59 pp** for arm 7 and **0.27 pp**
  for arm 8 — MobileNetV2 barely robustly overfits at all, in either arm, so
  there is nothing there for ADR to suppress. So a large part of ADR's
  *best-checkpoint* ResNet-18 gain is spent closing a gap that MobileNetV2 does
  not have, and the "ADR helps the smaller model more" reading holds on the
  preregistered quantity but **not** on the last checkpoint. **The preregistered
  quantity is the best checkpoint and this entry does not change it**; the
  reversal is recorded because M3's report must state both and because a
  capacity claim that rests only on the best checkpoint is weaker than it looks.

  **Robust-overfitting suppression replicates at all three seeds.** Arm 3's
  best-minus-last AutoAttack is 3.94 / 3.82 / **3.38** pp at seeds 0 / 1 / 2
  against arm 2's 6.62 / 7.00 / 6.55 pp — every ADR seed below every baseline
  seed, no overlap. Directional for these six runs.

  **Arm 3's seed spread turned out five times wider than two seeds suggested,
  exactly as the 19:45Z entry warned.** That entry measured a 0.10 pp model-axis
  spread from seeds 0 and 1 and called sampling error "a floor, not a confidence
  interval"; seed 2 at 48.22 makes the three-seed spread **0.53 pp**. Two
  further observations. First, the spread is almost entirely in *checkpoint
  selection*: arm 3's `last.pt` AutoAttack is 44.81 / 44.83 / 44.84 — a **0.03
  pp** spread across three independently seeded training runs — while its
  `best.pt` spread is 0.53 pp, so the three models end up in nearly the same
  place and the held-out CE-PGD-20 selection of *which* epoch to keep is what
  varies. Second, arm 3's clean accuracy rises monotonically with seed (82.58 /
  82.82 / 83.32) while its AutoAttack falls (48.75 / 48.65 / 48.22), i.e. the
  selected epochs sit at different points of the same clean-robust tradeoff.
  Neither observation is a defect; both belong in M3's report as measured seed
  behaviour, and neither is claimed to generalize beyond arm 3.

  **Secondary quantities, reported and not decisive.** Best-checkpoint clean
  accuracy gains are **+0.51 pp** on ResNet-18 (82.91 against 82.39) and
  **+2.01 pp** on MobileNetV2 (75.79 against 73.78); best-checkpoint CE-PGD-20
  gains are **+2.46 pp** (53.34 against 50.88) and **+4.18 pp** (48.37 against
  44.18). So on both legs ADR buys robustness without paying clean accuracy for
  it, and on MobileNetV2 it gains clean accuracy outright. Clean and robust are
  reported separately here and must stay separate in any table.

  **Cost, and a datum for packet 0010.** `created_at 19:36:21.081Z` →
  `finished_at 20:39:10.159Z` = **1 h 02 m 49.1 s**, `exit=0`, **first
  attempt** — the seed-2 sibling of the run that needed four. AutoAttack's own
  reported stage times account for 3,737.0 s of that, **99.2 %** of wall clock.
  Dividing each stage by the number of inputs that actually entered it gives
  **6.54 / 64.4 / 150.8 / 166.9 ms** for `best.pt` against **6.53 / 65.0 /
  153.3 / 169.5 ms** for `last.pt` — agreement within **1.7 % on all four
  stages**, so this run had its card to itself throughout. Arm 3's uncontended
  AutoAttack cost is now measured twice at **≈1 h 03 m ± 40 s** (seed 1
  1 h 04 m 09 s, seed 2 1 h 02 m 49 s). **This is a reconstruction from
  AutoAttack's cumulative timing marks and bundle timestamps, not a process
  table**; neither `ps` nor an `nvidia-smi` history is available to this session.

  **A second concurrent-AutoAttack observation, and it agrees with the first.**
  `canon-eval-lane-V.log` started `r18_trades_adr-s1`'s EMA evaluation at
  **19:36:18Z — three seconds before this run** — and finished it `exit=0` at
  **20:42:32Z**, 1 h 06 m 14 s. The two overlapped almost exactly and **neither
  was slowed**: this run held its uncontended rate to 1.7 % and lane V's total
  is within a minute of the arm-5/arm-3 uncontended band. That is the second
  independent instance of the 19:45Z conclusion — **two AutoAttack evaluations
  at once are not by themselves a hazard; two on one device are.**

  **The packet-0010 estimator tally gains two comparisons and its ceiling still
  does not move.** AutoAttack's own printed robust accuracy against the number
  `src/ard/evaluation/autoattack.py:213` recomputes in one unbatched forward:
  `best.pt` **48.22 % printed vs 48.22 % recorded** (exact agreement, the second
  one this campaign has produced) and `last.pt` **44.82 % vs 44.84 %**
  (+2 examples, +0.02 pp). The tally is now **13 disagreements out of 18
  comparisons, all thirteen in the same direction** (the recorded number is the
  higher one), with the magnitude ceiling still **0.09 pp**. Both figures here
  are far below every gap this entry reports, so no conclusion above turns on
  which estimator is used. **This postrun does not choose; packet 0010's
  `chosen` is still null.**

  **Three operational facts this postrun observed and did not cause.** First,
  **the ninth and tenth launches while packet 0010 is `status: pending`,
  `chosen: null`**: `canon-eval-lane-W.log:1` starts `r18_adr-s2`'s EMA
  evaluation at 20:40:11Z, **60 s** after this run freed its GPU, and
  `canon-eval-lane-X.log` restarts `r18_trades-s1`'s model evaluation at
  20:43:18Z. Second, **the evidence-overwrite pattern recurred on that restart**:
  lane X's first try at 20:42:59Z died `exit=1` with `FileExistsError: refusing
  to overwrite existing evaluation output`, and its second try 16 s later
  succeeded — so the 11:25:07Z lane-A attempt's output directory was deleted in
  between and that attempt's files are gone. Third, **the headless postrun path
  is still dead**, unchanged from 19:45Z: both
  `orchestration/ardx/claude-runs/20260910T203921Z-eval-5709dc2e…_evaluation.log`
  (this run) and `…20260910T204254Z-eval-533e04a8…_evaluation-ema.log`
  (`r18_trades_adr-s1` EMA) contain exactly one line, *"Ignoring 44
  permissions.allow entries … this workspace has not been trusted"*, for the
  `/home/islab/...` spelling the hook uses. The fix is
  `projects["/home/islab/workspace-local/shunsuke.naito/ard_codex_bootstrap"].hasTrustDialogAccepted:
  true` in `/home/shunsukenaito/.claude.json`, or accepting the dialog
  interactively at that exact path once; **it is a user-level config change and
  this postrun did not make it.** Until then every terminal result will keep
  needing a hand-run postrun. **This postrun started nothing and stopped
  nothing**; stopping or starting another session's jobs is the human's call.

  **Nothing imported.** `docs/experiments/adr_cifar10_replication_v1.json` does
  not exist and none can be written: `_load_arm_seed` raises on the first
  missing `evaluation-results.json`, and **3 of the 20 model-weights
  evaluations** and **3 of the 9 EMA evaluations** are still non-terminal or
  absent. Read against `scripts/aggregate_adr_cifar10_replication.py`, this
  bundle passes every gate the aggregator applies — `completion.json` present,
  `manifest.status` in `{completed, sync_pending}`, the exact error marker,
  `protocol_id` = `controlled_cifar10_r18_adr_v1`, `git.dirty` false, all seven
  declared artifacts present at their live paths, `count == 10000`,
  `dataset_identity.split == "test"`, `runtime_method == "adr"`, `weights ==
  "model"`, an `autoattack` dict on both rows, `attack_version == "standard"`,
  `expected_commit == a39220048b3c…`, and both `ALIASES` resolvable — so
  nothing about this run blocks M3. No evidence-ledger row is owed either, since
  a ledger row accompanies a record. **No decision packet is opened here**: this
  campaign's primary packet is M3's deliverable and is gated on the record, and
  packet 0010 receives its addendum instead.

  **What is left, and it is small.** The primary comparison needs nothing
  further — arms 2, 3, 7 and 8 are complete on the model axis. The record needs
  seven more results: model weights for `r18_trades-s1` (restarted 20:43:20Z,
  running), `r18_trades-s2` and `r18_trades_adr-s2` (running); EMA for
  `r18_adr-s2` (started 20:40:11Z, running), `r18_trades_adr-s0` (running) and
  `r18_trades_adr-s2` (never started). **`r18_trades-s2`'s evaluation is the one
  with no live job**: it reads `stale`, its last progress was 12:11:52Z, and
  `canon-eval-lane-B.log:632` shows it SIGTERM'd `exit=143` at 12:54:04Z
  mid-Square — the fourth instance of the SIGTERM defect, already recorded at
  12:54Z. Nothing has restarted it in the eight hours since. **This postrun does
  not restart it**; that is a launch, and launches are not this skill's to make.

  **Census at 20:46:41Z, from a full-root watcher scan (51 bundle rows, fresh
  cursor).** Training: **20 of 20** terminal and successful, unchanged.
  Model-weights contract evaluations: **17 of 20** terminal and successful (this
  run is the seventeenth); **0 terminal-failed**; **3 non-terminal**
  (`r18_trades-s1` running on a fresh attempt, `r18_trades-s2` `stale` and dead,
  `r18_trades_adr-s2` running). EMA-weights evaluations: **6 of 9** terminal and
  successful (`r18_adr-s0/-s1`, `mobilenetv2_adr-s0/-s1/-s2`, and
  `r18_trades_adr-s1`, which turned terminal at 20:42:32Z); **2 non-terminal**
  (`r18_adr-s2`, `r18_trades_adr-s0`); **1 never started**
  (`r18_trades_adr-s2`). The remaining 3 rows are the 2026-09-09
  `early-check-eval-*` diagnostics under `r18_adr-s1`, which are not contract
  evaluations. **One terminal successful result other than this one is
  unrecorded and owes a postrun**: `r18_trades_adr-s1` EMA
  (`eval-533e04a8c5a3f4378434`) — its headless postrun fired at 20:42:54Z and
  recorded nothing, per the trust defect above. **Arm 3 joins arms 2, 6, 7 and 8
  as complete on its model axis**; on its EMA axis it has two seeds of three
  with the third running. M1c's training half is closed and its evaluation half
  is not, so **M1c stays unticked**; M2 and M3 stay open.
- 2026-09-10 21:48Z: postrun of the `eval-669190483234c84c27f1` terminal event —
  **arm 3 (`cifar10_r18_adr`) seed 2, EMA weights, both checkpoints, official
  CIFAR-10 test set. Arm 3 is now complete on both of its axes, so the EMA axis
  is complete on both ADR arms (3 and 8) and the campaign's only remaining
  evaluations are on the TRADES legs.** The EMA axis does not change the
  preregistered comparison — it is a secondary axis and the preregistered
  quantity is model weights — but it does supply an independent check of the
  headline ordering, reported below. **Nothing is imported and no milestone is
  closed**; four evaluations are still outstanding.
  ...
  **Terminal status re-derived, not taken from the event.** The watcher line
  ...
  present (`{"status": "completed", "results": 2}`), `manifest.status:
  sync_pending`, and `error-marker.txt` = `no application error recorded`.
  Full-root watcher scan at 21:48:5xZ, fresh cursor, 51 bundle rows.

  **This run's numbers** (`weights=ema`, official 10,000-image CIFAR-10 test
  split, ε `8/255`, CE-PGD-20 at 20 steps, AutoAttack standard, percent):

  | checkpoint | clean | CE-PGD-20 | AutoAttack |
  |---|---|---|---|
  | `best-ema.pt` | 79.54 | 54.32 | **49.30** |
  | `last.pt` | 85.24 | 48.83 | **45.03** |

  Both rows carry `weights: ema`, so the `last.pt` row is that checkpoint's
  **EMA** weights, not the raw ones the model axis evaluated. `selection_weights`
  is `ema` on the `best-ema.pt` row and `model` on the `last.pt` row, which is
  the contract's own convention: `best-ema.pt` was *selected* on held-out
  CE-PGD-20 computed from EMA weights, `last.pt` was not selected at all.
  ...
  **Contract identity verified against the frozen arm-3 contract.** Protocol
  ...
  both rows, evaluation attack CE ε `8/255` step `2/255` 20 steps random start
  `temperature_squared: true` (`threat_hash` `7081101693340e70…`, byte-equal to
  the one every other run in this comparison carries), training attack
  KL/`rectified` at 10 steps, `adr` block `ema_decay 0.995 / T_high 2.5 /
  T_low 2.0 / λ_low 0.7 / λ_high 0.95`, `trades_beta 6.0`, `student_ema_decay
  0.9`, `oracle_mask false`, `frozen_oracle_manifest null`, SGD `lr 0.1 /
  momentum 0.9 / wd 5e-4 / nesterov true`, multistep `[100, 150] gamma 0.1`,
  `epochs 200`, `validation_fraction 0.1`, `batchnorm_mode local_per_rank`,
  world size 1 and per-rank = global batch 128, `deterministic: true`, all five
  `ARD_SEED` seeds **2** with the fixed `split: 20260722`, and
  `checkpoint_filename` `best-ema.pt` / `last.pt` — the EMA axis evaluated the
  EMA checkpoint it is supposed to, and no `best.pt` leaked onto it. AutoAttack
  `attack_version: standard` with `provenance.expected_commit` = `vcs_commit` =
  `a39220048b3c…`, the pinned upstream, on both rows.
  ...
  **Lineage verified.** Source SHA `cd0b571e4685…`, `dirty: false`,
  ...
  carries, with all five upstream pins at their contracted commits (`adr`
  `515da0e0…`, `trades` `6e8e11b7…`, `robustbench` `78fcc9e4…`, `saad`
  `295121c5…`, `da_alone_improves_at` `38b740ae…`). All seven declared artifacts
  exist both at their live path and as content-addressed copies under
  `run-bundle/artifacts/<name>/<sha256>/`, and each copy's directory name equals
  the hash the manifest declares. The chain to training closes:
  `evaluation-lineage.json`'s `training_config_hash` `c0a28b33…` equals the
  training bundle's own `config_hash` and equals the `config_hash` on both
  result rows, `training_config_migration.applied` is empty with
  `runtime_method_id` = `source_method_id` = `adr`, and `train_run_id` =
  `adr-campaign-v1-ferret-cifar10_r18_adr-s2` — the same training run the model
  axis evaluated, so the two axes are two readings of one model. **One check
  could not be run**, as in every entry since 15:12Z: the seven artifacts' and
  the two checkpoints' SHA-256 were **not independently recomputed** — this
  session's sandbox refuses to hash files outside the repository root, and the
  attempt on `best-ema.pt` was refused again here. They are asserted by the
  producer and internally consistent, not confirmed here.

  **Attack-quality sanity, all six checks pass.** AutoAttack ≤ CE-PGD-20 ≤ clean
  on both rows (49.30 ≤ 54.32 ≤ 79.54; 45.03 ≤ 48.83 ≤ 85.24).
  `canon-eval-lane-W.log:202,401` reports `max Linf perturbation: 0.03137`
  (= 8/255 exactly), `nan in tensor: 0`, and `max: 1.00000, min: 0.00000` on
  both checkpoints. The per-stage sequence is the normal shape for a robust
  model — APGD-CE 53.83 → APGD-T 49.27 → FAB-T 49.27 → SQUARE 49.27 for
  `best-ema.pt`, and 47.77 → 45.00 → 45.00 → 45.00 for `last.pt`: APGD-T does
  all the work after APGD-CE and neither FAB-T nor Square finds anything
  further. **No deviation and no bug found.**

  **Arm 3's EMA axis, all three seeds, re-read from the three
  `evaluation-results.json` files** (best-ema.pt, official test, percent):

  | seed | clean | CE-PGD-20 | AutoAttack | model-axis AutoAttack | EMA − model |
  |---|---|---|---|---|---|
  | 0 | 83.38 | 54.26 | **49.61** | 48.75 | **+0.86** |
  | 1 | 83.85 | 53.94 | **49.29** | 48.65 | **+0.64** |
  | 2 | 79.54 | 54.32 | **49.30** | 48.22 | **+1.08** |
  | mean | 82.26 | 54.17 | **49.40** | 48.54 | **+0.86** |
  | sd | 2.36 | 0.20 | 0.18 | 0.28 | 0.22 |

  EMA weights beat model weights on AutoAttack at **all three seeds**, by
  **+0.86 pp** on average. On the last checkpoint the same difference is
  **+0.10 pp** (44.92 / 44.83 / 45.03 against 44.81 / 44.83 / 44.84) — that is
  inside the ≈0.71 pp two-run sampling noise and is **not** a difference. So
  the EMA advantage lives entirely in *which epoch gets selected*, not in the
  averaging at the end of training. Directional for these six runs.

  **One number in that table is an outlier and it is a real one: seed 2's
  best-ema clean accuracy is 79.54, against 83.38 and 83.85 at seeds 0 and 1.**
  That is a **4.31 pp** clean spread on the EMA axis against **0.74 pp** on the
  model axis for the same three training runs, while the AutoAttack spread is
  the other way round (0.32 pp EMA against 0.53 pp model). Seed 2's EMA
  checkpoint also has the *highest* CE-PGD-20 of the three (54.32). Read
  together: held-out CE-PGD-20 selection on the EMA axis picked, for seed 2, a
  much more robustness-leaning point of the same clean-robust tradeoff, and it
  bought ≈0 AutoAttack for ≈4 pp of clean accuracy. **Clean and robust accuracy
  are reported separately here and must stay separate**; this is measured
  checkpoint-selection behaviour for arm 3, not a defect and not a claim that
  generalizes beyond it. M3's report should carry the EMA axis's clean spread
  next to its AutoAttack mean, or the mean will look steadier than the axis is.

  **The independent check of the headline ordering, and what it does not
  establish.** `scripts/aggregate_adr_cifar10_replication.py:427` emits a
  `best_checkpoint_ema_weights` comparison; `_paired_delta` at line 375-376
  takes the **treatment's EMA** weights against the **baseline's model**
  weights, because no plain-AT arm has an EMA axis at all. With arm 8's EMA
  axis (43.07 / 43.34 / 42.91, mean **43.11**) now joined by arm 3's, that
  comparison reads:

  | leg | ADR EMA | plain-AT model | seed-paired gains | mean |
  |---|---|---|---|---|
  | MobileNetV2 (8 vs 7) | 43.11 | 39.92 | +3.34 / +3.48 / +2.73 | **+3.18** |
  | ResNet-18 (3 vs 2) | 49.40 | 47.32 | +2.42 / +1.88 / +1.93 | **+2.08** |

  MobileNetV2's gain again exceeds ResNet-18's, by **+1.11 pp** against the
  model-weights comparison's **+1.58 pp** — **the same sign and the same
  ordering, at a smaller margin**, and positive in all six pairs. The margin
  shrinks because weight averaging helps arm 3 (+0.86 pp) more than arm 8
  (+0.39 pp). **This is a robustness check of the direction, not a second
  result**: it bundles ADR training with weight averaging on the treatment side
  only, so a gain measured this way cannot be attributed to ADR alone, and no
  matched EMA baseline exists or is contracted. The preregistered quantity is
  unchanged and **this postrun does not apply the rule** — that is M3's
  deliverable and the human's decision.

  **The preregistered primary comparison is untouched by this run.** It uses
  model weights at the best checkpoint, all twelve of those evaluations were
  already terminal at 20:47Z, and the arithmetic in that entry stands:
  MobileNetV2 **+2.79 pp** against ResNet-18 **+1.22 pp**, leg difference
  **+1.58 pp**, "sign confirmed" branch, ratio to the noise floor 2.8× on
  binomial sampling error and ≈5.4× taking the seed as the unit, neither of
  them a confidence interval. The **last-checkpoint reversal** recorded at
  20:47Z also stands and is not softened by anything here.

  **Cost, and a datum for packet 0010.** `created_at 20:40:14.085Z` →
  `finished_at 21:43:34.709Z` = **1 h 03 m 20.6 s**, `exit=0`, **first
  attempt**. AutoAttack's own reported stage times account for 3,768.7 s of
  that, **99.2 %** of wall clock. Dividing each stage by the number of inputs
  that actually entered it gives **6.54 / 64.3 / 152.2 / 165.9 ms** for
  `best-ema.pt` against **6.50 / 64.5 / 152.0 / 168.0 ms** for `last.pt` —
  agreement within **1.3 % on all four stages**, so this run had its card to
  itself throughout. Arm 3's uncontended AutoAttack cost is now measured three
  times at **≈1 h 03 m ± 40 s** (seed 1 model 1 h 04 m 09 s, seed 2 model
  1 h 02 m 49 s, seed 2 EMA 1 h 03 m 21 s). **This is a reconstruction from
  AutoAttack's cumulative timing marks and bundle timestamps, not a process
  table.**

  **A third concurrent-AutoAttack observation, and it agrees with the first
  two.** `canon-eval-lane-X.log` ran `r18_trades-s1`'s model evaluation from
  **20:43:18Z to 21:47:15Z** (`exit=0`), overlapping this run for essentially
  its whole hour, and **neither was slowed**: this run held its uncontended
  rate to 1.3 % and lane X's own APGD-CE rate (55.1 s over 8,276 inputs =
  6.66 ms) is the same rate this run measured. That is the third independent
  instance of the 19:45Z conclusion — **two AutoAttack evaluations at once are
  not by themselves a hazard; two on one device are.**

  **The packet-0010 estimator tally gains two comparisons and its ceiling still
  does not move.** AutoAttack's own printed robust accuracy against the number
  `src/ard/evaluation/autoattack.py:213` recomputes in one unbatched forward:
  `best-ema.pt` **49.27 % printed vs 49.30 % recorded** (+3 examples, +0.03 pp)
  and `last.pt` **45.00 % vs 45.03 %** (+3 examples, +0.03 pp). The tally is now
  **15 disagreements out of 20 comparisons, all fifteen in the same direction**
  (the recorded number is the higher one), with the magnitude ceiling still
  **0.09 pp** — an order of magnitude below every gap this entry reports, so no
  conclusion above turns on which estimator is used. **This postrun does not
  choose; packet 0010's `chosen` is still null.**

  **Three operational facts this postrun observed and did not cause.** First,
  **the eleventh launch while packet 0010 is `status: pending`, `chosen: null`**:
  `canon-eval-lane-Y.log:1` starts `r18_trades_adr-s0`'s EMA evaluation at
  **21:44:08Z**, 33 s after this run freed its GPU. Second, **the
  evidence-overwrite pattern recurred on that restart**: that same evaluation
  had been started by lane A at 11:24:11Z and killed `exit=143` at 11:25:07Z —
  the SIGTERM defect — and its bundle has read a false `running` ever since;
  lane Y did **not** hit `FileExistsError`, so the dead attempt's output
  directory was deleted in between, the fourth instance of this pattern. Third,
  **the headless postrun path is still dead**, unchanged from 19:45Z and 20:47Z:
  this run's hook log
  (`orchestration/ardx/claude-runs/20260910T214345Z-eval-669190483234c84c27f1_evaluation-ema.log`)
  and `r18_trades-s1`'s (`…20260910T214718Z-eval-01e527cf…_evaluation.log`) each
  contain exactly one line, *"Ignoring 44 permissions.allow entries … this
  workspace has not been trusted"*. The fix is
  `projects["/home/islab/workspace-local/shunsuke.naito/ard_codex_bootstrap"].hasTrustDialogAccepted:
  true` in `/home/shunsukenaito/.claude.json`, or accepting the dialog
  interactively at that exact path once; **it is a user-level config change and
  this postrun did not make it.** **This postrun started nothing and stopped
  nothing.**

  **Nothing imported.** `docs/experiments/adr_cifar10_replication_v1.json` does
  not exist and none can be written: `_load_arm_seed` raises on the first
  missing `evaluation-results.json`, and **2 of the 20 model-weights
  evaluations** and **2 of the 9 EMA evaluations** are still non-terminal or
  absent. Read against `scripts/aggregate_adr_cifar10_replication.py`, this
  bundle passes every gate the aggregator applies — `completion.json` present,
  `manifest.status` in `{completed, sync_pending}`, the exact error marker,
  `protocol_id` = `controlled_cifar10_r18_adr_v1`, `git.dirty` false, all seven
  declared artifacts present at their live paths, `count == 10000`,
  `dataset_identity.split == "test"`, `runtime_method == "adr"`, `weights ==
  "ema"` on both rows, an `autoattack` dict on both rows, `attack_version ==
  "standard"`, `expected_commit == a39220048b3c…`, and both `ALIASES` resolvable
  once `best-ema` is normalized to `best` (line 301) — so nothing about this run
  blocks M3. No evidence-ledger row is owed either, since a ledger row
  accompanies a record. **No decision packet is opened here**: this campaign's
  primary packet is M3's deliverable and is gated on the record, and packet 0010
  receives its addendum instead.

  **What is left, and it is all on the TRADES legs.** The record needs four more
  results: model weights for `r18_trades-s2` and `r18_trades_adr-s2`, and EMA
  for `r18_trades_adr-s0` (restarted 21:44:08Z, running) and `r18_trades_adr-s2`
  (never started). **`r18_trades-s2`'s evaluation is the one with no live job**:
  it reads `stale`, its last progress was 12:11:52Z, and
  `canon-eval-lane-B.log:632` shows it SIGTERM'd `exit=143` at 12:54:04Z
  mid-Square — the fourth instance of the SIGTERM defect, already recorded at
  12:54Z. Nothing has restarted it in the nine hours since. **This postrun does
  not restart it**; that is a launch, and launches are not this skill's to make.

  **Census at 21:48:5xZ, from a full-root watcher scan (51 bundle rows, fresh
  cursor).** Training: **20 of 20** terminal and successful, unchanged.
  Model-weights contract evaluations: **18 of 20** terminal and successful;
  **0 terminal-failed**; **2 non-terminal** (`r18_trades-s2` `stale` and dead,
  `r18_trades_adr-s2` running). EMA-weights evaluations: **7 of 9** terminal and
  successful (this run is the seventh); **1 non-terminal**
  (`r18_trades_adr-s0`); **1 never started** (`r18_trades_adr-s2`). The
  remaining 3 rows are the 2026-09-09 `early-check-eval-*` diagnostics under
  `r18_adr-s1`, which are not contract evaluations. **Two terminal successful
  results other than this one are unrecorded and owe postruns**:
  `r18_trades_adr-s1` EMA (`eval-533e04a8c5a3f4378434`, terminal 20:42:32Z) and
  `r18_trades-s1` model weights (`eval-01e527cfc8a9bdeb9146`, terminal
  21:47:15Z, i.e. **after** the census that opened this postrun) — both headless
  postruns recorded nothing, per the trust defect above. **Arm 3 is now complete
  on both axes**, joining arm 8; arms 2, 6 and 7 are complete on the only axis
  they define. M1c's training half is closed and its evaluation half is not, so
  **M1c stays unticked**; M2 and M3 stay open.
- 2026-09-10 22:55Z: postrun of the `eval-77f66adc6ec58702c412` terminal event —
  **arm 5 (`cifar10_r18_trades_adr`, method `adr_trades`) seed 0, EMA weights,
  both checkpoints, official CIFAR-10 test set. Arm 5 now has two seeds on both
  of its axes.** This run is on a TRADES leg and does not touch the
  preregistered primary comparison (arms 2, 3, 7, 8), whose arithmetic in the
  20:47Z entry stands unchanged. **Nothing is imported and no milestone is
  closed**; three evaluations are still outstanding. This entry was written by
  the headless postrun the watcher started at 22:50:10Z, and it corrects one
  claim the 19:45Z, 20:47Z and 21:48Z entries made about that path (below).
  ...
  **Terminal status re-derived, not taken from the event.** The watcher line
  ...
  present (`{"status": "completed", "results": 2}`), `manifest.status:
  sync_pending`, and `error-marker.txt` = `no application error recorded`.
  Full-root watcher scan at 22:52:02Z, fresh cursor, 51 bundle rows.
  ...
  **This run's numbers** (`weights=ema`, official 10,000-image CIFAR-10 test
  ...
  | checkpoint | clean | CE-PGD-20 | AutoAttack |
  |---|---|---|---|
  | `best-ema.pt` | 83.57 | 54.01 | **49.54** |
  | `last.pt` | 83.93 | 53.04 | **48.36** |

  Both rows carry `weights: ema`, so the `last.pt` row is that checkpoint's EMA
  weights. `selection_weights` is `ema` on the `best-ema.pt` row and `model` on
  the `last.pt` row, the contract's own convention.

  **Contract identity verified against the frozen arm-5 contract.** Protocol
  `controlled_cifar10_r18_adr_v1`, `method` = `runtime_method` = `adr_trades`,
  `teacher: null`, student `saad_resnet18_cifar_v1` with `pixel_0_1` input
  domain and the `cifar10_raw_identity` profile owned by the student adapter,
  `count: 10000` with `dataset_identity.split: test` and content fingerprint
  `c58f3010…` on both rows, evaluation attack CE ε `8/255` step `2/255` 20
  steps random start `temperature_squared: true` (`threat_hash`
  `7081101693340e70…`, byte-equal to every other run in this campaign),
  training attack KL (`kl_target: student_clean`) at 10 steps, `adr` block
  `ema_decay 0.995 / T_high 2.5 / T_low 2.0 / λ_low 0.7 / λ_high 0.95`,
  `trades_beta 6.0`, `student_ema_decay 0.9`, `entropy_gamma 1.0`, `oracle_mask
  false`, `frozen_oracle_manifest null`, SGD `lr 0.1 / momentum 0.9 / wd 5e-4 /
  nesterov true`, multistep `[100, 150] gamma 0.1`, `epochs 200`,
  `validation_fraction 0.1`, `batchnorm_mode local_per_rank`, world size 1 and
  per-rank = global batch 128, `deterministic: true`, all six `ARD_SEED` seeds
  **0** with the fixed `split: 20260722`, and `checkpoint_filename`
  `best-ema.pt` / `last.pt` — no `best.pt` leaked onto the EMA axis. AutoAttack
  `attack_version: standard` with `provenance.expected_commit` = `vcs_commit` =
  `a39220048b3c…`, the pinned upstream, on both rows.

  **Lineage verified.** Source SHA `cd0b571e4685…`, `dirty: false`,
  `diff_sha256` equal to the SHA-256 of the empty string. `external.lock`
  `05cfce4c…`, byte-identical to every other bundle in this campaign, with all
  five upstream checkouts at their contracted commits and a clean status (`adr`
  `515da0e0…`, `trades` `6e8e11b7…`, `robustbench` `78fcc9e4…`, `saad`
  `295121c5…`, `da_alone_improves_at` `38b740ae…`). All seven declared
  artifacts exist both at their live path and as content-addressed copies under
  `run-bundle/artifacts/<name>/<sha256>/`, and each copy's directory name equals
  the hash the manifest declares. The chain to training closes:
  `evaluation-lineage.json`'s `training_config_hash` `7d9f2051…` equals the
  training bundle's own `config_hash` and the `config_hash` on both result rows,
  `training_config_migration.applied` is empty with `runtime_method_id` =
  `source_method_id` = `adr_trades`, and `train_run_id` =
  `adr-campaign-v1-cifar10_r18_trades_adr-s0`, the training run the watcher
  reports terminal and successful. **One check could not be run**, as in every
  entry since 15:12Z: the artifacts' and checkpoints' SHA-256 were **not
  independently recomputed** — this session's shell is confined to the
  repository root. They are asserted by the producer and internally
  consistent, not confirmed here.

  **Attack-quality sanity, all six checks pass.** AutoAttack ≤ CE-PGD-20 ≤ clean
  on both rows (49.54 ≤ 54.01 ≤ 83.57; 48.36 ≤ 53.04 ≤ 83.93).
  `canon-eval-lane-Y.log:204,409` reports `max Linf perturbation: 0.03137`
  (= 8/255 exactly), `nan in tensor: 0`, and `max: 1.00000, min: 0.00000` on
  both checkpoints. The per-stage sequence is the normal shape for a robust
  model — APGD-CE 53.35 → APGD-T 49.51 → FAB-T 49.51 → SQUARE 49.51 for
  `best-ema.pt`, and 52.39 → 48.34 → 48.34 → 48.34 for `last.pt`. **No deviation
  and no bug found.**

  **Arm 5, both axes, two seeds, re-read from the four `evaluation-results.json`
  files** (official test, AutoAttack, percent):

  | seed | model `best.pt` | EMA `best-ema.pt` | EMA − model | model `last.pt` | EMA `last.pt` |
  |---|---|---|---|---|---|
  | 0 | 49.19 | **49.54** | +0.35 | 48.11 | 48.36 |
  | 1 | 48.45 | **49.49** | +1.04 | 47.68 | 47.69 |

  The EMA axis's two seeds differ by **0.05 pp**, against 0.74 pp on the model
  axis for the same two training runs; both are two-run spreads and neither is
  a measurement of seed variance. EMA weights beat model weights at the selected
  checkpoint at both seeds (+0.35 and +1.04 pp); on the last checkpoint the
  difference is +0.25 and +0.01 pp, inside the ≈0.71 pp two-run sampling noise.
  That is the same pattern arm 3 showed at 21:48Z: the EMA advantage sits in
  *which epoch gets selected*, not in the averaging at the end. Best-checkpoint
  clean accuracy on the EMA axis is 83.57 / 82.74, so arm 5 does not show arm 3
  seed 2's clean outlier. **Directional for these two seeds only.**

  **The TRADES-leg secondary comparison, not decisive.** The aggregator's
  `resnet18_trades_vs_plain_sgd_baseline` pair sets arm 5 against arm 4
  (`r18_trades`). Arm 4's best-checkpoint AutoAttack is **47.87** at seed 0 (the
  archived `docs/experiments/historical/trades_fix_v1_seed0/`) and **47.89** at
  seed 1 (`eval-01e527cfc8a9bdeb9146`, terminal and successful, still
  unrecorded). Seed-paired, arm 5 gains **+1.32 / +0.56 pp** on model weights
  and **+1.67 / +1.60 pp** when its EMA weights meet arm 4's model weights.
  **Neither is like-for-like**: arm 4 runs `controlled_cifar10_r18_v1` with
  plain SGD while arm 5 runs `controlled_cifar10_r18_adr_v1` with Nesterov, and
  the EMA version adds weight averaging on the treatment side only. Robust
  overfitting (best minus last AutoAttack) is 2.88 / 2.26 pp for arm 4 against
  1.08 / 0.77 pp for arm 5's model weights and 1.18 / 1.80 pp for its EMA
  weights: every arm-5 number sits below both arm-4 numbers, and the TRADES leg
  overfits far less than the plain-AT ResNet-18 leg (6.55–7.00 pp) in either
  arm. Directional for these four runs.

  **Cost, and a datum for packet 0010.** `created_at 21:44:10.347Z` →
  `finished_at 22:49:41.819Z` = **1 h 05 m 31.5 s**, `exit=0`, on lane Y's
  first attempt (the bundle's second overall; lane A's 11:24:11Z attempt died
  `exit=143` at 11:25:07Z). AutoAttack's own stage times account for 3,899.5 s
  of that, **99.2 %** of wall clock. Dividing each stage by the number of inputs
  that entered it gives **6.52 / 63.9 / 151.3 / 167.2 ms** for `best-ema.pt`
  against **6.52 / 63.7 / 151.3 / 167.0 ms** for `last.pt` — agreement within
  **0.3 % on all four stages**, so this run had its card to itself. This is
  arm 5's first uncontended AutoAttack cost measurement; it sits about 2 min
  above arm 3's three measurements (≈1 h 03 m ± 40 s). **Reconstructed from
  AutoAttack's cumulative timing marks and bundle timestamps, not a process
  table.**

  **A fourth concurrent-AutoAttack observation, and it agrees with the first
  three.** `canon-eval-lane-Z.log` started `r18_trades_adr-s2`'s model evaluation
  at **21:47:47Z** and was still running when this run finished, so the two
  overlapped for essentially this run's whole hour; this run held its
  uncontended per-stage rate to 0.3 %, and lane Z's APGD-CE took 55.1 s for
  `best.pt` against this run's 54.5 s. **Two AutoAttack evaluations at once are
  not by themselves a hazard; two on one device are.**

  **The packet-0010 estimator tally gains two comparisons and its ceiling still
  does not move.** AutoAttack's own printed robust accuracy against the number
  `src/ard/evaluation/autoattack.py:213` recomputes in one unbatched forward:
  `best-ema.pt` **49.51 % printed vs 49.54 % recorded** (+3 examples,
  +0.03 pp) and `last.pt` **48.34 % vs 48.36 %** (+2 examples, +0.02 pp). The
  tally is now **17 disagreements out of 22 comparisons, all seventeen in the
  same direction** (the recorded number is the higher one), ceiling still
  **0.09 pp** — an order of magnitude below every gap this entry reports.
  **This postrun does not choose; packet 0010's `chosen` is still null.**

  **Operational facts this postrun observed and did not cause.**
  First, **the twelfth and thirteenth launches while packet 0010 is `status:
  pending`, `chosen: null`**: lane Z (above) at 21:47:47Z, 32 s after lane X
  freed its GPU and not counted by the 21:48Z entry, and
  `canon-eval-lane-AA.log:1` starting `r18_trades-s2`'s model evaluation at
  **22:50:26Z**, 44 s after this run freed its GPU. Second, **the
  evidence-overwrite pattern recurred twice more, the fifth and sixth
  instances**: lane AA started without `FileExistsError` at the path where lane
  B's 11:10:21Z attempt had run until its 12:54:04Z SIGTERM, and lane Z did the
  same at the path of lane I's 09:09:31Z attempt, whose bundle the 20:47Z census
  read as `running` (lane I's log holds only its start line, so how that attempt
  ended is not recorded). Both earlier output directories were therefore
  deleted in between.
  Third, **a correction: the headless postrun path is not dead.** The 19:45Z,
  20:47Z and 21:48Z entries said every Bash call in a headless session is
  denied because of the trust warning. That is wrong. `scripts/ardx/postrun_hook.sh:168-169`
  passes the allowlist on the command line (`--allowedTools`), so the warning
  only drops the settings-file copy of it; allowlisted commands run. The
  21:47:18Z headless postrun for `eval-01e527cfc8a9bdeb9146` ran 70 turns,
  re-derived that bundle's terminal status, and **declined to write** because
  a peer session was editing this plan and packet 0010 at that moment (its
  result JSON under `orchestration/ardx/claude-runs/`). This entry is the first
  one the headless path has written. What *is* denied is anything off the
  allowlist, including any read outside the repository root through the shell,
  which is why checksums still cannot be recomputed here.
  Fourth, **the 20:47Z and 21:48Z entries and their packet-0010 addenda were
  found uncommitted**, untouched since 21:52:54Z, and are committed with this
  entry unchanged. **This postrun started nothing and stopped nothing.**

  **Nothing imported.** `docs/experiments/adr_cifar10_replication_v1.json` does
  not exist and none can be written: `_load_arm_seed` raises on the first
  missing `evaluation-results.json`, and **2 of the 20 model-weights
  evaluations** and **1 of the 9 EMA evaluations** are still outstanding. Read
  against `scripts/aggregate_adr_cifar10_replication.py`, this bundle passes
  every gate the aggregator applies — `completion.json` present,
  `manifest.status` in `{completed, sync_pending}`, the exact error marker,
  `protocol_id` = `controlled_cifar10_r18_adr_v1`, `git.dirty` false, all seven
  declared artifacts present at their live paths, `count == 10000`,
  `dataset_identity.split == "test"`, `runtime_method == "adr_trades"`,
  `weights == "ema"` on both rows, an `autoattack` dict on both rows,
  `attack_version == "standard"`, `expected_commit == a39220048b3c…`, and both
  `ALIASES` resolvable once `best-ema` is normalized to `best` — so nothing about
  this run blocks M3. No evidence-ledger row is owed, since a ledger row
  accompanies a record, and **no decision packet is opened here**: the
  campaign's primary packet is M3's deliverable, and packet 0010 receives its
  addendum instead.

  **What is left: three evaluations, two of them already running.** Model
  weights for `r18_trades_adr-s2` (lane Z, running since 21:47:47Z) and
  `r18_trades-s2` (lane AA, running since 22:50:26Z — so it is no longer the
  evaluation "with no live job"), and EMA for `r18_trades_adr-s2` (never
  started).

  **Census at 22:52:02Z, from a full-root watcher scan (51 bundle rows, fresh
  cursor).** Training: **20 of 20** terminal and successful, unchanged.
  Model-weights contract evaluations: **18 of 20** terminal and successful;
  **0 terminal-failed**; **2 non-terminal**, both `running` (`r18_trades_adr-s2`,
  `r18_trades-s2`). EMA-weights evaluations: **8 of 9** terminal and successful
  (this run is the eighth); **0 non-terminal**; **1 never started**
  (`r18_trades_adr-s2`). The remaining 3 rows are the 2026-09-09
  `early-check-eval-*` diagnostics under `r18_adr-s1`, which are not contract
  evaluations. **Two terminal successful results other than this one are
  unrecorded and owe postruns**: `r18_trades_adr-s1` EMA
  (`eval-533e04a8c5a3f4378434`) and `r18_trades-s1` model weights
  (`eval-01e527cfc8a9bdeb9146`); this entry reads their numbers for the
  comparison above but does not stand in for their postruns. Arms 2, 3, 6, 7
  and 8 are complete; arm 5 has two seeds of three on both axes and arm 4 two of
  three on its only axis. M1c's training half is closed and its evaluation half
  is not, so **M1c stays unticked**; M2 and M3 stay open.

  **Verify caveat.** The first `scripts/verify.py --changed` run of this postrun
  was red on one test,
  `tests/integration/test_tracking_evaluation.py::test_resume_with_no_remaining_epochs_uses_checkpoint_last_metrics`
  (`TrackingError: resume manifest lineage drift: git`). That test compares the
  repository's git state at a training run and at its resume, while verify ran
  the selected test files concurrently. Run alone, the same test passes. The
  diff here is documentation only; the flake is recorded for a separate plan
  and is not fixed here.
- 2026-09-10 22:55Z: postrun of the `eval-0f548c3b6b3edadb6524` terminal event —
  **arm 5 (`cifar10_r18_trades_adr`) seed 2, model weights, both checkpoints,
  official CIFAR-10 test set.** This is on the secondary TRADES leg, not on the
  preregistered comparison. **Nothing is imported and no milestone is closed.**

  **Terminal status re-derived.** The watcher line whose `path` is the bundle
  manifest reads `completed / success=true / terminal=true /
  failure_class=null`. `completion.json` is present, `manifest.status` is
  `sync_pending`, and `error-marker.txt` is exactly `no application error
  recorded`. Run `created_at 21:47:49.7Z` → `finished_at 22:53:19.5Z`
  (1 h 05 m 30 s).

  **This run's numbers** (`weights=model`, `runtime_method=adr_trades`,
  `count=10000`, `split=test`, CE-PGD-20 ε `8/255` step `2/255`, AutoAttack
  `standard` at pinned `a39220048b3c…` on both rows, training seeds all 2 with
  split seed `20260722`, world size 1, global batch 128, percent):

  | checkpoint | clean | CE-PGD-20 | AutoAttack |
  |---|---|---|---|
  | `best.pt` | 83.14 | 52.96 | **48.55** |
  | `last.pt` | 84.17 | 52.42 | **48.09** |

  AutoAttack ≤ CE-PGD-20 ≤ clean holds on both rows.

  **Lineage.** Protocol `controlled_cifar10_r18_adr_v1`. Source `cd0b571e4685…`,
  `dirty: false`, empty-diff hash. `external.lock` `05cfce4c…`, with all five
  upstream pins at their contracted commits and clean. All seven declared
  artifacts exist at their live paths. Checkpoint SHA-256 as the producer
  declares them: `best.pt` `6afc68a8…`, `last.pt` `f7ab5d70…`. **One check was
  not run:** this headless session's sandbox refuses to hash files outside the
  repository, so the artifact and checkpoint hashes are asserted by the
  producer and were not recomputed here.

  **Arm 5 on the model axis now has all three seeds** (read from the three
  `evaluation-results.json` files, percent):

  | seed | clean best | AutoAttack best | AutoAttack last | best − last |
  |---|---|---|---|---|
  | 0 | 83.68 | 49.19 | 48.11 | 1.08 |
  | 1 | 81.88 | 48.45 | 47.68 | 0.77 |
  | 2 | 83.14 | 48.55 | 48.09 | 0.46 |
  | mean | 82.90 | **48.73** | 47.96 | 0.77 |

  The seed-2 best-checkpoint AutoAttack sits between seeds 0 and 1. The
  best-minus-last gap stays small on all three seeds; this directional reading
  covers these three runs only. **No seed-2 arm 5 − arm 4 pair exists yet**:
  arm 4 seed 2's model evaluation (`r18_trades-s2`) is still the stale,
  SIGTERM'd job recorded at 12:54Z, with nothing restarted.

  **The `r18_trades-s1` model evaluation (`eval-01e527cfc8a9bdeb9146`, terminal
  21:47:15Z) had no entry.** Its headless postrun verified it but deferred its
  write to a peer session and ended without writing. For the record, its
  numbers read from its `evaluation-results.json` (`weights=model`,
  `runtime_method=trades`): `best.pt` 82.76 / 50.67 / **47.89**, `last.pt`
  82.52 / 48.20 / **45.63** (clean / CE-PGD-20 / AutoAttack). That session
  reported both checkpoint hashes as matching (`f48510db…`, `c4ce086e…`); this
  entry did not recompute them. Seed-paired arm 5 − arm 4 on best AutoAttack
  is now **+1.32 (s0), +0.56 (s1)**. Both pairs are positive, which is
  directional for these two seeds only.

  **The headless postrun path is failing, not merely slow.** Postruns
  `20260910T203921Z` (`5709dc2e`), `…204254Z` (`533e04a8`) and `…214345Z`
  (`669190…`) ended at `error_max_turns` (60) without committing. Their plan
  and packet-0010 text sits uncommitted in the tree. `…214718Z` (`01e527…`)
  ended without writing. Every one of them burned turns on permission
  denials, because `permissions.allow` is ignored in an untrusted workspace
  (hook log line 1). The fix is unchanged: accept the trust dialog once at this
  exact path, or set `hasTrustDialogAccepted: true` for it in
  `/home/shunsukenaito/.claude.json`. That is a user-level change, and this
  postrun did not make it. Postrun `…225010Z` (`77f66adc…`, an EMA evaluation)
  was running concurrently with this one.

  **Outstanding for the record, as of the 21:48Z census plus this run:**
  model `r18_trades-s2` (no live job), EMA `r18_trades_adr-s0` (lane Y), EMA
  `r18_trades_adr-s2` (not started at 21:48Z). This postrun did not re-run the
  full-root census. **It started nothing and stopped nothing.** M1c, M2 and M3
  stay open.

- 2026-09-10 23:56Z: postrun of the `eval-81e1a96c5d8ed98aaab1` terminal event —
  **arm 4 (`cifar10_r18_trades`) seed 2, model weights, both checkpoints,
  official CIFAR-10 test set.** This is the last model-weights evaluation of
  the campaign, and it is on the secondary TRADES leg, not on the preregistered
  comparison. **Nothing is imported and no milestone is closed.**

  **Terminal status re-derived.** The watcher line whose `path` is the bundle
  manifest reads `completed / success=true / terminal=true /
  failure_class=null`. `run-bundle/completion.json` reads `{"status":
  "completed", "results": 2}`, `manifest.status` is `sync_pending`, and
  `run-bundle/error-marker.txt` is exactly `no application error recorded`.
  Run `created_at 22:50:29.0Z` → `finished_at 23:52:33.0Z` (1 h 02 m 04 s).
  This is lane AA's attempt; the 11:10:21Z attempt at the same path was
  SIGTERM'd at 12:54:04Z (22:55Z entry).

  **This run's numbers** (`weights=model`, `runtime_method=trades`,
  `count=10000`, `split=test`, CE-PGD-20 ε `8/255` step `2/255` random start,
  AutoAttack `standard` at pinned `a39220048b3c…` on both rows, threat hash
  `7081101693340e70…`, training seeds all 2 with split seed `20260722`,
  evaluation seed 0, world size 1, global batch 128, percent):

  | checkpoint | clean | CE-PGD-20 | AutoAttack |
  |---|---|---|---|
  | `best.pt` | 79.84 | 50.55 | **46.90** |
  | `last.pt` | 83.00 | 48.58 | **45.80** |

  AutoAttack ≤ CE-PGD-20 ≤ clean holds on both rows. The low best-checkpoint
  clean accuracy has a visible cause: this seed's validation-selected best epoch
  is **102**, two epochs after the first learning-rate decay, while seed 1's is
  157 (both from the training manifests' `best_epoch`). That is the selection
  rule working as contracted, not a defect.

  **Lineage.** Protocol `controlled_cifar10_r18_v1`, `nesterov: false`, method
  `trades` v1 (`trades_beta 6.0`, `adr: null`), student `saad_resnet18_cifar_v1`
  with `cifar10_raw_identity` normalization, `teacher: null`, `epochs=200`,
  `milestones=[100,150]`, `validation_fraction=0.1`. Training config hash
  `dc73a1bf0190…` on both rows and in `evaluation-lineage.json`, no migration
  applied. Source `cd0b571e4685…`, `dirty: false`, empty-diff hash.
  `external.lock` `05cfce4c…` with all five upstream pins at their contracted
  commits and clean. All seven declared artifacts are at their live Hamster
  paths, and `evaluation-results.json`'s content-addressed copy sits under a
  directory named for its manifest hash (`c9a49e7d…`). Checkpoint SHA-256 as
  the producer declares them: `best.pt` `c00bc3d6…`, `last.pt` `af19f446…`.
  **One check was not run:** this session's sandbox refuses to hash files
  outside the repository, and the training bundle records no checkpoint hash
  to compare against, so both checkpoint hashes are asserted by the producer
  and were not recomputed here.

  **Arm 4 on the model axis now has all three seeds** (seed 0 is the reused
  archived bundle of M1b; seeds 1 and 2 are read from their
  `evaluation-results.json` files; percent):

  | seed | clean best | AutoAttack best | AutoAttack last | best − last |
  |---|---|---|---|---|
  | 0 (archived) | 82.35 | 47.87 | 44.99 | 2.88 |
  | 1 | 82.76 | 47.89 | 45.63 | 2.26 |
  | 2 | 79.84 | 46.90 | 45.80 | 1.10 |
  | mean | 81.65 | **47.55** | 45.47 | 2.08 |

  The best-checkpoint AutoAttack spread is **0.99 pp**, the widest of any
  three-seed arm in this campaign (next is arm 5's 0.74 pp), and seed 2 is the
  low one. Against ≈0.71 pp of binomial sampling error on a two-run difference,
  that is ≈1.4× — directional for these three runs, not a variance estimate.

  **Secondary TRADES leg, seed-paired arm 5 − arm 4 on AutoAttack** (percent):
  ...
  Positive in all three pairs on both checkpoints. **This leg is not a matched
  comparison**: arm 5 trains with Nesterov (`controlled_cifar10_r18_adr_v1`) and
  arm 4 without (`controlled_cifar10_r18_v1`), so the gap bundles ADR with the
  optimizer change, and the preregistered rule reads it as directional only.
  The seed-2 pair is the largest partly because arm 4 seed 2's early best
  epoch (above) gives it the arm's lowest best-checkpoint number. On the last
  checkpoint the gap roughly doubles because arm 4 robustly overfits by
  ≈2.1 pp and arm 5 by ≈0.8 pp. Arm 6's preregistered reading (47.64 against
  arm 4 seed 0's 47.87) is unchanged; for context only, 47.64 also sits inside
  arm 4's three-seed range.

  **Nothing imported.** `docs/experiments/adr_cifar10_replication_v1.json`
  does not exist and still cannot be written, for two reasons. First, one
  evaluation is outstanding: EMA for `r18_trades_adr-s2`
  (`eval-0aa9853bd2757a416327`), `running` since 22:54:08Z, last progress
  23:28:19Z. Second, the `-ferret-` staging-path defect in `_verify_bundle`
  (2026-09-10 `r18_trades-s2` training entry) is still unfixed: this run's own
  training manifest declares its artifacts under
  `runs/adr-campaign-v1-ferret-cifar10_r18_trades-s2/outputs/train/`, no such
  directory exists locally, and `_load_arm_seed` verifies the training bundle
  before the evaluation, so M3 aborts on this arm-seed until the fallback to
  `local_path` lands. This evaluation bundle itself passes every gate the
  aggregator applies — `completion.json`, `manifest.status`, the exact error
  marker, `protocol_id` = `controlled_cifar10_r18_v1`, clean source tree,
  declared artifacts present, `count == 10000`, `split == "test"`,
  `runtime_method == "trades"`, `weights == "model"` on both rows, an
  `autoattack` dict with `attack_version == "standard"` and the expected commit
  on both rows. No evidence-ledger row is owed, since a ledger row accompanies a
  record, and no decision packet is opened; the campaign's packet is M3's
  deliverable.

  **Census at 23:54:13Z, from a full-root watcher scan (fresh cursor).**
  Training: **20 of 20** terminal and successful. Model-weights contract
  evaluations: **20 of 20** terminal and successful (this run is the
  twentieth). EMA-weights evaluations: **8 of 9** terminal and successful,
  **1 running** (`r18_trades_adr-s2`), **0 never started**. The three
  `early-check-eval-*` rows under `r18_adr-s1` are 2026-09-09 diagnostics, not
  contract evaluations. Arms 2, 3, 4, 6, 7 and 8 are complete on every axis
  their contract defines; arm 5 is complete on the model axis and has two of
  three EMA seeds. **M1c stays unticked** until the last EMA evaluation is
  terminal; M2 and M3 stay open.

  **Operational fact this postrun observed and did not cause.** The
  `r18_trades_adr-s2` EMA evaluation started at 22:54:08Z, after the 22:52:02Z
  census, while packet 0010 is still `status: pending`, `chosen: null`. That
  makes it the fourteenth launch during that packet's pending state (the 22:55Z
  entry counted thirteen). **This postrun started nothing and stopped
  nothing.**

- 2026-09-11 00:05Z: postrun of the `eval-0aa9853bd2757a416327` terminal event —
  **arm 5 (`cifar10_r18_trades_adr`, method `adr_trades`) seed 2, EMA weights,
  both checkpoints, official CIFAR-10 test set. This is the campaign's last
  evaluation: every training and contract evaluation job is now terminal and
  successful, and M1c is ticked.** The run is on the secondary TRADES leg and
  does not touch the preregistered primary comparison (20:47Z entry). **Nothing
  is imported**: M3 is blocked by the unfixed `-ferret-` staging-path defect
  (below), not by missing evidence.

  **Terminal status re-derived.** The watcher line whose `path` is the bundle
  manifest reads `completed / success=true / terminal=true /
  failure_class=null`. `run-bundle/completion.json` reads `{"status":
  "completed", "results": 2}`, `manifest.status` is `sync_pending`, and
  `error-marker.txt` is exactly `no application error recorded`. Run
  `created_at 22:54:08.4Z` → `finished_at 00:01:04.0Z` (1 h 06 m 56 s), lane AB
  (`canon-eval-lane-AB.log`, `exit=0`), first attempt.

  **This run's numbers** (`weights=ema` on both rows, `runtime_method=adr_trades`,
  `count=10000`, `split=test`, CE-PGD-20 ε `8/255` step `2/255` random start,
  AutoAttack `standard` at pinned `a39220048b3c…` on both rows, threat hash
  `7081101693340e70…`, training seeds all 2 with split seed `20260722`,
  evaluation seed 0, world size 1, global batch 128, percent):
  ...
  | checkpoint | clean | CE-PGD-20 | AutoAttack |
  |---|---|---|---|
  | `best-ema.pt` | 83.02 | 54.36 | **49.46** |
  | `last.pt` | 83.94 | 52.39 | **48.22** |

  `selection_weights` is `ema` on the `best-ema.pt` row and `model` on the
  `last.pt` row, the contract's convention; no `best.pt` appears on the EMA axis.

  **Contract and lineage.** Protocol `controlled_cifar10_r18_adr_v1` (SGD
  `lr 0.1 / momentum 0.9 / wd 5e-4 / nesterov true`, multistep `[100, 150]`
  `gamma 0.1`, `epochs 200`, `validation_fraction 0.1`), `teacher: null`,
  student `saad_resnet18_cifar_v1` with `cifar10_raw_identity`, training attack
  KL (`kl_target: student_clean`) 10 steps, `adr` block `0.995 / 2.5 / 2.0 /
  0.7 / 0.95`, `trades_beta 6.0` — identical to the 22:55Z seed-0 EMA entry.
  Source `cd0b571e4685…`, `dirty: false`, empty-diff hash; `external.lock`
  `05cfce4c…` with all five upstream pins at their contracted commits and clean.
  All seven declared artifacts exist at their live paths and as
  content-addressed copies. `evaluation-lineage.json`'s `training_config_hash`
  `350881af…` equals the training bundle's `config_hash` and both rows'
  `config_hash`, migration `applied: []`, `train_run_id`
  `adr-campaign-v1-ferret-cifar10_r18_trades_adr-s2`. Checkpoint SHA-256 as
  declared: `best-ema.pt` `85f9176e…`, `last.pt` `f7ab5d70…`. The `last.pt` hash
  equals the one the seed-2 model-weights evaluation declared for the same file
  (22:55Z entry), so two separate processes agree on it. **One check was not
  run:** the sandbox refuses `sha256sum` outside the repository root, so no hash
  was recomputed here.

  **Attack sanity.** AutoAttack ≤ CE-PGD-20 ≤ clean on both rows.
  `canon-eval-lane-AB.log:204,409` reports `max Linf perturbation: 0.03137`,
  `nan in tensor: 0`, `max 1.0 / min 0.0` for both checkpoints. Stage sequence
  APGD-CE 53.77 → APGD-T 49.45 → FAB-T 49.45 → SQUARE 49.45 (`best-ema.pt`) and
  51.71 → 48.20 → 48.19 → 48.19 (`last.pt`). Printed vs recorded AutoAttack:
  49.45 vs **49.46** (+1 example) and 48.19 vs **48.22** (+3 examples) — the
  same direction as every earlier packet-0010 comparison, inside its 0.09 pp
  ceiling. This entry does not recount packet 0010's running tally.

  **Arm 5 now has all three seeds on both axes** (AutoAttack, percent; model
  numbers from the 22:55Z entries):

  | seed | model `best.pt` | EMA `best-ema.pt` | EMA − model | model `last.pt` | EMA `last.pt` |
  |---|---|---|---|---|---|
  | 0 | 49.19 | 49.54 | +0.35 | 48.11 | 48.36 |
  | 1 | 48.45 | 49.49 | +1.04 | 47.68 | 47.69 |
  | 2 | 48.55 | 49.46 | +0.91 | 48.09 | 48.22 |
  | mean | 48.73 | **49.50** | +0.77 | 47.96 | 48.09 |

  The EMA axis's three seeds span **0.08 pp**, against 0.74 pp on the model
  axis for the same three training runs. EMA beats model weights at the
  selected checkpoint at all three seeds; at the last checkpoint the
  difference is +0.25 / +0.01 / +0.13 pp, inside the ≈0.71 pp two-run sampling
  noise. That is arm 3's pattern (21:48Z entry: +0.86 pp at the selected
  checkpoint): the EMA advantage sits in which epoch is selected, not in the
  final average. Directional for these three runs.

  **TRADES leg, EMA side, not like-for-like** (arm 5 EMA against arm 4 model
  weights, best checkpoint): **+1.67 / +1.60 / +2.56 pp**, mean **+1.94 pp**.
  This bundles ADR, weight averaging and Nesterov on the treatment side only.
  The seed-2 pair is the largest because arm 4 seed 2's early best epoch (102)
  gives it the arm's lowest number (23:56Z entry).

  **Census at 00:02:21Z, from a full-root watcher scan (fresh cursor, 52
  bundle rows).** Training: **20 of 20** terminal and successful.
  Model-weights contract evaluations: **20 of 20**. EMA-weights evaluations:
  **9 of 9** (this run is the ninth). The remaining 3 rows are the 2026-09-09
  `early-check-eval-*` diagnostics under `r18_adr-s1`. **0 non-terminal, 0
  failed.** Every arm is complete on every axis its contract defines. **M1c is
  ticked**: all 20 new training jobs and all 29 contract evaluations were
  launched and are terminal and successful. **M2 stays open**: its lineage half
  asks for every artifact and checkpoint hash to be verified, and the only
  process here that recomputes them is the aggregator's `_verify_bundle`,
  which has not run.

  **Why nothing is imported.** `scripts/aggregate_adr_cifar10_replication.py:257`
  still reads each training artifact from the manifest's absolute `path`. For
  this run that is
  `runs/adr-campaign-v1-ferret-cifar10_r18_trades_adr-s2/outputs/train/…`, and
  no `runs/adr-campaign-v1-ferret-*` directory exists on Hamster. Both
  artifacts do exist locally as content-addressed copies under
  `run-bundle/artifacts/<name>/<sha256>/` (`local_path`), so a `local_path`
  fallback that still hashes the file would unblock M3 without weakening the
  check. The fix is a code change in the aggregator; it needs a test and a
  new pinned worktree to run from, since `cd0b571e4685` predates both the
  `dir_prefix` fix and this one. **This postrun did not make it.** No
  evidence-ledger row is owed, since a ledger row accompanies a record, and
  the campaign's decision packet remains M3's deliverable.
  **This postrun started nothing and stopped nothing.**
