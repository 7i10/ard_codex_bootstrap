...
| Controlled TRADES seed 0（修正後） | train、official PGD、AA完了 | official best: clean 82.35 / PGD-20 50.66 / AA 47.87; last: clean 82.24 / PGD-20 47.49 / AA 44.99 | clean 目標の detach を外した後の値。欠陥版から AA で +2.73 pp。`docs/debugging/0028-trades-clean-target-detached.md` |
| Controlled TRADES seed 0（修正後）| train のみ完了。**official PGD / AA は未実施** | **validation** best (epoch 150): clean 83.32 / CE-PGD-20 51.76; last (epoch 199): clean 83.28 / CE-PGD-20 48.66 | 上の行を**まだ置き換えない**。上は official test、この行は validation であり、比較可能な量ではない。同一 config・同一 seed の欠陥版 validation（best 82.00 / 48.62、last 83.10 / 45.74）に対し robust は best `+3.14 pp`、last `+2.92 pp`。best-to-last gap は 3.10 pp で、PGD-AT の 8.54 pp より小さいという TRADES 本来の性質を保ったまま robust が上がっている。run `trades-fix-v1-s0-attempt2`、SHA `ee9ced0`。official 評価の判断は [`decision 0005`](decisions/0005-trades-fix-official-evaluation.md) |
