...
| ARD | this repository: RSLAD + I100 schedule, Chen LTD WRN-34-10 teacher (AA 56.94, no extra data) | RN-18 | **53.08 / 53.02 / 53.19** last, 53.08 / 53.02 / 52.96 best | 3 unused confirmation seeds | 200 epochs, PGD-10 | VERIFIED, `docs/archive/ard-distillation-2026/ERT_RSLAD_I100_OFFICIAL_TEST_AUTOATTACK.md` |
...
| ARD | this repository: RSLAD canonical, seed 0 | RN-18 | 51.90 best / 51.78 last | 1 | 200 epochs | VERIFIED, `docs/archive/ard-distillation-2026/EXPERIMENT_DASHBOARD.md` section 4 |
...
2. **A demonstrated bit-deterministic pipeline** (`REF2 - REF1 = 0.0000`), so that any difference between two runs is attributable to a named random stream. VERIFIED, `docs/archive/ard-distillation-2026/EVIDENCE_RECLASSIFICATION.md` row G1. No paper examined claims this.
...
**Power.** *Corrected 2026-09-08.* The SD used here was an assumed 0.3 pp; the measured one is larger. `docs/archive/ard-distillation-2026/MEASUREMENT_DESIGN.md` line 298 records I100 at epoch 199 across five seeds (61.18 / 60.56 / 60.90 / 60.60 / 60.30) with a per-run SD of **0.339 pp**. The standard error of a difference between two five-seed means is `sigma * sqrt(2/5) = 0.632 * sigma`, so 0.214 pp, and the minimum detectable difference at 80 % power and the 5 % level is `2.80 * 0.214 = ` **0.60 pp**, not 0.5. Declare 0.6 pp, and preregister "inconclusive" as a third reportable outcome distinct from a null. The original sentence, kept for the record: with independent seeds and SD about 0.3 pp, five seeds per arm give a minimum detectable difference of about 0.5 pp; with common random numbers across arms it is somewhat smaller but not measured for cross-method contrasts. A teacher effect of 1 pp is resolvable; one of 0.3 pp is not, and a null at that size is still an answer ("the teacher adds less than half a point"). INFERRED from section 3.
...
Repository records: `docs/archive/ard-distillation-2026/ERT_RSLAD_I100_OFFICIAL_TEST_AUTOATTACK.md` (record `docs/experiments/ard_i100_official_test_autoattack_v1.json`); `docs/archive/ard-distillation-2026/EXPERIMENT_DASHBOARD.md` section 4 and lines 165-167; `docs/POST_DECAY_FLOOR.md` (record `ard_post_decay_floor_v1.json`); `docs/archive/ard-distillation-2026/ERT_RSLAD_FIVE_SEED_STOCHASTICITY.md`; `docs/archive/ard-distillation-2026/EVIDENCE_RECLASSIFICATION.md`; `docs/archive/ard-distillation-2026/METHOD_DIRECTIONS.md`; `docs/UPSTREAM_BASELINES.md` sections 7, 8, 10; `docs/REPRODUCTION_STATUS.md`; `src/ard/data/datasets.py`; `configs/scientific/`.
